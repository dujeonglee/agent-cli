...
실측 근거: 35B bakeoff 0-op 캡처의 17%가 md_array 회귀 (PHASE2.md §8).
...
# 실측 캡처 (35B, 2026-07-17) — md_array 회귀 예1
MD_ARRAY_LEAK = (
    "## Thought\n"
...
# 실측 캡처 예5 — md_array 회귀 + 미닫힘 배열 (md_array 수리 기계가 살림)
...
def production_default(monkeypatch):
    ...
    순서(DEFAULT 우선)를 검증하는 테스트는 프로덕션 기본(md_array)으로."""
    monkeypatch.setattr("agent_cli.wire_formats.DEFAULT_WIRE_FORMAT", "md_array")


class TestTryForeignParse:
    def test_md_array_leak_rescued_in_xml_fc_stream(self, production_default):
        rescued = try_foreign_parse(get_wf("xml_fc"), MD_ARRAY_LEAK)
        assert rescued is not None
        turn, source = rescued
        assert source == "md_array"
    ...
    def test_unclosed_array_leak_rescued_via_md_array_repair(self, production_default):
    ...
    def test_xml_fc_leak_rescued_in_md_array_stream(self):
        # 대칭: md_array-바인딩 스트림에 xml_fc emission
        xml_emission = (
            "reading now.\n\n<tool_call>\n<function=read_file>\n"
            "<parameter=path>a.py</parameter>\n</function>\n</tool_call>"
        )
        rescued = try_foreign_parse(get_wf("md_array"), xml_emission)
        assert rescued is not None
        turn, source = rescued
    ...
    def test_bound_format_never_selected(self, production_default):
        ...
        rescued = try_foreign_parse(get_wf("md_array"), MD_ARRAY_LEAK)
        assert rescued is None or rescued[1] != "md_array"

    def test_default_format_tried_first(self, production_default):
        # 순서 결정성: DEFAULT(md_array) 가 최우선 (누출 최빈 포맷)
        rescued = try_foreign_parse(get_wf("xml_fc"), MD_ARRAY_LEAK)
        assert rescued is not None and rescued[1] == "md_array"
...
class TestDispatchRescue:
    def test_leaked_md_array_op_executes_in_xml_fc_loop(
    ...
    def test_rescue_labeled_foreign_format_in_turns_jsonl(
        ...
        assert any(p.startswith("foreign_parse:md_array") for p in prims)

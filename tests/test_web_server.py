...
class TestStaticUI:
    ...
    def test_index_html_is_served(self, server_and_client):
        ...
        body = resp.text
        # Sanity: the page must reference the app script (SSE + input
        # flow). v7.7.0: 입장 게이트(boot.js) 제거 — 직접 로드로 복귀.
    ...
    def test_every_js_class_has_a_css_rule(self, server_and_client):
        ...
        unstyled_ok = {
            ...
            "gen-label",
            # 트리의 자식 컨테이너 — 위치/여백은 부모 `.dl-row` 가 정한다
            "dl-kids",
    ...
    def test_every_emitted_event_has_a_listener(self, server_and_client):
        ...
        **리스너는 이벤트 이름을 리터럴로 써야 한다.** 표로 접어
        `[[ev, ce]].forEach(p => es.addEventListener(p[0], …))` 같이 쓰면 몇 줄은
        줄지만 이 가드가 그 리스너들을 못 보게 되어 조용히 무력해진다(실제로
        중계 5개를 접었다가 여기서 잡혔다). 20줄보다 가드가 값지다."""
        import re
    ...
    def test_app_js_is_served(self, server_and_client):
        ...
        # The SSE must be released when the page is hidden (navigation / tab
        # close / bfcache) so the server stops counting a departed viewer and
        # idle-reap can fire; bfcache restore reloads for a fresh connection.
        assert "pagehide" in body
        assert "es.close()" in body
...
class TestTabPresenceBeacon:
    ...
    (v7.7.0: 입장 게이트 제거 후 비콘만 app.js 에 존속 — board 열기
    게이트의 유일한 데이터 소스)."""

    def test_beacon_wired_in_app_js(self, server_and_client):
        _, _, client = server_and_client
        js = client.get("/static/app.js").text
...
class TestTeammateEndpoints:
    """P4: 대화 창의 인간 개입 — input(닉네임 attribution)/kill."""
    ...
    class _FakeRegistry:
        ...
        def request(self, key, message, *, author="main"):
            if key == "agt-missing":
                return f"unknown teammate '{key}'"
            self.requests.append((key, message, author))
            return ""
    ...
    def test_input_routes_with_nickname_attribution(self, server_and_client):
        ...
        assert reg.requests == [("agt-1", "do it", "user:bob")]

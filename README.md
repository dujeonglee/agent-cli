...
- **SSE 입장 제어 (v7.5.0)** — 페이지가 SSE 를 연결하기 **전에**(boot.js) 자기 origin 의 연결 보유 탭 수를 세어, 이미 5개면 연결하지 않고 파킹 화면("Connection limit — parked") + 5초 주기 자동 재시도. "보유 연결 ≤5" 불변식이 자가 강제되어 항상 1슬롯이 비므로 새 탭 로드가 굶지 않음 — URL 직접 입력·브라우저 세션 복원·탭 복제 등 board 열기 게이트 밖 진입 전부 커버(무한 스피너 소멸). 탭 하나가 닫히면 파킹 탭이 자동 입장

...
```json
{
  ...
  "api_key": "",
  "default_model": "Qwen3.6-35B-A3B-8bit"
}
```

**모델 이름은 부팅 때 검증됩니다 (v9.5.0)**. 서버의 `/v1/models` 에 그 이름이
없으면 **그 자리에서 멈추고** 사용 가능한 목록을 보여줍니다 — 터미널에서 실행
중이면 골라서 바로 저장할 수 있고, 다음부터 묻지 않습니다. 종전에는 없어진
모델 이름이 첫 LLM 호출까지 살아남아 `404 Model not found` 로 터졌고, 상주
에이전트에서는 대화 한복판의 거부 카드로 보였습니다.

`/v1/models` 를 읽을 수 없는 경우(오프라인, 그 엔드포인트가 없는 서버)에는
**경고만 남기고 진행**합니다 — 확인할 수 없는 것을 틀렸다고 단정하면 오프라인
작업이 통째로 막히기 때문입니다. 에이전트 역할 파일(`.agent-cli/agents/*.md`)이
`model:` 로 다른 모델을 지정한 경우에도 **spawn 시점에** 같은 검사를 합니다.

`default_model` 을 비워두면 추측하지 않고 "모델을 지정하라"고 말합니다 —
`gpt-4o` 같은 이름을 몰래 끼워 넣던 동작은 v9.5.0 에서 제거됐습니다(로컬
서버에는 있을 리 없는 이름이라, 원인을 가리기만 했습니다). 설계 근거는
`docs/model-resolution/DESIGN.md`.
...
- 웹 헤더의 📝 **Directive Editor** 가 이 스코프 구조를 탭(Common/Main/Sub-agents)으로 그대로 편집하며, ✨ 버튼으로 대략적 의도를 LLM 초안으로 만들 수 있습니다.
...
> **LLM 요청 재시도**는 고정 상수로 동작한다(더 이상 env 로 조정 불가). 네트워크 에러(Timeout / ConnectionError)는 최대 10회, 일시적 게이트웨이 5xx(**502/503/504**)는 **독립 카운터**로 최대 3회, 재시도 간격 1초. 4xx·bare 500 은 무재시도. 스트리밍은 헤더 대기를 30초로 바운드하고, body 가 10분 연속 침묵하면 연결을 끊고 재전송한다 — **총 4회**(첫 전송 포함)까지, 즉 최장 40분 뒤 실패. 이 두 축만은 고정이 아니라 세션 노브다: 한도는 `--stall`, 횟수는 `--stall-attempts`(웹은 ⏳ 칩 하나에 두 입력, 팝업이 곱한 최대 대기를 같이 보여준다). 대기 중에는 남은 시간과 `시도 n/4` 가 웹·CLI 양쪽에 표시되고, 반복되는 대기는 한 줄을 제자리 갱신하며 재전송·실패만 기록으로 남는다. 스트림이 **중간에 끊긴 경우**(서버가 종결 청크 없이 연결을 닫음 — 재시작·프록시 끊김·OOM)도 재전송한다: 고정 10회, 1초 간격, 무진전 예산과 **별개**(즉시 실패라 공유하면 순식간에 소진된다). 재전송 전에는 항상 부분 출력을 걷는다 — 재전송은 생성을 처음부터 다시 하므로 남겨두면 새 출력이 옛 부분 뒤에 이어붙어 보인다. 자세한 동작은 `agent_cli/providers/http.py` 참고.
>
> **세션 노브의 수명**: 웹 헤더의 🗜️ 👥 ⏳ 🧠 ⚡ 는 전부 **세션 한정**(서버 메모리)이다. 브라우저 창을 닫거나 다른 기기로 다시 붙어도 값은 유지되지만(sticky 스냅샷), **프로세스를 재시작하면 기본값으로 돌아간다** — `--resume` 으로도 복원되지 않는다(세션 메타에 담지 않는다; 특히 ⚡ 자동 승인처럼 안전 게이트를 끄는 설정이 재시작을 넘어 살아남으면 위험하다). 재시작 후에도 값을 유지하려면 위 env 또는 CLI 인자로 부팅 시점에 지정한다. 🎨 테마·닉네임만 브라우저 localStorage 라 예외적으로 계속 남는다.
>
...
- **shell**: 명령에서 절대경로·`../` 탈출 토큰을 **best-effort** 로 추출해 검사합니다.
  ...
  `rm`/`rmdir`/`mv` 를 강조하는 것과 동일 — CLI 볼드-레드, 웹 `.danger` 스팬).
...
오래 도는 작업을 백그라운드로 띄워 놓고 **조건이 맞으면 보고받습니다.**

```
shell("nohup ./build.sh > /tmp/b.log 2>&1 & echo $!")
monitor(add, when={"type":"match","file":"/tmp/b.log","pattern":"ERROR|FAILED"})
```

- **조건 3종**: `match`(파일의 새 줄 + 정규식) · `silence`(파일이 N초간
  멈추면 — **침묵은 성공이 아닙니다**, 죽은 스크립트를 잡습니다) ·
  `command`(명령을 주기적으로 돌려 exit 0 이면 발화, stdout 이 보고 본문)
- **알림은 자동**입니다 — 폴링하지 마세요. 조건이 맞으면 관찰로 배달되고,
  쉬고 있었다면 그 순간 깨어납니다
- **건 쪽으로 갑니다** (v9.14.0): main 이 걸면 main 의 대화로, 상주
  에이전트가 걸면 **그 에이전트의 런**으로. 그 안에서 돌던 스킬·delegate 가
  건 것도 부모 에이전트에게 갑니다. 종전엔 주소가 없어 에이전트가 건 감시의
  보고까지 main 이 가져갔습니다
- `deadline` 기본 2시간(60초~24시간), `once` 기본 참(한 번 보고하고 은퇴).
  **해제될 때도 알립니다** — 만료든 은퇴든 조용히 사라지지 않습니다
- 선택 `run="<명령>"` 은 보고 **전에** 실행되고 결과가 보고문에 실립니다
  (알림 없는 실행은 만들 수 없습니다 — 새벽에 명령이 돌았는데 아무도 모르는
  상태를 막기 위해서입니다)
- `command`/`run` 은 **등록할 때** 확인받습니다. 발화할 땐 사람이 없으니까요
- 내장 에이전트 프로파일 여섯 개도 monitor 를 씁니다 (v9.14.0). 권한이
  넓어진 것은 아닙니다 — 여섯 다 이미 `shell` 을 가지고 있고, monitor 는
  같은 셸 권한에 **보고가 반드시 따라오는** 형태입니다
- 세션이 끝나면 모니터도 끝납니다. 다음 세션엔 "복원되지 않았습니다"만
  알립니다 — board 세션에서 세션보다 오래 가는 주기 작업이 필요하면
  `schedule` 이 맞는 도구입니다

**백그라운드 실행 관용구** — `2>&1` 을 빠뜨리면 `shell` 이 타임아웃까지
막힙니다. 출력 버퍼링도 풀어야 건강한 스크립트가 침묵으로 보이지 않습니다:

```bash
nohup CMD > /tmp/x.log 2>&1 & echo $!     # 양쪽 스트림 모두 리다이렉션
python -u / PYTHONUNBUFFERED=1 / stdbuf -oL
( CMD; echo "EXIT:$?" ) > /tmp/x.log 2>&1 &   # 종료 코드가 필요하면
```
...
resume 시 `web`은 이전 대화(turn)를 UI에 그대로 재생해 어디서 끊겼는지 바로 확인할 수 있습니다 (중간 도구 호출/관찰 포함). **Team 뷰의 활동 막대**(스킬 밴드·일회성 run·팀원 작업 구간)도 함께 복구됩니다 — 이 이벤트는 대화 히스토리에 없어 세션 디렉토리의 `scopes.jsonl` 사이드카에서 재생됩니다. **사용자 레인과 ✓ 회신 화살표도 복구됩니다** (히스토리 user 레코드의 작성자 + complete 귀속을 재생에서 동일 규칙으로 재도출; v8.2.0 이전에 기록된 세션은 작성자 정보가 없어 화살표 없이 카드만 복원).
...
agent-cli 인스턴스 하나를 단일 세션으로 브라우저 UI 에 노출. **기본 바인드는 loopback(`127.0.0.1`)** — LAN 노출은 `--host 0.0.0.0` opt-in(웹 UI 가 셸을 구동할 수 있으니 신뢰된 네트워크에서만). 자동 토큰 생성(또는 `--token` 지정) — 토큰은 시작 URL 에 한 번 실려 오고, 서버가 그걸 **HttpOnly 쿠키**로 교환한 뒤엔 브라우저 URL 에서 사라집니다(주소창·로그·Referer 누출 방지). 다른 사람에게 접속을 공유하려면 헤더의 **🔗 버튼**으로 토큰 포함 링크를 복사해 전달하세요(주소창엔 토큰이 없으므로 주소창 복사로는 공유 안 됨). **다중 뷰어 (모두 동등)**: 모든 탭이 스트림을 보고 **모두 입력·큐 가능**(controller/observer 구분 없음). 각 탭은 접속 시 **재미있는 기본 닉네임**이 채워진 입력으로 이름을 정할 수 있고(✕로 기본값 유지, 한 번 정하면 다음 접속에 기억), 접속자 로스터에 닉네임이 표시됩니다. 닉네임은 접속 후에도 로스터 옆 **✎ 버튼**으로 언제든 다시 열어 변경할 수 있습니다(현재 닉네임이 채워진 채 재노출 → 즉시 로스터에 반영). **메시지 큐**: 에이전트가 실행 중이어도 메시지를 보내면 **큐에 쌓여 실시간 표시**되고, **매 턴 경계에서 대기 중 큐 전부를 한 번에 주입(drain-all, v8.16.1)** — 여러 요청이 다음 턴 LLM 컨텍스트에 **함께** 들어가 모델이 모두 보고 이어갑니다(Claude Code 식 묶음 반영; 종전엔 턴당 1건씩 순차 스티어링). 자기가 큐한 메시지는 처리 전 ✕로 취소 가능. 주입되는 batch 앞에는 **"앞서 답하지 않은 요청도 함께 처리하라"는 시스템 notice 가 한 번 붙어(v8.14.2)**, 최신 것만 답하고 이전 요청을 흘리지 않도록 합니다(시스템 프롬프트 지침으로도 보강). **⚡ 자동 승인(확인 없이 실행, v8.17.0 · v8.18.0 헤더 이동)**: **헤더의 `⚡ 확인 없이 실행` 토글 버튼**으로 이 세션의 **위험 명령(`rm` 등)·워크스페이스 밖 접근 확인 프롬프트를 건너뛰고 자동 허용**할 수 있습니다(무엇이 실행됐는지는 타임라인에 관찰로 남음). **세션 한정·재시작 시 자동 해제**(런타임 전용)이며, 안전 게이트를 끄는 것이므로 신뢰된 작업에서만 켜세요. 기본은 꺼짐(확인 요청). **🧠 사고/추론 노력(v8.18.0)**: ctx 팝오버의 두 셀렉트로 **사고(thinking) on/off** 와 **reasoning effort(low/medium/high)** 를 세션 단위로 조절할 수 있습니다 — **다음 LLM 요청부터 반영**(공유 컨텍스트). 사고 끄기는 토큰 절약·속도↑(OpenAI 계열은 `chat_template_kwargs.enable_thinking` 지원 시 — Qwen/MLX 확인됨; Anthropic 은 `thinking` 블록 활성/비활성으로 반영). effort 는 provider 별로 번역됩니다 — **OpenAI 계열**은 `reasoning_effort` 필드(백엔드가 존중할 때 유효), **Anthropic**은 effort 를 `thinking.budget_tokens` 로 번역(low=4096 / medium=16384 / high=32768; Anthropic 은 effort enum 이 없어 budget 이 유일 레버). 기본은 `자동`(모델 기본값 유지). **모델이 사고를 지원하지 않으면(`supports_thinking=false`) 이 컨트롤은 잠깁니다** — provider 가 사고 파라미터를 아예 보내지 않아(엄격 백엔드 400 방지, v8.21.1) 조정 자체가 무의미하기 때문입니다. **📐 문법 제약(v9.24.0)**: 서버가 문법 제약 디코딩(`guided_grammar`, omlx/xgrammar·vLLM·llama.cpp)을 지원하면 매 요청에 wire format 의 EBNF 를 실어 **도구 호출 형식을 생성 단계에서 강제**합니다 — 호출이 나오는 한 형식 오류·모르는 도구·모르는 인자·본문 안의 추론 폭주가 아예 나올 수 없습니다(원칙은 **하니스가 구제할 수 있는 모양은 문법이 막지 않는다** — 호출 없이 끝나는 턴, 빈 배열, JSON 문자열의 raw 개행, 태그 사이 공백은 모두 허용하고 종전대로 복구 계층·파서가 처리합니다. 호출을 필수로 하면 EOS 가 마스킹되어 산문 폭주·가짜 호출이 나오는 것을 실측)(사고 채널은 제약하지 않고, 산문·도구 본문 모두 길이 제한 없음 — 폭주는 `max_output_tokens` 와 복구 계층이 막습니다). 비용은 **생성 속도 저하** — 규칙을 xgrammar 의 빠른 경로에 맞춘 뒤 토큰당 마스크 비용이 두 형식 모두 0.1ms 아래라(실제 토크나이저 실측; 첫 판은 json 문자열 규칙 하나가 65ms/토큰이라 −37% 였음) 실측 −6% 수준입니다(첫 토큰 시간은 동일). 헤더 칩 📐 의 배지가 사실을 말합니다: `강제`(지원+켬) / `끔`(세션에서 끔) / `미지원`(감지 결과 서버가 강제 못 함 — 잠김) / `미확인`(`models.json` 에 지원 여부가 없음 — 잠김, 모델 감지를 다시 실행하면 채워짐). 팝오버의 셀렉트(켬/끔)는 지원으로 기록된 모델에서만 열리고, 다음 요청부터 반영되며 세션 한정입니다. 켜져 있으면 생성 중 줄과 턴 통계에 📐 가 붙어 속도가 왜 다른지 그 자리에서 읽히고, 🔍 프롬프트 인스펙터의 마지막 섹션 `Decoding grammar` 에서 전문을 볼 수 있습니다(`서버 강제 · 프롬프트 아님` 태그 — 토큰을 먹지 않습니다). 지원 여부는 `models.json` 의 `supports_grammar`(true/false) — **모델 감지(프로브) 때 한 번 판정해 저장**하고, 부트에서는 프로브하지 않습니다(모델 서버가 바쁠 때 그 요청 하나에 인스턴스 열기가 수십 초 밀리는 것을 실측). 키가 없는 기존 엔트리는 미확인으로 잠기며 감지를 다시 돌리면 채워집니다. 문법이 없는 서버에선 종전대로 텍스트 프로토콜 + 복구 계층이 돕니다. 기본값은 **켬(지원으로 기록된 모델에서)** 이고 `AGENT_CLI_GRAMMAR=off` 또는 칩으로 끕니다. 산문 규칙은 여는 **시퀀스**(`\n\n[` / `\n\n<tool_call>`)만 금지하므로 생각 속 `<`·`[`(정규식 lookbehind, 시프트, 인덱싱)는 자유입니다 — 문자를 통째로 막았던 첫 판은 Harbor A/B 에서 그런 토큰이 마스킹되어 추론이 뜻을 바꾸거나 폭주하는 것이 확인돼 릴리스 전에 고쳤습니다. 모든 사용자 요청은 닉네임 라벨로 task 로그에 누적됩니다. 시작 시 토큰 포함 URL이 출력되고, **로컬 bind 일 때만** 브라우저 자동 오픈(원격 `--host <ip>` 면 생략 — 아래 표 참조).

**대화 화면 — 간단 명료 · 투명성 (v9.4.0, docs/chat-ui)**: 종전엔 같은 대화를 `개요 · 흐름 · 전문` 세 가지로 그렸고, 정작 **투명성(도구 호출·reasoning)은 전문 드로어 안에만** 있었다. 이제 화면은 **하나**다. **모든 줄이 같은 리듬**을 가진다 — `아이콘 · 종류 · 한 줄 요약 · 펼침표시`:

```
💭 생각   테스트 러너를 먼저 확인한다
⚡ shell  pytest tests/ -q                      ▸
✓ shell  3753 passed, 27 skipped in 38.04s     ▸
```

**기본은 한 줄, 누르면 전문** — 다 보여주되 **깊이로** 보여준다(실패한 결과만 펼친 채 시작한다. 지금 봐야 하는 유일한 줄이라서). 요약은 도구마다 다르고, 출력은 **마지막** 유의미한 줄을 쓴다(셸·테스트·린트의 결론이 끝에 있다). **라이브 스트리밍은 버렸다** — 대신 `● 생성 중 · 4.8K tokens · 💭 사고 2.1K` 한 줄이 살아 있음을 알린다. 본문이 흐르지 않으니 카드가 자라며 화면이 튀지 않고, 사고 토큰이 따로 잡혀 러너웨이가 바로 보인다.

**대화 채널 — main 도 채널이다**: 입력창 위 칩 바에서 상대(`main` 또는 상주 에이전트)를 고르면 **화면이 그 대화만** 보여주고 입력도 그 상대에게 간다. 에이전트 창도 main 과 **똑같이 투명하다** — 생각·도구·결과가 다 보인다. 에이전트마다 key 로 정해지는 고유 이모지(🦊🐙🦉…)가 붙고, 칩엔 상태 dot(대기=초록·작업 중=주황·답변 대기=하늘·종료=회색)과 **✕ 종료 / ↻ 되살리기**가 있다.

**런 블록 (v9.23.0)**: 에이전트 채널의 단위는 런이다 — 런이 시작될 때 블록이 열리고, 받은 항목(들)이 머리(`← 본문 · 누가`), 단계가 몸통, 결과가 꼬리(최종답 카드에 `✓ (2.1s)`; 결과 없이 끝나면 상태 한 줄). 보낸 메시지는 `→ message / reply / answer` 도구 줄에 상대와 배달 상태(✓/✗)가 붙어 한 줄로 끝나고, 종전 `← 받음 / → 보냄 / ❓ 질문` 줄은 없다(같은 본문이 두 번 보이던 자리). 아직 시작 안 한 항목은 `⏳ 대기` 로 보이다가 런이 열리면 머리로 올라가고, 사람 창 배치는 머리가 N줄이다. 회신 없이 끝나 하네스가 런 요약을 대신 보낸 경우만 `⚠ 폴백` 줄. resume 뒤에도 같은 블록이 선다(런 경계는 `scopes.jsonl`, 도구 줄은 에이전트 history, 수신은 `conversation.jsonl`). **점프**는 main 의 `⚡ agent` 도구 줄의 상대 이름에서만 — 그 채널의 마지막 카드가 하이라이트된다(에이전트 쪽 상대 이름은 글자일 뿐 눌리지 않는다 — peer 만 되고 main·사람은 안 되던 반쪽 버튼을 걷어냈다). 점프는 네비게이션이 아니라 **엿보기**라 **돌아가기는 한 단계**다 — 새 점프가 이전 것을 덮고, 한 번 누르면 사라진다. skill(🪄)과 일회성 에이전트는 말을 걸 수 없으므로 칩이 아니라 **중첩 블록**(좌측 레일: 보라=skill, amber=일회성)으로 그려지고 머리줄로 접힌다.

**답변 트레이는 채널과 무관**: 상주 에이전트의 질문도, 메인 LLM 의 질문·위험 명령 확인도 입력창 위 트레이에 모인다 — 다른 대화를 보는 중에도 놓치지 않고 인라인으로 답한다(누가 먼저 답하면 모두에게서 사라진다). **입력창은 항상 채팅 전용**이라 "답변 대기라 입력이 잠기는" 혼선이 없다. 하단 대기 큐는 `⏳[닉네임]` 으로 표시되다가 **`✓주입됨`**/**`✕취소됨`** 영수증을 잠시 보여준다. **🔍 프롬프트 인스펙터도 현재 대화 상대를 따른다** — 에이전트 채널에서 누르면 그 에이전트의 시스템 프롬프트+컨텍스트를 보여준다.

> **v9.4.0 에서 없어진 것**: 흐름(팀 스윔레인) 뷰와 `개요 · 흐름 · 전문` 탭, 전문 드로어, 라이브 스트리밍 카드. 스윔레인이 하던 "누가 무엇을 하고 있나"는 **채널 칩의 상태 dot** 이, "어디서 왔나"는 **왕래 줄과 점프**가 대신한다 — 에이전트가 하나뿐인 흔한 경우에 스윔레인은 값을 못 했고, 같은 대화를 세 벌 그리는 비용만 남았다.
...
> **헤더 (v8.57.0):** 토큰 상세(`ctx used/window (%) · ↓출력 · Σ↓누적 · 💭사고`)가 헤더에 상시 표시됩니다 — 사고(💭) 카운터는 생성 중 실시간 증가해 러너웨이가 바로 보입니다. 세션 노브(🗜️ 압축 임계 · 👥 에이전트 상한 · ⏳ Stall · 🧠 사고/노력)는 각자 현재 값을 배지로 단 칩이고, 클릭하면 그 노브만의 팝업이 열립니다. 창이 좁아지면 헤더가 자동으로 줄바꿈되어 모든 칩·아이콘이 계속 보입니다.

> **오케스트레이션(인스턴스 파일):** `web` 은 시작 시 `.agent-cli/sessions/<id>/web.json`(`{session_id, host, port, token, pid}`)을 기록하고 종료 시 제거합니다. `--idle-timeout` 의 자가종료와 짝을 이뤄 — 외부 서비스가 인스턴스를 온디맨드로 띄우고, 프로세스를 추적·종료하지 않고도, 파일 하나로 "이 세션 web 떠 있나/어디로" 를 알아 접속(또는 `--resume` 재기동)할 수 있습니다.
| `--resume <id>` | 이전 세션 이어서 실행 (`agent-cli sessions`로 ID 확인) | — |
| 기타 (`-p`, `-m`, `-n`, `--max-depth` 등) | `run`과 동일 | |

UI 기능:
- 좌측 어시스턴트 카드 (markdown 렌더링: 헤더 `#`/`##`/`###`, GFM 파이프 표, 순서/비순서 리스트, **bold**/*italic*, 인라인 코드, 펜스 코드 블록) + 우측 사용자 bubble (같은 렌더러). 강조는 CommonMark flanking 규칙을 따릅니다(v9.22.3) — 공백을 사이에 둔 별표(`0 9 * * 1-5`, `2 * 3`)는 글자 그대로 남습니다.
...
- **부재 중 질문 대기 (v7.8.0)**: `ask`/위험 shell 승인/워크스페이스 밖 접근 승인은 **지금 보는 사람이 없어도 대기**합니다 — 즉시 "(no response)"로 포기하지 않고, 세션 상태가 `awaiting_input` 이 되어 board 목록에 "답변 필요"로 표시되며, 나중에 접속하면 대기 중이던 질문이 그대로 떠서(재접속 replay) 답할 수 있습니다. 다중 방 운용에서 다른 방을 보는 사이 도착한 질문을 놓치지 않음. CLI(터미널 없음=답 불가)는 종전대로 즉시 폴백
...
- **압축 임계 슬라이더 (5.14.0)**: 토큰 현황 옆 `압축 80%` 슬라이더로 컨텍스트 **compaction 목표 비율**을 **세션 한정**으로 조절합니다(50~95%, 5% 스텝). 가용 컨텍스트(context_window − system − output)가 이 %에 닿으면 압축이 걸립니다 — 낮출수록 일찍/자주 압축(컨텍스트 적게 유지, 오버플로 여유↑), 높일수록 늦게 압축(컨텍스트 많이 유지). web 과 loop 이 같은 `ContextManager` 를 공유하므로 슬라이더를 놓는 즉시 **다음 LLM 콜에 반영**되고, 여러 뷰어의 슬라이더는 sticky 브로드캐스트로 동기화됩니다. main 대화 전용이며, 서브에이전트는 **spawn/resume 시점의 값을 상속**합니다(이미 도는 서브에 새 값을 적용하려면 재spawn). `GET/POST /api/compaction`, 토큰 인증. (재시작하면 기본 80% 로 복귀 — 영속 아님.)
...
**🎨 테마 피커:** 헤더의 🎨 버튼으로 드롭다운을 열어 **5가지 큐레이션 테마**(각 항목에 색 스와치 + 현재 ✓)를 고릅니다 — **Amber**(기본, 따뜻한 차콜+호박색), **Slate**(중성 다크), **Midnight**(딥 블루), **Terminal**(틸/near-black), **Light**. 선택은 즉시 적용 + 브라우저에 기억되고(localStorage), 첫 페인트 전에 적용되어 깜빡임이 없습니다. 다크 테마들은 **반투명 헤어라인 테두리**로 부드러운 경계를 씁니다. 모든 색은 CSS 디자인 토큰(`:root` 베이스 + `[data-theme="…"]` 오버라이드) 한 곳에서 파생되어, 테마 추가 = 토큰 블록 하나입니다. **버튼 체계 (5.5.0)**: 변형 4종 — `btn-primary`(화면당 주행동 하나: Send/저장/다운로드) / `btn-ghost`(hairline 기본값: 취소·보조 동작) / `btn-danger`(Stop·삭제 — 평시엔 조용, hover 에서만 채움) / `btn-icon`(헤더 도구·✕ 닫기) — 이 전부이며, 전역 베이스가 모든 버튼에 투명 배경+테마 글자색을 강제해 스타일 누락 버튼이 브라우저 기본 배경으로 '유령'이 되는 사고를 차단합니다. amber/terminal 은 밝은 accent 위 대비를 위해 어두운 잉크 글자(`--on-accent`)를 씁니다.

**🔍 프롬프트 인스펙션 (컨텍스트, 8.11.0 재설계):** 전역 인스펙터 메뉴 대신 **대화에서 바로** 프롬프트를 검사합니다 — 대화의 agent/skill 카드 헤더의 **🔍** 버튼을 누르면 그 스코프(그 카드의 `data-task-id`)의 프롬프트 인스펙션 드로어가 열립니다. **메인 루프**는 입력창 옆의 🔍(footer) 버튼으로 엽니다. 드로어는 **클릭한 스코프로 고정**(스코프 칩 없음)되고, 제목에 `agent`/`skill`/`main` 태그 + 이름이 표시됩니다. 내용은 **현재 턴에 실제로 전송된 시스템 프롬프트 + 그 시점 대화·관찰**을 섹션별로 보여줍니다: `System prompt` / `Conversation · observations` **스티키 그룹 헤더**로 나뉘고, 종류별 **의미색 좌측선**(system=accent, 대화=cyan)·섹션별 **점유율 바**·토큰 뱃지·**섹션 복사(⧉)**·**전체 접기/전체 복사**·검색 필터를 제공합니다. 열 때마다 최신 스냅샷을 가져오며(`GET /api/debug/prompt?task_id=`, 토큰 인증), 훅이 주입한 동적 섹션도 `Hook: <이름>`으로, compaction 요약·파일 목록도 함께 표시됩니다. **agent·skill 동일 뷰** — skill 도 카드 `data-task-id` 로 전체 스냅샷이 조회됩니다(8.11.0: skill 스코프 id 통일). LLM 호출 전이면 "아직 LLM 호출이 없습니다" 안내를 보여줍니다.

**📝 Directive Editor (헤더 📝):** 헤더의 📝 버튼은 **에이전트 지시문 편집 전용** 드로어입니다(프롬프트 뷰어와 분리 — 8.11.0). 프로젝트 `.agent-cli/DIRECTIVE.md` 를 편집합니다 — **에디터 구조 = 파일 구조**: `🌐 Common`(main+모든 서브에이전트) / `🎯 Main`(`## @main`) / `🤖 Sub-agents`(`## @agents`) 세 탭이 파일의 스코프 블록과 1:1 로 대응하고(분해·조립은 서버의 U-C 파서 단일 출처), 내용 있는 탭엔 ● 뱃지가 붙습니다. 각 탭은 **직접 편집**하거나, 아래 입력줄에 넣고 싶은 내용을 대략적으로 적고 **✨ 생성**을 누르면 LLM 이 그 청중용 directive 초안을 써서 탭에 반영합니다(기존 내용이 있으면 병합/개정) — 초안은 **미저장** 상태로 들어와 검토 후 저장. 생성은 **산문 직접 호출**(provider 1콜 + 구조적 새니타이저)입니다 — Qwen3.6 세대에서 CoT 누출 무발생을 재실측(16/16)한 뒤 단순화(5.7.0). 메인 대화/워커와 무접촉이라 타임라인에 흔적이 없고, **탭별로 동시에 여러 생성**을 걸 수 있습니다. 저장하면 세 탭이 하나의 파일로 조립되어 기록되고 **다음 LLM 호출부터 적용**(KV 캐시 prefix 리셋), 여러 인스턴스 간 브로드캐스트 동기화. API: `GET/POST /api/directives`(scopes 계약) · `POST /api/directives/generate`, 토큰 인증, **프로젝트 파일 전용**.
...
**📁 Workspace files (다운로드 + 업로드 + 삭제):** 헤더의 📁 버튼으로 우측 드로어를 열면 **워크스페이스 파일 트리**가 나타납니다(디렉토리는 ▶로 펼쳐 하위 탐색). **파일·디렉토리 옆에 크기 표시 — 디렉토리는 하위 전체를 합산한 재귀 크기**이고, 맨 위 루트 행(`📁 /`)에는 **워크스페이스 총 크기**가 표시됩니다. 한 드로어에서 다운로드·업로드·삭제를 모두 합니다.

- **다운로드**: 원하는 파일/디렉토리를 체크하거나 **트리 맨 위 루트 행(`📁 /`)의 체크박스로 워크스페이스 전체**를 선택한 뒤 **⬇ Download zip** 을 누르면 선택 항목이 임시 zip으로 압축되어 다운로드됩니다(전송 후 서버의 임시 파일은 삭제). 디렉토리를 고르면 그 하위 전체가, 파일을 고르면 그 파일만 담깁니다. 트리에서 폴더를 클릭하면 그 폴더가 업로드 대상이 됩니다(루트 행 클릭 = 루트).
- **업로드**: 드로어 안으로 **파일이나 폴더를 드래그-드롭**하거나 `파일`/`폴더` 버튼으로 선택하면 업로드됩니다(파일당 한 요청). **폴더를 드롭/선택하면 하위 구조를 그대로 보존**해 재귀로 업로드합니다(중간 디렉토리는 서버가 자동 생성). 업로드 위치는 **트리에서 클릭한 폴더**(클릭하면 하이라이트되고 "⬆ 업로드 대상" 에 표시), 클릭하지 않으면 워크스페이스 루트입니다. 결과(✓ 경로, 덮어쓴 경우 표시) 후 트리가 자동 갱신됩니다.
- **삭제 (🗑)**: 체크한 파일/디렉토리를 **🗑 Delete** 로 삭제합니다. **영구 삭제라 먼저 확인(confirm) 창**이 뜨고, 디렉토리는 하위까지 재귀 삭제됩니다. 삭제 후 트리 자동 갱신(개수·실패 보고).

보안 가드: 워크스페이스(서버 실행 디렉토리) 밖 경로는 서버가 차단하고, 업로드는 경로의 모든 세그먼트에서 `..`/절대경로 금지(traversal 차단)·resolve 후 워크스페이스 하위 재검증·대상 경로 존재 확인·토큰 인증을 적용합니다. 파일 크기 상한은 없습니다(v9.22.1 — 본문을 스트리밍으로 임시 파일에 쓰고 완료 시 제자리로 옮기므로 서버 메모리에 올리지 않고, 중단된 업로드는 워크스페이스에 남지 않습니다). **삭제는 가장 파괴적이라 가드가 가장 강합니다 — 워크스페이스 하위만, traversal 차단, 워크스페이스 루트 자체는 삭제 거부, 토큰 인증.** (드래그로 파일을 브라우저 *밖으로* 끌어내는 다운로드는 브라우저 제약상 미지원 — zip 버튼이 전 브라우저·디렉토리 경로입니다.)
...
- `/<skill> <args>` — 스킬 직접 실행 (예: `/optimize ./`)
  - **경로 오인 방지 (v4.51.1)**: `/Users/me/proj 분석해줘` 처럼 **경로로 시작하는 메시지는 명령이 아니라 일반 요청**으로 LLM 에 전달됩니다 — 첫 토큰에 두 번째 `/` 가 있거나, 실존 경로(`/etc`, `/tmp` 등)거나, 명령 문법(영숫자·`-`·`_`)이 아니면 스킬 디스패치를 건너뜁니다. 오타(`/no-such-skill`)는 여전히 not-found 안내.
...
`## Open Requests` 가 있는 런에서 `answers` 없이 완료하려 하면 — 요청이 하나여도 — **런당 한 번** 되돌려 다시 요구합니다(v9.22.0; 종전엔 단건 런을 "결과가 곧 그 답" 으로 봐줬는데 아래 혼합 런에서 틀립니다)(결과를 붙잡는 것이 아니라 같은 산출물의 형식만 고치게 하는 것이라, 완료가 다른 요청을 기다리지 않습니다). 되돌림 관찰이 결과 원문을 인용하므로 물린 `complete` 자체는 저장되지 않고, 성공하면 그 관찰도 컨텍스트에서 접힙니다 — 재시도는 기록하지 않습니다 (v9.21.2).

**미답이 남으면 런이 끝나지 않습니다 (v9.17.0)**. `answers` 가 일부만 가리키면 최종 답변은 **그대로 즉시** 나가고(붙잡지 않습니다 — 답은 이미 렌더되고 기록됩니다), 루프는 닫히지 않은 채 남은 요청을 가리켜 계속 일합니다. 주장된 요청은 회계에서 지워져 다음 턴 꼬리에서 사라집니다. 독촉은 **요청당 한 번**입니다 — 그래도 남으면 최종 답변 말미에 `⏳ … were not answered` 각주로 닫습니다. `answers` 를 아예 안 밝힌 경우는 무엇이 남았는지 알 수 없어 독촉하지 않고 `📋 … is undeclared` 로 적습니다.

**수신 유형별로 한 가지 규칙 (v9.22.0, v9.22.1 에서 코드도 하나로)**. 런이 `complete` 을 낼 때 하네스는 그 런이 **무엇을 받았는지**로 판정합니다 — main 이냐 상주 에이전트냐가 아니라. 루프는 원래 하나이고(`agent_cli/loop/`), v9.22.1 부터 회신 빚 장부도 하나입니다(레지스트리의 주소별 `ReplyDebt`) — main 은 배달된 `message` 를, 상주는 시작 항목을 같은 장부에 적고, 거부/독촉의 갈림은 "이 런에 사용자 요청이 있었는가" 뿐입니다. 그래서 main 의 🤝 웨이크 런(사용자 발화 없음)에 빚이 남으면 상주처럼 **거부**되고, 상주의 사람 창 런(사용자 요청 있음)에 빚이 남으면 main 처럼 **수락 후 독촉**됩니다. 질문을 건 런의 회신 억제는 **물어본 상대의 빚에만** 걸립니다(v9.22.2) — `ask(to="user")` 로 사람에게 물어도 원 요청자(main)에게 진 빚은 남아 독촉·폴백을 받습니다. 종전엔 사람에게 물으면 main 이 결과를 영영 못 받았습니다.

| 받은 것 | `complete` 이 오면 |
|---|---|
| 사용자 요청 (main 채팅 · 에이전트 대화창 입력) | 수락. `answers` 의 id 를 회계에서 지우고, 남은 요청은 독촉(요청당 한 번). `answers` 가 없으면 되돌림(런당 한 번). |
| 에이전트의 `message` / `ask` | 회신·답을 안 했으면 상주는 **거부**(결과 원문을 인용해 `reply`/`answer` 로 안내), main 은 **수락 후 독촉**. 3회까지 — 그래도 남으면 런은 끝나고 하네스가 정리합니다(질문은 "답 없이 런 종료" 로 닫히고, main 의 회신 빚은 라벨 붙은 런 요약을 폴백 배달). |
| 둘 다 섞임 | 수락 → `answers` 의 요청 정산 → 남은 `message`/`ask` 독촉. |

에이전트 대화창에 사람이 직접 입력한 요청도 main 과 같은 회계를 받습니다: 각 입력이 id 를 갖고 `## Open Requests` 에 오르며, 여러 건이 한 턴에 합쳐졌을 때 `complete` 은 `answers` 로 무엇에 답했는지 밝혀야 합니다. 그 런에서 `reply` 는 여전히 거부됩니다 — 요청자가 창을 보는 사람이라 `complete` 의 결과가 그대로 창에 뜹니다.

**어떤 요청에 대한 답인지 보입니다 (v9.19.0)**. 최종 답변 아래에 `답한 요청 [2] Ann 이건 REQ-A 와 별개다` 처럼 주장된 요청이 칩으로 붙습니다(CLI 는 `↳ answers [2] (Ann) …` 한 줄). 합쳐진 런에서는 카드 위치가 알려주지 않습니다 — 요청 둘이 한 턴에 들어오고 최종 답변은 하나이기 때문입니다. `--resume` 후에도 같은 칩이 뜹니다.
...
**형식 재시도는 기록하지 않습니다 (v9.23.2)**: 모델 출력에 도구 호출이 없거나 JSON 이 깨졌거나, 모르는 도구·틀린 인자를 보냈거나, 출력이 토큰 상한에서 잘렸을 때 — 그 실패한 출력은 컨텍스트에 저장하지 않고, 안내 메시지가 **앞뒤 일부만**(2000자 이하는 전문, 넘으면 머리 1400자 + 생략 표시 + 꼬리 600자) 인용합니다. 회복하면 안내도 접힙니다. 종전엔 실패 출력을 저장하고 안내에 전문을 다시 인용해, 폭주 출력 한 번(3만 자)이 컨텍스트에 두 벌로 쌓이고 모델이 그 망가진 조각을 흉내 내는 악순환이 있었습니다(Harbor 벤치 실측). 같은 도구를 같은 인자로 연달아 부를 때의 반복 경고(B1)도 같습니다(v9.23.4): 막힌 호출은 기록하지 않고, 한 턴에 여러 op 를 보낸 배치 안에서 걸리면 그 op 만 건너뛰고 나머지는 실행합니다.
...
**상주 에이전트의 `ask` 는 막지 않습니다** — 질문을 등록하고 즉시 돌아오며, 답은 새 메시지(= 새 런)로 옵니다. 주소는 기본적으로 **그 일을 시킨 쪽**(main 또는 사람)입니다. main 이 시킨 일에서 사람이 결정해야 하면 `to: "user"` 로 사람에게 직접 묻습니다 (v9.20.0) — 질문이 ❓ 트레이에 뜨고, 어느 뷰어가 답하든 그 답이 에이전트에게 새 런으로 갑니다. 이 파라미터가 없던 때는 그 질문이 main 에게 갔고, main 이 사용자 대신 답을 지어내는 일이 실측됐습니다.

```json
{"action": "ask", "action_input": {"question": "legacy 컬럼은 버릴까요, 남길까요?", "to": "user"}}
```

사람은 독촉받지 않습니다 — 답을 안 하면 질문은 트레이에 남고 에이전트는 다른 일감을 받습니다. 답은 도는 런에 닿을 수 없으므로 에이전트는 기다리거나 폴링하지 않고 `complete` 합니다.
...
- **`complete` 은 국소, 배달은 `message`/`reply` 뿐 (v9.21.0)**: 상주 에이전트의 `complete` 출력은 **아무에게도 자동 배달되지 않습니다** — 창과 history 에만 남습니다. 종전엔 런의 출력이 요청자에게 회신으로 자동 갔는데, 대화(끝말잇기 실측)에서 상대가 이미 `message` 로 답한 뒤 그 에코가 같은 턴의 두 번째 항목이 되어 한 명이 한 턴에 세 번 답하는 탈동기가 났습니다. 이제 요청(`expects_reply=True`)으로 시작한 런은 요청자에게 **회신 하나를 빚집니다**. `reply(text)` 가 갚습니다(돌려받을 것 없음, `to` 불필요). `message(to, text)` 는 상대에게 회신을 요구하는 새 요청이되, 내 요청자에게 보내면 내 빚도 갚습니다 — "내 수, 이제 네 차례". 빚진 게 없는 런(받은 회신·질문·독촉으로 시작)에서 `reply` 를 부르면 거부됩니다 — 그 자리가 "확인했습니다" 류 ack 가 새던 곳입니다. 갚지 않고 `complete` 하면 **최대 3회** 독촉합니다 — 거부 관찰이 그 `complete` 의 원문을 인용하며 "이게 답이었다면 `reply` 로 보내고, 남은 일이 없으면 `complete`" 로 안내하고, 물린 `complete` 자체는 컨텍스트에 남지 않습니다(v9.21.1). 그래도 없으면 하네스가 런 요약을 `(no explicit reply — …)` 라벨로 대신 보냅니다 — 요청자가 침묵을 받지는 않습니다. 실측(3인 끝말잇기 2라운드): 한 수 = 항목 1개, 독촉은 에이전트당 첫 런 1회 뒤 0.
- **배치 중간의 입력 검증 실패 (v9.21.0)**: 한 턴의 여러 op 중 하나가 스키마 검증에 걸리면 종전엔 배치가 **중단**되고 뒤의 op 가 말없이 버려졌습니다(실측: `message` 가 안 나갔는데 관찰은 `[1/2] OK [2/2] OK` 라 모델이 보냈다고 믿었음). 이제 그 op 만 `[k/N] — FAILED` 로 적고 나머지는 실행하며, 진짜 중단이 남으면 안 돌린 op 를 이름으로 알립니다.
...
- **이름과 주소**: 화면은 **이름을 앞에, key 를 흐리게 뒤에** 답니다 — `🦊 code-writer · ui  agt-c83d4f82`. 프로파일도 `name` 도 없는 즉석 에이전트는 key 해시로 정해지는 별명(`🦁 조용한 사자`)으로 폴백합니다. 별명은 **표시 전용**이고 부를 때 쓰는 주소는 언제나 key 입니다(`@agt-<key>`, 도구의 `"key"`) — 그래서 이름 옆에 늘 key 가 붙어 있습니다. 별명은 key 의 순수 함수라 resume·재접속에도 같은 얼굴이 유지되고, 사람 닉네임 풀이 영어(`Funky Gecko`)라 사람과 에이전트가 한눈에 갈립니다.
...
- **사용자 직접 명령 (`@` 구문)**: 채팅박스(웹)나 CLI 에서 —
  ...
  - `@agt-<key> <메시지>` — 특정 상주 에이전트에게 **직접** 전송 (`@agt-<key>` 만 치면 상태). 사용자 발신이므로 회신은 main LLM 대화에 섞이지 않고 — 웹은 🤝 창으로, **CLI 는 콘솔 라인**(`🦊 [code-writer · ui (agt-x) → user]`)으로 도착합니다.
...
`agent-cli web` 헤더의 🔌 칩이 부팅 시 붙인 MCP 서버 상태를 보여줍니다. 배지는 **붙은/전체**(`1/2`)라 하나라도 실패하면 숫자만 보고 알고, 칩 색이 경고색으로 바뀝니다. 클릭하면 서버별 전송 방식·도구 수·이름, 실패한 서버는 **이유**가 나옵니다 — 지금까지 이 정보는 stderr 에만 있어 웹에서는 볼 방법이 없었습니다. 등록된 서버가 없으면 칩은 숨겨집니다.

**읽기 전용**입니다. 등록·수정은 `agent-cli mcp` 에서만 합니다 — MCP 등록은 `command` + `args` 로 **로컬 프로세스를 실행**하는 설정이라, 링크를 가진 사람이 서버 머신에서 임의 명령을 돌릴 수 있게 되는 웹 UI 의 신뢰 수준을 넘습니다. 그래서 `POST /api/mcp` 는 존재하지 않습니다(테스트로 고정). 부팅 시점 상태라 세션 중엔 바뀌지 않습니다 — 서버를 추가하면 재시작해야 붙습니다.

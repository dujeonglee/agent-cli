...
> **헤더 (v8.57.0):** 토큰 상세(`ctx used/window (%) · ↓출력 · Σ↓누적 · 💭사고`)가 헤더에 상시 표시됩니다 — 사고(💭) 카운터는 생성 중 실시간 증가해 러너웨이가 바로 보입니다. 세션 노브(🗜️ 압축 임계 · 👥 에이전트 상한 · ⏳ Stall · 🧠 사고/노력)는 각자 현재 값을 배지로 단 칩이고, 클릭하면 그 노브만의 팝업이 열립니다. 창이 좁아지면 우선순위 낮은 칩·아이콘이 보따리(🎒)로 수납되고 넓히면 되나옵니다(한 줄 수렴 유지).

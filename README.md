...
**왕래와 점프**: 에이전트끼리·사람과 주고받은 메시지는 `← 받음 / → 보냄 / ❓ 질문` 줄로 남고 **오른쪽에 상대 이름**이 붙는다. 그 이름을 누르면 **그 채널로 건너뛰어** 대응하는 줄이 하이라이트된다(main→에이전트는 `⚡ agent` 호출에서, 에이전트끼리는 왕래 줄에서). 점프는 네비게이션이 아니라 **엿보기**라 **돌아가기는 한 단계**다 — 새 점프가 이전 것을 덮고, 한 번 누르면 사라진다. skill(🪄)과 일회성 에이전트는 말을 걸 수 없으므로 칩이 아니라 **중첩 블록**(좌측 레일: 보라=skill, amber=일회성)으로 그려지고 머리줄로 접힌다.

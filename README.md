...
- 모든 web UI 탭은 같은 origin 의 대시보드(agent-board)가 "연결을 잡고 있는 탭 수"를 셀 수 있게 BroadcastChannel ping 에 응답 — board 의 탭 가드(연결 고갈 예방) 데이터 소스
- **SSE 입장 제어 (v7.5.0, v7.6.0 에서 Web Locks 재구현)** — 페이지가 SSE 를 연결하기 **전에**(boot.js) **연결 슬롯 락**(`agentcli-conn-slot-0..4`, Web Locks API)을 원자적으로 획득. 슬롯이 없으면 연결하지 않고 파킹 화면을 띄우고 **모든 슬롯에 대기 요청**을 걸어둠 — 다른 탭이 닫히는 **순간** 브라우저가 락을 넘겨줘 즉시 자동 입장(폴링·재시도 버튼 없음). 슬롯 5 = 브라우저 origin 당 6연결 중 1개를 페이지 로드/클릭용으로 예약. "보유 ≤5" 불변식이 원자적으로 강제되어 URL 직접 입력·세션 복원·탭 복제·동시 대량 오픈까지 전부 커버(무한 스피너 소멸). 카운트도 `locks.query()` 정확값 — 샘플링 레이스 없음
- 열려 있는 탭들도 30초마다 보유 탭 수를 감시해 **5개면 상단 과밀 배너**(✕ 닫기, 수 증가 시 재경고) — 새 탭이 왜 파킹되는지 기존 탭 쪽에서도 보임

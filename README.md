...
**형식 재시도는 기록하지 않습니다 (v9.23.2)**: 모델 출력에 도구 호출이 없거나 JSON 이 깨졌거나, 모르는 도구·틀린 인자를 보냈거나, 출력이 토큰 상한에서 잘렸을 때 — 그 실패한 출력은 컨텍스트에 저장하지 않고, 안내 메시지가 **앞뒤 일부만**(2000자 이하는 전문, 넘으면 머리 1400자 + 생략 표시 + 꼬리 600자) 인용합니다. 회복하면 안내도 접힙니다. 종전엔 실패 출력을 저장하고 안내에 전문을 다시 인용해, 폭주 출력 한 번(3만 자)이 컨텍스트에 두 벌로 쌓이고 모델이 그 망가진 조각을 흉내 내는 악순환이 있었습니다(Harbor 벤치 실측).

...
agent-cli 인스턴스 하나를 단일 세션으로 LAN에 노출. 자동 토큰 생성(또는 `--token` 지정). **다중 뷰어 (모두 동등)**: 모든 탭이 스트림을 보고 **모두 입력·큐 가능**(controller/observer 구분 없음). 각 탭은 `identity` 이벤트로 conn_id를 받아 접속자 로스터의 "(you)" 표시·큐 메시지 소유에 사용. 시작 시 토큰 포함 URL이 출력되고 (선택적으로) 브라우저 자동 오픈.

...
| `--result-file` | 최종 답변(원문)을 지정 경로에 기록 — 렌더러 장식 없는 기계 소비용 (스크립팅·웹 ✨ 생성이 소비). 실패 시 파일 미생성 | (없음) |

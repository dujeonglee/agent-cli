...
resume 시 `web`은 이전 대화(turn)를 UI에 그대로 재생해 어디서 끊겼는지 바로 확인할 수 있습니다 (중간 도구 호출/관찰 포함). **Team 뷰의 활동 막대**(스킬 밴드·일회성 run·팀원 작업 구간)도 함께 복구됩니다 — 이 이벤트는 대화 히스토리에 없어 세션 디렉토리의 `scopes.jsonl` 사이드카에서 재생됩니다.
...
| `--turn-scoping` | **턴 스코핑.** `parallel` 일 때 각 턴의 시스템 프롬프트에 현재 요청을 지정합니다. 모델 순응에 의존하는 의미적 완화이며 파일 격리 보장은 아닙니다. 절제·비교가 필요하면 `--no-turn-scoping` 으로 끕니다. | 켜짐 (parallel 에서만 효과) |
| `--turn-local-context` | 다른 **진행 중** turn의 레코드를 현재 LLM 입력에서 제외합니다. 완료된 turn은 다음 추론 경계부터 다시 공유됩니다. | 꺼짐 (P1 실험 기능) |
...
- **관전 모드 (읽기 전용 링크, `--spectators`)**: `agent-cli web --spectators` 로 띄우면 평소 URL 옆에 **Watch URL** 이 하나 더 출력됩니다. 이 링크로 들어온 사람은 진행 상황을 실시간으로 **보기만** 합니다 — 입력창 대신 "👁 Watching — read-only" 안내가 뜨고, 메시지 전송·중단·설정 변경·워크스페이스 파일 접근·프롬프트 인스펙터는 전부 막힙니다(서버가 403). 시연·리뷰·인수인계처럼 "보여는 주되 건드리게 하고 싶지 않을 때" 쓰세요. 고정된 토큰이 필요하면 `--view-token <값>` (지정하면 관전 모드가 자동으로 켜집니다). **기본은 off** — 옵션을 안 주면 읽기 전용 자격증명 자체가 존재하지 않아 종전과 완전히 동일하게 동작합니다.
  - 접속자 로스터는 관전자를 `👁` 로 구분 표시하고, `GET /api/health` 는 `viewers`(전체) 와 `spectators`(그중 관전자) 를 함께 돌려줍니다 — "3명이 보고 있다"가 "3명이 질문에 답할 수 있다"로 잘못 읽히지 않게.
  - `--trust-local` 의 의미는 그대로입니다: 신뢰된 루프백 요청은 **전권**이며, 관전자로 강등되지 않습니다.
- **끊긴 연결은 놓친 부분만 이어받습니다 (증분 재생)**: 지하철·와이파이 전환 등으로 SSE 가 끊겼다 붙으면 브라우저가 마지막으로 받은 이벤트 번호(`Last-Event-ID`)를 자동으로 되돌려주고, 서버는 **그 뒤 이벤트만** 보냅니다 — 화면의 대화가 처음부터 다시 그려지거나 두 배로 늘어나지 않습니다. 이어받을 수 없는 경우(세션이 그 사이 재시작됐거나, 끊겨 있던 동안 재생 윈도우가 지나가 버린 경우)에만 화면을 비우고 전체를 다시 받습니다. 브라우저가 아닌 클라이언트는 `GET /api/stream?after=<마지막 id>` 로 같은 동작을 씁니다 (헤더를 못 붙이는 경우)
...
**⚡ Prompt Inspector:** 헤더의 ⚡ 버튼으로 우측 드로어를 열면 **현재 턴에 실제로 전송된 시스템 프롬프트**를 섹션별로 확인할 수 있습니다. 상단의 토큰 예산 스택바(섹션별 색·비율), 섹션 아코디언(이름·토큰 뱃지·본문), 검색 필터를 제공합니다. 열 때마다 최신 LLM 호출의 스냅샷을 가져오며(`GET /api/debug/prompt`, 토큰 인증), 훅이 주입한 동적 섹션도 `Hook: <이름>`으로 표시됩니다. 컨텍스트 압축(compaction)이 일어나면 그 요약과 파일 목록도 `⊙ Compaction summary / Files touched (user-injected)` 섹션으로 함께 보여, 모델이 실제로 받는 압축 컨텍스트를 검사할 수 있습니다. **정적 시스템 프롬프트 아래에는 `── 동적 컨텍스트 (대화 · 관찰) ──` 구분선과 함께 현재 컨텍스트 윈도우에 든 대화·관찰(`ctx.get_messages()` 의 system 제외분)이 메시지별 섹션으로 표시**되어, 시스템 프롬프트뿐 아니라 LLM 이 실제로 받는 전체 입력을 검사할 수 있습니다(메인 스코프 한정). **첫 메시지 전에도 채워집니다** — 시작 시 시스템 프롬프트를 미리 캡처하고(`Hook:` 동적 섹션은 첫 LLM 호출 후 채워짐), `--resume` 면 복원된 대화도 드로어를 열자마자 보입니다. **에이전트별 스코프**: 서브에이전트(agent run)가 돌면 드로어 상단에 `[Main] [explorer·1] [coder·2]` 칩 row가 나타나, 칩을 클릭하면 해당 서브에이전트가 실제로 받은 시스템 프롬프트로 전환되고 `Main`을 누르면 메인으로 돌아옵니다. 끝난 서브에이전트의 프롬프트도 사후 검사를 위해 남아 있으며, 칩의 ✕로 개별 제거할 수 있습니다(Main은 제거 불가). **📝 Directives 에디터 (청중 스코프 탭, 5.4.0)**: 드로어 상단에서 프로젝트 `.agent-cli/DIRECTIVE.md` 를 편집합니다 — **에디터 구조 = 파일 구조**: `🌐 공통`(main+모든 서브에이전트) / `🎯 Main`(`## @main`) / `🤖 서브에이전트`(`## @agents`) 세 탭이 파일의 스코프 블록과 1:1 로 대응하고(분해·조립은 서버의 U-C 파서 단일 출처), 내용 있는 탭엔 ● 뱃지가 붙습니다. 각 탭은 **직접 편집**하거나, 아래 입력줄에 넣고 싶은 내용을 대략적으로 적고 **✨ 생성**을 누르면 LLM 이 그 청중용 directive 초안을 써서 탭에 반영합니다(기존 내용이 있으면 병합/개정) — 초안은 **미저장** 상태로 들어와 검토 후 저장. 생성은 **산문 직접 호출**(provider 1콜 + 구조적 새니타이저)입니다 — Qwen3.6 세대에서 CoT 누출 무발생을 재실측(16/16)한 뒤 단순화(5.7.0). 메인 대화/워커와 무접촉이라 타임라인에 흔적이 없고, **탭별로 동시에 여러 생성**을 걸 수 있습니다. 새니타이저는 `<think>` 블록·코드 펜스만 벗기는 포장 제거 전용(내용 필터 없음)이라 미래 모델 교체로 누출이 재발해도 조용히 오염되지 않습니다. 저장하면 세 탭이 하나의 파일로 조립되어 기록되고 **다음 LLM 호출부터 적용**(KV 캐시 prefix 리셋), 여러 인스펙터 간 브로드캐스트 동기화. (구 성격/업무/지침 3축·프리셋 라이브러리·📋 템플릿은 5.4.0 에서 폐지 — "모든 방 공유" 지침은 전역 `~/.agent-cli/DIRECTIVE.md` 가 담당합니다.) API: `GET/POST /api/directives`(scopes 계약) · `POST /api/directives/generate`, 토큰 인증, **프로젝트 파일 전용**.

**동시 턴 (`--concurrency-contract parallel`, 실험):** 기본값 `serial` 에서는 메시지가 큐에 쌓였다가 **턴 경계마다 하나씩** 주입됩니다(위 "메시지 큐" 설명 그대로). `parallel` 로 켜면 여러 사용자의 메시지가 **각자의 턴으로 동시에 추론**되고, 대화 기록(`history.jsonl`)·컨텍스트는 그대로 하나를 공유합니다. 실행 중에도 Send 버튼이 잠기지 않으며, `/api/stop` 은 활성 턴 **전부**를 중단시킵니다.

병렬화되는 것은 **추론**이고, 부수효과는 `--lock-scope` 가 충돌 단위로 직렬화합니다(기본 `conflict`). 즉 **서로 다른 파일을 만지는 턴은 그대로 병렬로 돌고, 같은 파일·셸·삭제만 줄을 섭니다.** 공정성은 워크스페이스별 엄격 FIFO(추월 금지)라 파일 작업이 계속 들어와도 셸이 굶지 않습니다.

현재 알려진 한계:

- **락의 단위는 도구 호출 1건**입니다 — `read_file` 로 보고 `edit_file` 로 고치는 두 호출 사이에 다른 턴이 끼어들 수 있습니다(트랜잭션이 아닙니다). 편집은 hashline 참조로 검증되므로 조용히 덮어쓰이지는 않고, 참조가 낡으면 실패로 드러납니다.
- **여러 턴이 같은 일을 중복 수행할 수 있음** — 트랜스크립트가 공유되므로 각 턴이 다른 사용자의 요청까지 보고 함께 처리하려 들 수 있습니다(실측: 세 사용자가 각자 한 줄 추가를 요청하자 턴마다 세 줄씩 붙였습니다). 락은 **손상**을 막지 중복 **의도**를 막지는 않습니다.
- **화면 카드가 섞임** — 동시 턴의 출력이 하나의 타임라인에 인터리브됩니다(턴별 카드 그룹화는 미구현).
- **"누구에게 답할지"는 모델 판단** — 트랜스크립트가 공유되므로 어떤 턴의 프롬프트 끝이 **다른 사용자의 메시지**일 수 있습니다. 하네스는 `[닉네임]:` 라벨과 기록상의 응답↔질문 귀속(`reply_to`)까지 제공합니다.

**공정성 (per-user 1활성턴):** 한 사용자(연결 단위)는 **동시에 한 턴만** 갖습니다. cap 에 여유가 있어도 같은 사람의 두 번째 메시지는 첫 턴이 끝날 때까지 기다리고, 그 사이 **다른 사용자가 슬롯을 얻습니다** — 한 명이 연달아 보내 cap 을 독점하는 것을 막습니다(대기열 순서 자체는 보존되며, 스킵은 재정렬이 아닙니다).

**턴 단위 중단:**

| 엔드포인트 | 용도 |
|---|---|
| `GET /api/turns` | 지금 도는 턴 목록(id·발화자). 중단하려면 id 가 필요합니다. |
| `POST /api/turn/{id}/interrupt` | **그 턴 하나만** 중단 — 형제 턴은 그대로. `conn_id` 를 주면 소유자만 취소할 수 있습니다(큐 메시지 취소와 같은 규율). |
| `POST /api/stop` | 세션 전역 — 활성 턴 **전부** 중단(기존 동작). |

직렬 세션에서 앞의 두 엔드포인트는 빈 목록 대신 **409** 로 "이 세션엔 개별 턴이 없다"고 답합니다(빈 목록은 "지금 아무것도 안 돈다"와 구별되지 않으므로).

세션 상태 파일(`status.json`)에는 `parallel` 일 때만 `active_turns` 필드가 추가됩니다(직렬 세션은 종전과 동일한 shape).
...
P1 검증 게시가 필요한 자동화 클라이언트는 chat body에
`"write_paths":["result.txt"]`와 같은 키 집합을 정확히 갖는
`"expected_contents":{"result.txt":"..."}`를 함께 보낼 수 있습니다. 이
경우 해당 요청은 승인 경로 밖의 변경과 shell/복합 도구를 차단하고, 파일을
turn-local staging에 쓴 뒤 oracle을 통과한 경우에만 파일별 atomic replace로
게시합니다. 텍스트 oracle은 관례적인 마지막 줄바꿈 하나만 정규화하고 그 밖의
내용과 공백은 엄격히 비교합니다. oracle이 없으면 자동 게시하지 않습니다.
...
**산문 최종답변 자동 수용 (v7.14)**: 모델이 도구 호출 없이 **순수 자연어로 최종 답변만** 내고 `complete` op 을 빠뜨리면, 그 산문을 `result` 로 하는 완료로 수용합니다(넛지 왕복 없이 종료). 단 파싱 실패한 액션 잔해(`[{"action"…` 같은 깨진 op)·빈 출력은 **완료가 아니라** 재시도 넛지로 처리되어, 의도한 도구 호출이 조기 종결되지 않습니다.

...
**📎 Upload:** 헤더의 📎 버튼으로 드로어를 열고 **파일을 드래그-드롭하거나 선택** 하면 워크스페이스로 업로드됩니다(파일당 한 요청). **대상 디렉토리** 입력칸을 비우면 워크스페이스 루트, `src` 처럼 적으면 그 하위(이미 존재하는 디렉토리)로 들어갑니다. 각 파일의 업로드 결과(✓ 경로, 덮어쓴 경우 표시)가 드로어에 나타납니다. 보안 가드: 파일명은 경로 성분 없는 단일 이름만 허용(traversal 차단), 대상 경로는 워크스페이스 하위만, 파일당 50MB 제한, 토큰 인증. 같은 이름 파일은 덮어쓰되 결과에 명시합니다.

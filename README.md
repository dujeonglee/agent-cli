...
**점진적 공개 — 레벨 컨트롤 (v8.12.0)**: 헤더의 **`개요 · 흐름 · 전문`** 세그먼트 탭으로 정보 밀도를 고른다 (presentation-only — 서버·SSE·데이터 무변경, 기존 이벤트를 재배치할 뿐). **개요(기본)** 는 요약 화면 — **응답 블록 시퀀스**가 흐른다 (구 상단 ambient 줄은 정리됨: 에이전트 상태 dot 은 **채널 칩**(입력 위 agent 탭)에 직접(v8.33.0), 컨텍스트%·뷰어·모델은 헤더와 중복이라 제거). 내부적으로는 **플랫 로그**(v8.15.0) — 사용자 입력과 완료된 응답(메인 답변 + 직접 실행한 `/skill`·`@agent`의 결과)을 도착 순서대로만 기록하고, **[연속 사용자 쿼리 → 응답] 블록 그룹핑은 화면 그릴 때 순서로만 도출**한다(짝짓기를 상태로 저장하지 않아 "응답 대기" 정체·쿼리↔응답 오귀속이 원천적으로 없다). **최신 응답이 주인공(accent)**, 이전 응답은 dim 처리된다. hero 본문은 **마크다운으로 렌더**(제목 · 목록 · 표 · 코드블록 · 강조), 생성 중에는 **라이브 스트리밍 + 커서**로 자라난다. 완료된 응답마다 **⧉ 복사**(본문 클립보드 복사) · **▤ 전체 대화**(그 응답 카드로 타임라인 열기) 액션이 붙는다. **회신 주체 배지(v8.23.0)**: 각 응답 카드 좌상단에 주체를 표시한다 — 메인 LLM 은 조용한 `main` 배지, 직접 실행한 `@agent`/`/skill` 결과는 그 이름(에이전트별 고유 아이콘 + 이름, 예 `🐌 pudding`)을 accent 배지로 → 여러 에이전트와 대화하는 시나리오에서 어느 주체의 응답인지 한눈에 구분된다. **대화 채널(v8.25.0~v8.28.0)**: 입력창 위 **채널 칩 바로 대화 상대(`main` 또는 상주 에이전트)를 고른다** — 고르면 개요가 **그 대화만** 보여주고(여러 대화가 섞이지 않음) 입력도 그 상대에게 간다(agent 는 `/api/agent/<key>/input` 로). 발신 메시지엔 대상 배지(`→ 🐌 pudding`)가 붙는다. **에이전트마다 고유 아이콘(v8.32.0)** — key 로 결정되는 재미있는 이모지(🦊🐙🦉…)라 여러 에이전트를 한눈에 구분한다. **각 채널 칩엔 상태가 표시된다** — 상태 dot(대기 중=초록·작업 중=주황·답변 대기=하늘·종료=회색)·❓(답변 대기)·종료됨(취소선). 칩의 **✕(종료)/↻(되살리기)** 로 에이전트를 관리하고, 종료된 에이전트를 고르면 입력창이 잠기며 사망을 알린다. **🔍 프롬프트 인스펙터도 현재 대화 상대를 따른다(v8.30.0)** — agent 채널을 보는 중 🔍 를 누르면 main 이 아니라 **그 에이전트의 시스템 프롬프트+컨텍스트(토큰 수 포함)** 를 보여준다. (전체 조망은 **흐름** 스윔레인에서.) **글로벌 답변 트레이(v8.26.0~v8.27.0)**: 질문·확인은 **채널과 무관한 답변 트레이**(입력창 위)에 모인다 — **상주 에이전트의 질문(ask)**은 물론 **메인 LLM 의 질문·위험 명령 확인(confirm)**도 여기 뜬다(v8.27.0). 다른 에이전트 대화를 보는 중에도 놓치지 않고 **인라인으로 바로 답변**할 수 있고(누가 먼저 답하면 모두에게서 사라짐), 여러 주체가 동시에 물으면 트레이에 쌓인다. **입력창은 항상 채팅 전용**이라 "답변 대기 중이라 입력창이 잠기는" 혼선이 없다. 하단 **다음 턴 대기 큐**는 `⏳[닉네임]` 으로 표시되다가, 큐가 소진되면 **`✓주입됨`**(내가 보낸 것은 *내 요청 반영됨*) / 취소 시 **`✕취소됨`** 영수증을 잠시 보여줘 — 큐에서 그냥 사라져 반영 여부를 알 수 없던 문제를 해소한다. **흐름** 은 팀 스윔레인(아래 설명), **전문** 은 상세 대화(타임라인) 드로어 + 🔍 프롬프트 인스펙션이다. **전문은 개요/흐름을 대체하지 않고 그 위에 뜨는 사이드 패널 (v8.14.0)**: 개요/흐름을 보면서 **전문** 을 누르면 타임라인이 오른쪽에 붙고 왼쪽 뷰는 폭을 내줘 **나란히**(겹침 없이) 보인다 — 요약(개요)이나 팀 스윔레인(흐름)을 상세 대화와 동시에 확인할 수 있다(흐름의 스윔레인도 좁아진 폭에 맞춰 자동 압축, v8.14.1). 다시 누르거나 드로어의 ✕ 로 닫으면 원래 base(개요/흐름)가 그대로 남는다. **선택→고정 상세 패널 (Tier 2, v8.13.0)**: 흐름의 막대·화살표나 개요의 응답 헤더를 **클릭**하면 그 요소 하나만 집중해서 보여 주는 **우측 고정 패널**이 뜬다 (hover=툴팁 미리보기 · click=고정). 스코프의 결과·메시지 전문·관찰을 마크다운으로 보여 주고, 패널의 **[▤ 전체 타임라인]** 은 그 카드로 드로어를 열고 **[🔍 프롬프트]** 는 그 스코프의 인스펙터를 연다(Tier-3 승격). Esc 로 닫힌다.

**흐름(팀 스윔레인)**: 사용자·main·에이전트들의 활동과 메시지 흐름이 한눈에 보이는 개요이자 네비게이터. 상세한 대화(타임라인)는 **▤ 상세 대화** 버튼(또는 스윔레인의 막대·화살표·응답 독 클릭)으로 오른쪽에서 열리는 **온디맨드 드로어**입니다 — 클릭한 항목의 카드로 스크롤+하이라이트되어(같은 `task_id`/이벤트 시각으로 연결) 바로 맥락을 볼 수 있습니다. **👤 사용자 레인**: 맨 왼쪽 레인 하나에 모든 웹 사용자가 **multiplex** 되어 표시됩니다 — 각 메시지 마크 옆에 보낸 사람의 닉네임이 붙고, 요청은 실선 화살표로 main 에 꽂힙니다. **✓ 회신 화살표**: main 이 `complete` 하면 사용자 레인으로 **파선 화살표**가 그려지고, 라벨에 **그 런이 처리한 모든 요청자**(런을 시작한 사람 + 실행 중 조향 주입한 사람들, 예: `✓ Bob·두정`)가 표시됩니다 — hover 하면 답 요약. 🤝 에이전트 보고가 깨운 런도 **원 요청 승계**로 귀속됩니다 (v8.5.0): main 이 에이전트에게 일을 시킬 때 그 시점 요청자들이 요청에 스냅샷되고, 회신이 이를 되실어 와 회신을 접은 런의 최종답이 원 요청자에게 화살표로 연결됩니다 — 응답 전에 다른 사용자의 요청이 끼어들어도 요청↔회신 쌍에 묶여 교차 오염이 없습니다. 승계할 요청자가 없는 런만 무화살표(독에 무귀속 표기). **응답 독**: 입력창 위에 **최신 답변**이 3줄 미리보기로 상시 표시됩니다 — 귀속(`→ [Bob, 두정]`) 과 시각이 함께 뜨고, 새 답 도착 시 펄스로 알립니다. 답이 3줄을 넘을 때만 **펼치기** 버튼이 나타나 그 자리에서 전문을 읽을 수 있고, **본문을 클릭**하면 드로어가 그 답의 카드 위치로 열립니다. ask/confirm 응답 표면은 기존 그대로 입력창 옆입니다. **Team** 뷰는 팀 활동을 **세로 시퀀스 다이어그램**으로 그립니다 — **시간이 아래로** 흐르고 에이전트는 **세로 컬럼**(상단에 이름 헤더가 고정), 작업 구간은 세로 막대, 에이전트끼리 주고받은 메시지는 컬럼 사이 가로 화살표로 **요청·회신 왕복**이 모두 보입니다. 세로축은 **균등한 시간축이 아니라 이벤트 기준**입니다 — 요청/회신/작업 시작·종료 같은 각 이벤트가 **고정 간격의 한 행**을 차지하므로, 실제로 오래 걸린 구간이 화면을 잡아먹지 않고 에이전트들이 어떻게 피드백을 주고받았는지가 한눈에 보입니다(왼쪽에 각 이벤트의 실제 시각 표시, **실제 수행 시간은 막대에 마우스를 올리면** 툴팁으로). skill(🪄)·일회성 `run`(⟲)은 호출자가 그동안 블록되므로 호출자 컬럼 안에 다른 색 구간으로 표시됩니다. **스킬 안에서 다시 스킬/에이전트를 실행하면**(중첩) 자식이 부모 바로 **오른쪽 칸**에 그려지고 가는 선으로 부모와 이어집니다 — 깊이는 툴팁에 표시됩니다. 차례로 실행되는 형제는 같은 칸을 재사용하므로 스킬을 여러 개 연달아 돌려도 폭이 늘지 않고, **한 턴에 여러 에이전트를 동시에 띄운 경우**(병렬 배치)만 옆 칸으로 갈라지며 **⋔ 포크**와 동시 개수(`⋔3`)로 표시됩니다(동시 시작이라 같은 행에서 출발하고, 끝나는 시각은 각자). 오른쪽 타임라인에서도 **자식 카드가 부모 카드 안에 들어갑니다** — 부모는 기본 접힘이라, 안에서 뭔가 돌고 있으면 부모 헤더에 `▸ … 실행 중` 배지가 뜨고, 스윔레인에서 중첩된 막대를 클릭하면 부모 카드들이 자동으로 펼쳐진 뒤 해당 카드로 이동합니다. 각 컬럼 맨 아래에는 **현재 상태**가 표시됩니다 — 작업 중인 에이전트는 도는 스피너, 쉬는 에이전트는 녹색 체크(✓), **main 도 응답 생성 중이면 스피너**가 돕니다(타임라인이 드로어 안이라 기본 화면의 유일한 '응답 중' 단서). 이벤트가 도착할 때만 갱신되므로 주기적 폴링은 없습니다. 색은 현재 테마에 반응하고, 에이전트 없는 단순 대화도 사용자·main 두 레인으로 흐름이 그려집니다.

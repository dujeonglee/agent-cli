...
**⚡ Prompt Inspector:** 헤더의 ⚡ 버튼으로 우측 드로어를 열면 **현재 턴에 실제로 전송된 시스템 프롬프트**를 섹션별로 확인할 수 있습니다. 상단의 토큰 예산 스택바(섹션별 색·비율), 섹션 아코디언(이름·토큰 뱃지·본문), 검색 필터를 제공합니다. 열 때마다 최신 LLM 호출의 스냅샷을 가져오며(`GET /api/debug/prompt`, 토큰 인증), 훅이 주입한 동적 섹션도 `Hook: <이름>`으로 표시됩니다. 컨텍스트 압축(compaction)이 일어나면 그 요약과 파일 목록도 `⊙ Compaction summary / Files touched (user-injected)` 섹션으로 함께 보여, 모델이 실제로 받는 압축 컨텍스트를 검사할 수 있습니다. **정적 시스템 프롬프트 아래에는 `── 동적 컨텍스트 (대화 · 관찰) ──` 구분선과 함께 현재 컨텍스트 윈도우에 든 대화·관찰(`ctx.get_messages()` 의 system 제외분)이 메시지별 섹션으로 표시**되어, 시스템 프롬프트뿐 아니라 LLM 이 실제로 받는 전체 입력을 검사할 수 있습니다(메인 스코프 한정). **첫 메시지 전에도 채워집니다** — 시작 시 시스템 프롬프트를 미리 캡처하고(`Hook:` 동적 섹션은 첫 LLM 호출 후 채워짐), `--resume` 면 복원된 대화도 드로어를 열자마자 보입니다. **에이전트별 스코프**: delegate 서브에이전트가 돌면 드로어 상단에 `[Main] [explorer·1] [coder·2]` 칩 row가 나타나, 칩을 클릭하면 해당 서브에이전트가 실제로 받은 시스템 프롬프트로 전환되고 `Main`을 누르면 메인으로 돌아옵니다. 끝난 서브에이전트의 프롬프트도 사후 검사를 위해 남아 있으며, 칩의 ✕로 개별 제거할 수 있습니다(Main은 제거 불가). **📝 Directives 에디터**: 드로어 상단에서 프로젝트 `.agent-cli/DIRECTIVE.md` 를 **바로 편집·저장**할 수 있습니다(파일이 없어도 항상 표시 — 저장 시 새로 생성). **취소** 버튼은 미저장 편집을 버리고 파일의 현재 내용을 편집창에 다시 불러옵니다. **🪄 자동 생성**: 두 개의 드롭다운(**성격** · **업무**)에서 원하는 축을 고른 뒤 **🪄 생성**을 누르면 세션 모델이 그 조합으로 DIRECTIVE 를 만들어 줍니다. **업무**(역할/작업 축)는 `업무: 없음`(기본) · `✨ 현재 내용 정리`(편집창 초안 보강) · 스타터 프리셋(드라이버 TDD 개발 · KUnit 테스트 · 이슈 분석 · 코드 리뷰) 중 선택. **성격**(🎭 캐릭터/말투 축)은 업무와 **직교하는 축**으로 `## 페르소나` 섹션으로 주입됩니다 — `성격: 없음`(기본) · 고정 캐릭터(고양이 집사 · 욕쟁이 할머니 · 조선의 임금 · 머슴 · 하드보일드 탐정 · 영국 집사 · 해적 선장 · 냉정한 로봇 · 판타지 현자 · 전문 프로그래머 · 교수) · `✏ 직접 입력…`(원하는 캐릭터를 텍스트로 적어 확장). 성격은 **편집창의 업무 지시를 보존한 채** 맨 앞에 얹히고, 다시 생성하면 **덮어쓰지 스택되지 않습니다**(목소리만 바뀌고 작업 정확성은 유지). 생성된 페르소나에는 **목소리를 어디에 쓸지**가 명시됩니다 — 사용자 대면 답변·최종 결과·질문(ask)은 캐릭터 목소리로, **도구 호출·파일 경로·명령어·코드·사실 정확성은 그대로 정확하게**. (예전엔 "완전히 장식적" 문구 때문에 도구 위주 코딩 세션에서 페르소나가 거의 발현되지 않았는데, "적용 위치 지정"으로 바꿔 해결.) 두 드롭다운을 **모두 `없음`**으로 두고 🪄를 누르면 편집창을 **비웁니다**(LLM 호출 없이 삭제 경로 — 검토 후 저장하면 DIRECTIVE 제거). 생성 결과는 **미저장** 상태로 들어오므로 검토 후 저장하거나 취소로 되돌릴 수 있습니다(`POST /api/directives/compose` · `GET /api/directives/presets` · `GET /api/directives/personas`, 세션 모델 재사용 — LLM 미배선 시 드롭다운 행 숨김). 저장하면 파일에 기록되고 **다음 LLM 호출부터 시스템 프롬프트에 반영**됩니다(idle이면 다음 쿼리부터; KV 캐시 prefix는 리셋됨). 프롬프트 뷰의 `Directives` 섹션은 *저장 즉시가 아니라 실제 적용되는 순간*(그 rebuild 시점)에 갱신되어 — 항상 **LLM 이 실제로 받는** 내용을 보여줍니다. 여러 명이 보고 있으면 한 명의 저장·적용이 다른 인스펙터에도 브로드캐스트되어 동기화됩니다(`GET`/`POST /api/directives`, 토큰 인증, **프로젝트 파일 전용** — 사용자 전역 `~/.agent-cli/DIRECTIVE.md`는 편집 대상 아님). **📥 세션 학습**: **업무 자동생성은 근거 없는 일반론 위험** 때문에 제거하고, 대신 **📥 학습** 버튼으로 *실제 세션 대화에서* 재사용 가능한 교훈만 뽑아 관리 섹션 `## 학습된 지침`에 누적합니다. 클릭하면 전용 요약 호출(유일 임무=이식 가능한 교훈 추출, 코딩·로그 분석·리서치 등 세션 종류 무관)이 현재 대화를 읽고 세션-특정 사실(예: 특정 에러 메시지·파일/줄·일회성 값)은 빼고 이식 가능한 운영 지침만 추려 에디터에 **미저장** 반영(검토 후 저장) — 기존 수기 지시는 바이트 그대로 두고, 다시 학습하면 중복 제거·통합(누적하되 비대 금지)합니다. 추출된 교훈은 세션 memory 스토어에도 **시스템이 결정적으로** 기록됩니다(모델 자율 아님 — 27B는 memory 도구를 자율 사용하지 않음이 실측됨). **💾 프리셋 라이브러리**: 현재 편집 내용을 **프리셋 ▼** 옆 💾로 홈(`~/.agent-cli/directive-presets/`)에 이름 저장하고, 드롭다운에서 골라 불러오거나 🗑로 삭제합니다(저장 시 이름 입력창은 현재 프리셋 이름을 기본값으로 채워 주고, 같은 이름이 이미 있으면 덮어쓸지 확인합니다) — **모든 방(agent-cli 인스턴스)에서 공유**되는 "골라 쓰는" 라이브러리로, 항상 적용되는 전역 `~/.agent-cli/DIRECTIVE.md`와는 분리됩니다(불러오기도 미저장 → 검토 후 저장). 프리셋 이름은 한글·공백을 그대로 보존하되 경로 traversal은 차단합니다(`POST /api/directives/learn` · `GET/POST/GET-by-id/DELETE /api/directives/presets/library`).

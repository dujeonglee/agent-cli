...
```
{project}/.agent-cli/
  skills/                               # 프로젝트 로컬 스킬 파일
  hooks.json                            # hook 설정
  sessions/
    {session_id}/                       # 세션별 디렉토리
      session.jsonl                     # 메타데이터 (id, workspace, updated_at, query)
...
**ContextManager (context/manager.py)** — FIFO + history.jsonl
- 최근 N개 메시지만 메모리 캐시 (기본 N=100)
- 전체 대화를 history.jsonl에 append-only로 기록
- JSON → 자연어 변환하여 LLM에 전달 (`thought: ...\naction: ...` 형식)
- 세션 재개 시 history.jsonl에서 마지막 N개 복원
- LLM 기반 압축 없음 — 순수 FIFO
- `--resume <session_id>`: 이전 세션을 이어서 작업

**Session (context/session.py)** — 세션 메타데이터
- `session.jsonl`: 1줄 메타 (session_id, workspace, updated_at, query)
- `updated_at`과 `query`는 매 쿼리마다 최신으로 갱신
- `agent-cli sessions`: 마지막 활동 시간 + 마지막 쿼리 표시

**Context Recovery** — 과거 맥락 복구
- System prompt에 `history.jsonl` 경로를 안내 (Context Recovery Guide)
- FIFO에서 밀려난 과거 메시지가 필요하면 LLM이 `read_file(history.jsonl)` 실행
- history.jsonl 내 artifact 경로로 delegate/skill 상세 결과 접근 가능

#### 컨텍스트 윈도우 레이아웃

`ctx.get_messages()`가 반환하는 메시지 (history.jsonl의 마지막 N개를 자연어 변환):

```
[0] user:      "hooks.py 분석해줘"
[1] assistant: "thought: hooks.py를 읽어 구조를 파악해야 한다\naction: read_file(hooks.py)"
[2] user:      "Observation: STATUS: success\nRESULT:\n1#PS:..."
[3] assistant: "thought: 분석이 완료되었다\n\nhooks.py는 3개의 hook 타입을 지원..."
```

- Scratchpad/Summary inject 없음 — messages만 (FIFO N=100)
- 도구 결과 전문 포함 (잘림 없음)
- FIFO에서 밀려나면 history.jsonl에서 복구

#### 세션 관리 명령어

```bash
# 세션 목록 (마지막 활동 시간 + 마지막 쿼리)
agent-cli sessions
...
- **50 turn** 도달 시 첫 체크포인트 — 최근 20회 도구 호출 이력을 LLM에게 보여주고 자기 판단 요청
- 이후 **매 20 turn**마다 반복 체크포인트

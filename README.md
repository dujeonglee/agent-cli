...
agent-cli 인스턴스 하나를 단일 세션으로 LAN에 노출. 자동 토큰 생성(또는 `--token` 지정). **다중 뷰어 (모두 동등)**: 모든 탭이 스트림을 보고 **모두 입력·큐 가능**(controller/observer 구분 없음). 각 탭은 `identity` 이벤트로 conn_id를 받아 접속자 로스터의 "(you)" 표시·큐 메시지 소유에 사용. **메시지 큐**: 에이전트가 실행 중이어도 메시지를 보내면 **큐에 쌓여 실시간 표시**되고, 매 턴 종료 시 하나씩 디큐되어 대화에 주입(steering)됩니다. 자기가 큐한 메시지는 처리 전 ✕로 취소 가능. 모든 사용자 요청은 닉네임 라벨로 task 로그에 누적됩니다. 시작 시 토큰 포함 URL이 출력되고 (선택적으로) 브라우저 자동 오픈.

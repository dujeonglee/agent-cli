...
**Timeline / Team 뷰 (팀 스윔레인)**: 여러 에이전트가 함께 일하면(`/orchestrate` 또는 상주 에이전트 spawn) 메시지 영역 상단에 **Timeline / Team** 토글이 나타납니다. **Team** 뷰는 팀 활동을 **라이브 스윔레인**으로 그립니다(이벤트마다 실시간 갱신) — 에이전트별 가로 레인, 작업 구간 바, 에이전트끼리 주고받은 peer 메시지 커넥터. skill(🪄)·일회성 `run`(⟲)은 **호출자가 그동안 블록**되므로 호출자 레인 안에 다른 색 구간으로 표시됩니다. 시간축은 전체를 폭에 맞추고 길이에 비례해 눈금 간격이 자동 조절됩니다. 막대·화살표에 **마우스를 올리면** 무슨 작업/메시지인지 툴팁이 뜨고, 뷰가 열려 있는 동안 **5초마다 갱신**돼 진행 중 막대가 자랍니다. 색은 현재 테마에 반응하고, 평범한 단일-에이전트 대화에는 토글이 뜨지 않습니다. (`/orchestrate` 같은 skill 은 이제 timeline 에도 🪄 카드로 표시됩니다 — skill/delegate 카드 생성이 단일 경로로 통합.)

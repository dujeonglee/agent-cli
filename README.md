...
병렬화되는 것은 **추론**입니다 — 파일 쓰기 같은 부수효과의 충돌 방지(효과 계층 락)는 아직 들어가지 않았으므로, **같은 파일을 동시에 고치는 작업을 여러 사용자가 함께 시키면 서로 덮어쓸 수 있습니다.** 현재 알려진 한계:

- **부수효과 직렬화 없음** — 위 이유로, 읽기·조사 위주의 협업에 적합합니다.

...
- **업로드**: 드로어 안으로 **파일을 드래그-드롭하거나 `선택`** 하면 업로드됩니다(파일당 한 요청). 업로드 위치는 **트리에서 클릭한 폴더**(클릭하면 하이라이트되고 "⬆ 업로드 대상" 에 표시), 클릭하지 않으면 워크스페이스 루트입니다. 결과(✓ 경로, 덮어쓴 경우 표시) 후 트리가 자동 갱신됩니다.

보안 가드: 워크스페이스(서버 실행 디렉토리) 밖 경로는 서버가 차단하고, 업로드는 파일명에 경로 성분 금지(traversal 차단)·대상 경로 워크스페이스 하위만·파일당 50MB·토큰 인증을 적용합니다. (드래그로 파일을 브라우저 *밖으로* 끌어내는 다운로드는 브라우저 제약상 미지원 — zip 버튼이 전 브라우저·디렉토리 경로입니다.)

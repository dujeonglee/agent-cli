...
**점진적 공개 — 레벨 컨트롤 (v8.12.0)**: 헤더의 **`개요 · 흐름 · 전문`** 세그먼트 탭으로 정보 밀도를 고른다 (presentation-only — 서버·SSE·데이터 무변경, 기존 이벤트를 재배치할 뿐). **개요(기본)** 는 요약 화면 — 상단 ambient 한 줄(스킬 상태 · 에이전트 상태 dot · 컨텍스트 % · 뷰어 · 모델) 아래로 **응답 블록 시퀀스**가 흐른다. 각 블록 = **[직전 응답 이후 누적된 사용자 쿼리] → [응답 hero]** 이고, **최신 응답이 주인공(accent)**, 이전 응답은 dim 처리된다. hero 본문은 **마크다운으로 렌더**(제목 · 목록 · 표 · 코드블록 · 강조), 생성 중에는 **라이브 스트리밍 + 커서**로 자라난다. 완료된 응답마다 **⧉ 복사**(본문 클립보드 복사) · **▤ 전체 대화**(그 응답 카드로 타임라인 열기) 액션이 붙는다. 하단 **다음 턴 대기 큐**는 `⏳[닉네임]` 으로 표시되다가, 큐가 소진되면 **`✓주입됨`**(내가 보낸 것은 *내 요청 반영됨*) / 취소 시 **`✕취소됨`** 영수증을 잠시 보여줘 — 큐에서 그냥 사라져 반영 여부를 알 수 없던 문제를 해소한다. **흐름** 은 팀 스윔레인(아래 설명), **전문** 은 상세 대화(타임라인) 드로어 + 🔍 프롬프트 인스펙션이다. **선택→고정 상세 패널 (Tier 2, v8.13.0)**: 흐름의 막대·화살표나 개요의 응답 헤더를 **클릭**하면 그 요소 하나만 집중해서 보여 주는 **우측 고정 패널**이 뜬다 (hover=툴팁 미리보기 · click=고정). 스코프의 결과·메시지 전문·관찰을 마크다운으로 보여 주고, 패널의 **[▤ 전체 타임라인]** 은 그 카드로 드로어를 열고 **[🔍 프롬프트]** 는 그 스코프의 인스펙터를 연다(Tier-3 승격). Esc 로 닫힌다.

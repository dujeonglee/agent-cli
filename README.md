...
- 페이지 스스로도 30초마다 같은 채널로 자기 origin 의 연결 보유 탭 수를 세어, **5개 이상이면 상단 과밀 경고 배너**(✕ 닫기 가능, 수가 늘면 재경고) — board 의 열기 게이트를 우회하는 진입(URL 직접 입력·브라우저 세션 복원·탭 복제)에서도 "왜 새 탭이 안 열리는지"가 보임

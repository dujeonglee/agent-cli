...
**Timeline / Team 뷰 (팀 스윔레인)**: 여러 에이전트가 함께 일하면(`/orchestrate` 또는 상주 에이전트 spawn) 메시지 영역 상단에 **Timeline / Team** 토글이 나타납니다. **Team** 뷰는 팀 활동을 **세로 시퀀스 다이어그램**으로 그립니다 — **시간이 아래로** 흐르고 에이전트는 **세로 컬럼**(상단에 이름 헤더가 고정), 작업 구간은 세로 막대, 에이전트끼리 주고받은 메시지는 컬럼 사이 가로 화살표로 **요청·회신 왕복**이 모두 보입니다. skill(🪄)·일회성 `run`(⟲)은 **호출자가 그동안 블록**되므로 호출자 컬럼 안에 다른 색 구간으로 표시됩니다. 시간축은 **1시간까지 화면 높이에 맞춰 압축**하고, 그보다 길어지면 압축을 멈추고 아래로 이어붙여 **세로 스크롤**합니다(진행 중이면 바닥의 "지금"에 자동으로 붙어 있고, 위로 스크롤하면 과거를 봅니다). 막대·화살표에 **마우스를 올리면** 무슨 작업/메시지인지 툴팁이 뜨고, 뷰가 열려 있는 동안 **5초마다 갱신**됩니다. 색은 현재 테마에 반응하고, 평범한 단일-에이전트 대화에는 토글이 뜨지 않습니다. (`/orchestrate` 같은 skill 은 timeline 에도 🪄 카드로 표시됩니다.)

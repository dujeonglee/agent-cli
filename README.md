...
| `--turn-scoping` | **턴 스코핑.** `parallel` 일 때 각 턴의 시스템 프롬프트에 **그 턴이 수행 중인 요청**을 못 박고, 공유 트랜스크립트에 보이는 다른 사용자의 동시 메시지는 지시가 아니라 맥락임을 명시합니다. 한 턴이 **남의 질문에 답해 버리는** 의미론적 혼선의 완화이며, 답이 누구에게 귀속되는지(구조적이고 이미 정확함)는 바꾸지 않습니다. 측정 결과(라이브 80 턴 중 56 → 0, 자기 과제 완수 상승; 다른 모델에서는 제거가 아니라 감소 — 효과 크기는 모델에 따라 다르나 방향은 일관되게 양(+)이고 비용이 없음) 이후 병렬 계약의 기본값이 됐고, 절제·비교가 필요하면 `--no-turn-scoping` 으로 끕니다. 직렬 계약에서는 걸리지 않습니다 — 거기서 실행 중 주입된 질문은 오히려 그 턴이 수행할 대상이기 때문입니다. | 켜짐 (parallel 에서만 효과) |

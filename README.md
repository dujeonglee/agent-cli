...
**Timeline / Team 뷰 (팀 스윔레인)**: 여러 에이전트가 함께 일하면(`/orchestrate` 또는 상주 에이전트 spawn) 메시지 영역 상단에 **Timeline / Team** 토글이 나타납니다. **Team** 뷰는 팀 활동을 **라이브 스윔레인**으로 그립니다 — 에이전트별 가로 레인, 작업 구간 바, 에이전트끼리 주고받은 peer 메시지 커넥터, 감싸는 skill 밴드(🪄), 일회성 `run` 서브트랙(⟲). 시간축은 **Fit(전체)** 또는 **Live window(최근 구간)** 로 스케일. 색은 현재 테마에 반응하고, 평범한 단일-에이전트 대화에는 토글이 뜨지 않습니다. (`/orchestrate` 같은 skill 은 이제 timeline 에도 🪄 카드로 표시됩니다 — skill/delegate 카드 생성이 단일 경로로 통합.)

...
시나리오: 사용자 A 가 L ms 걸리는 턴을 시작하고, 0.5s 뒤 사용자 B 가 짧은
질문을 던진다. **B 의 TTFT 가 L 에 얼마나 끌려가는가(회귀 기울기)** 가 핵심
지표다.

A 의 "길이"는 순수 추론 스트리밍(토큰 수 × 간격)으로 만든다. 초기 탐색에서는
셸 sleep 도구를 썼지만, 직렬 계약의 HOL 은 "워커가 L 동안 점유된다"는 사실에서

...
- ~~[중간][중복성] `.agent-cli` 경로쌍 6곳(순서·병합·시점 제각각) / `scoped_paths()` 단일화.~~ ✅ v8.40.0 (선언 캐노니컬=[프로젝트, 사용자]; mcp 의 역순 소비는 reversed 파생으로 보존; 병합 입도는 자원별 의도 보존 — hooks.json 만 통일)
...
- ~~[높음][효율성] 청크마다 개요 전체 innerHTML+마크다운 재실행 / 스트립만 갱신+엔트리 캐시.~~ ✅ v8.42.0 (rAF 병합 — 이벤트 폭주 구간 프레임당 1회; 888이벤트 스냅샷 프로파일에서 ovRender 585ms→20ms)
...
- [중간][효율성] ready 재수신마다 리스너 재부착, ~~카드/청크마다 강제 리플로우~~(✅ v8.42.0 — scrollToBottom rAF 병합, 400ms→0)+무상한 누적, 인스펙터 필터 매 키 재소문자화 / 1회 바인딩·rAF 병합·캐시.

...
판정은 **"이 런이 그 주소에게 질문을 걸었나"**(`tm.asked_targets`, v9.22.2 — 종전 bool `asked_this_run` 은 사람에게 물은 런의 요청자 빚까지 억제해 main 이 결과를 영영 못 받았다; 억제의 근거 "답 런이 같은 빚을 진다" 는 질문의 주소가 빚의 주소일 때만 성립한다)다. *"런이 끝나는
...
회신이 영영 사라진다. `asked_targets` 는 배달 성공 뒤에만 채운다.

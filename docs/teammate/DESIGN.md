...
- 2026-07-11 **@명령 구현 (v4.62.0, 범위 B)**: `@teammates` roster ·
  `@agt-<key>` 상태 · `@agt-<key> <msg>` 직접 request (author="user" —
  D8 비배달). 사용자가 `@teammates` 를 쳤다가 agents 폴백을 본 것이 착수
  계기. CLI 수신 = MinimalRenderer.teammate_message 콘솔 라인 + `to`
  필드(worker 가 수신자 태깅 — main 발신분 이중 표시 방지), run 경로는
  @agt 후 quiescence 대기. 직접 스폰 명령은 보류 유지.

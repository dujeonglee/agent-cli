...
- **구현**: `n3c_scoping_real.py --workload realistic --arms off --reps 20` — 워크로드를 매개변수화(`Task`/`WORKLOADS`)해 판정 기계를 그대로 재사용했고, `--rederive` 로 커밋된 confusable 요약이 바이트 동일하게 재도출됨을 확인했다(리팩터 무해 검증).

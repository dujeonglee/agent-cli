...
3. **Two independent open implementations** — Coagora (TypeScript, event-loop server, worker process) and agent-cli (Python, thread-based CLI) — the second re-built from the contract's *invariants*, not its code, each with a deterministic, key-free reproduction harness and committed raw data (§5).
4. **Empirical validation across both runtimes**: TTFT independence (slope 0.000 vs. 1.000 serial and 1.010 reject-retry, n = 180, replicated at 0.000/1.02/1.008 on the second implementation ✔), integrity ablation (74% → 0% on Coagora; 8.2% → 0% on agent-cli, where the physical hazard differs but the direction and the zero are invariant ✔), characterization of where the benefit collapses as workloads shift from inference-heavy to write-heavy (§6), **concurrent-context compaction** that stays available while turns run (§6.6), **structurally deterministic reply attribution** at scale (100/100 bijection; §6.7), per-user fairness under flooding (24× short-turn wait reduction; §6.8), lifecycle durability (204 turns across 3 suspend/resume cycles, context bounded by compaction; §6.9), and live-LLM confirmation with a measured 1.49× (not N×) parallel token premium (§6.10).
...
We reran the two headline experiments on the second implementation (§5.1), driving the real server over HTTP against the same mock-LLM scenario grammar, with all intervals computed from the internal event stream on one monotonic clock (n = 20 per cell).

| Contract (agent-cli) | L = 2 s | L = 6 s | L = 15 s | slope dTTFT/dL |
|---|---|---|---|---|
| Serial (FIFO) | 2.08 s | 6.16 s | 15.34 s | 1.02 |
| Reject + retry | 2.31 s | 6.34 s | 15.42 s | 1.008 |
| **Parallel** | **0.29 s** | **0.29 s** | **0.29 s** | **0.000** |

The ranking, the slopes, and the flat parallel line replicate (Coagora: 1.000 / 1.010 / 0.000); the parallel contract's constant ≈0.29 s is the second implementation's per-turn setup plus the scripted 200 ms TTFT, and its p95 (0.293 s) sits 2 ms above its p50 — B's latency is a property of B's request on this runtime too. The integrity ablation also replicates in direction and in zero, while the *magnitude* differs in an instructive way: bypassing the lock yields 8.2% corrupted snapshots on agent-cli (two writers' markers coexisting mid-file) versus 74% on Coagora, because the physical hazard differs — Python's `write_text` issues one truncate plus one large `write(2)`, so corruption requires interleaving inside that narrow window, where Coagora's executor streamed writes and interleaved generously. Both lock scopes reduce violations to **0%** on both runtimes. This is the shape of evidence we want from a replication: the *contract-level* claims (ranking, slope, zero) are invariant; the *mechanism-level* constants (absolute TTFT floor, violation intensity) shift with the runtime, as they should.

### 6.6 Q6 — Compacting the shared context while turns run ✔

Coagora has no compaction (its limitation (2)); agent-cli brings token-budget compaction into the shared session, and with it the first version of a question no prior work answers: what happens when the conversation is summarized *while* concurrent turns are appending to it? The design is optimistic: split and record a generation under the lock, summarize outside the lock (the multi-second LLM call), then revalidate the generation and commit atomically, absorbing the tail that concurrent turns appended meanwhile.
...
Honest boundaries of the current system and results. (1) *Token cost — now measured* (§6.10): on an identical three-question workload the parallel contract paid 1.49× the serial contract's input tokens (3 calls vs. 2 — serial batches queued questions into running turns via mid-run injection, an efficiency parallelism deliberately gives up for latency independence). The premium tracks call count, not context re-reads, while per-call input is dominated by the fixed system prompt; it approaches N× only if serial batching were perfect and histories long — and provider-side prompt caching (absent on our deployment) would shrink the fixed term further. (2) *Context capacity — partially resolved*: one shared context fills N times faster. Coagora has no compaction; the second implementation integrates token-budget compaction with concurrent turns and measures it (§6.6) — availability holds and no records are lost, at the cost of occasional wasted summary calls under sustained overflow. Long-horizon capacity limits (what N sustained users do to one window over hours) remain unmeasured. (3) *Logical last-wins*: serialization prevents physical corruption, not semantic conflict — two turns may disagree about what a file should contain, and the later one wins; surfacing semantic conflicts to humans is open. The shared transcript also lets a turn's *answer* address another user's later question — now quantified at a 38% worst-case rate under an adversarial always-answer-latest model (§6.7) — which structural attribution records but does not prevent. (4) *Lock envelope*: effects of processes a tool detaches into the background escape the serial executor. (5) *Snapshot staleness* (§4.3). (6) *Single-instance*: pub/sub, locks, and registries are in-process; the pub/sub seam is interface-ready for a distributed backend but unimplemented. (7) *Isolation depth*: directory-level, not container-level (§5); the open-participation model (any authenticated user may execute code in any post's sandbox) is intended for the community setting and demands the stated threat model. (8) *Steer-with-interrupt* is unsupported on concurrent turns (pure cancellation only). (9) *Attribution — resolved in the second implementation*: Coagora attributes tool bubbles heuristically under concurrency; agent-cli's thread-scoped attribution is structural and verified exact at scale (§6.7), so the heuristic caveat is now scoped to the first implementation only.

---

## 8 Conclusion

Every deployed coding agent is single-player, and the industry's team workaround — delegation by @-mention — structurally forecloses the two things teams need most from a collaborator: the ability to intervene while work happens, and a rule for what happens when two people speak at once. The conventional wisdom held that fixing this inside a shared session forces serialization or forking. We showed it does not. Locating responsiveness and integrity in different layers — inference parallel and never blocked, side effects serialized and never concurrent — yields a session that multiple humans genuinely share: everyone's question answered at its own speed (TTFT slope 0.000), no file ever corrupted by a race (74% → 0%), every answer attributed to its asker, and every participant able to stop their own work without touching anyone else's. And the contract is portable: rebuilt from its invariants in a second runtime with a disjoint hazard set — threads over an event loop, shared memory over message passing — it reproduces every headline number (slope 0.000, lock-on 0%), keeps the shared context compactable while turns run, and makes attribution exact rather than heuristic. The design space of §3 says this coordinate was empty; the systems say it is habitable; the measurements — from two independent implementations — say it is worth inhabiting.
...
The implementation, the deterministic benchmark harness (mock LLM with scripted latencies), all raw measurement files (JSONL), the CDF/figure generation scripts, and the browser E2E suite are available in the repository. Every number in §6.1–6.4 marked ✔ regenerates via `npm run bench:e2` / `bench:e1` without API keys or network access. The second implementation and its harness live in the agent-cli repository under `bench/multiuser/` (`e2_hol.py`, `e1_ablation.py`, `n1_compaction.py`, `n3_attribution.py`, `p4_fairness.py`, `p2_grid.py`, all stdlib-only against a Python mock LLM sharing the fork's scenario directive grammar); every number in §6.5–6.10 regenerates from there (P6 requires the on-prem model endpoint via AGENT_CLI_* environment variables; all others are mock-only) and the raw JSONL is committed under `bench/multiuser/out/`. [NOTE: replace with anonymized artifact links for double-blind review.]

...
> Full paper draft v0.3 — 2026-08-06. Based on `07-new-paper-draft.md`; v0.3 adds §5.1 (two runtimes), §6.5–6.8 (cross-runtime replication, concurrent compaction, structural attribution, fairness), the measured P2 collapse boundary in §6.3, refs [33]–[37], and rewrites Limitations (2)/(9).
> Status notes for authors appear as `[NOTE: …]` and `[TODO: …]` and must be removed before submission.
> Numbers marked ✔ are backed by committed raw data in `Aidit-Code/backend/bench/out/` (§6.1–6.4) and `agent-cli/bench/multiuser/out/` (§6.3 grid, §6.5–6.8).
...
The second implementation also ships the measurement plane the fork only designed: an opt-in event stream (`turns.jsonl`) recording turn lifecycle (enqueue → dispatch → first-token → complete), effect-lock waits, compaction generations, and rejects on one monotonic clock — every number in §6.5–6.8 is computed from this file, and the mock-LLM harness reuses the fork's scenario directive grammar so both implementations run the same experiment scripts.

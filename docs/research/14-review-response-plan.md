...
- **Phase 2 부분 완료**: Track C 구현(`--turn-scoping`)+절제(`n3b_scoping.py`) 완료. **라이브 팔 전부 차단** — 이 환경에 온프레미스 엔드포인트가 없다(env 미설정, 로컬 LLM 포트 없음, `~/.agent-cli/models.json` 에 URL 없음). 막힌 것: C2-live(실모델 순응률), B1/B2(§6.10 승격), F2(shell-heavy 운용점). 재개하려면 `AGENT_CLI_BASE_URL`/`AGENT_CLI_API_KEY`/`AGENT_CLI_MODEL` 필요.

...
5. **드로어 제거** — ✅ **완료(v8.29.0)**. 구 "Agents" 드로어(헤더 🤝 `#agent-btn`·`#tm-drawer` 마크업·드로어 IIFE·`.tm-*` CSS·`agentcli:tm-*` CustomEvent 중계) 전부 삭제 — 기능이 전부 개요 채널로 이관됨. `agent_cleared` 는 드로어 대신 `ovOnAgentCleared`(채널·트레이·unread 정리, resume 재생 중복 방지)로 재라우팅. 라이브: 🤝 버튼·드로어 부재, 채널 바 정상, 콘솔 에러 0. 게이트 e2e 55 passed.
6. **문서화 + 릴리스**.

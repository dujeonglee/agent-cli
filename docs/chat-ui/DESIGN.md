...
**kill→resume 뒤 전부 `⏳ 대기`.** 초판의 `agent_cleared` 는 블록 전체(블록 +
안의 턴 카드)를 DOM 에서 지웠다. 그런데 서버의 `clear_agent_conversation` 은
버퍼에서 **`agent_msg` 만** 지우고 스코프·턴 이벤트는 남기며, resume 은
`conversation.jsonl`(수신·폴백 줄)만 재생한다 — 재생된 수신 줄이 붙을 블록이
없어 대기로 떨어졌고, 새로고침해야 버퍼의 스코프가 블록을 다시 세웠다. 정리
대상을 서버와 같게 좁혔다: 머리의 `run-item`, 몸통의 `card-fallback`, 루트의
`card-queued`. 블록과 `runBySeq` 는 남으므로 재생된 수신 줄이 제자리 머리로
돌아온다.

**`✓ (58.8s)` 가 최종답 첫 줄을 덮음.** 초판이 최종답의 왼쪽 여백을 27px 로
줄였다. 여백은 다른 줄의 아이콘·종류 칸과 같은 92px 열로 두고 ✓ 를 그 안
(`.final` 기준 absolute)에 둔다 — 위에 생각 줄이 있어도 겹치지 않는다. 폰
폭(여백 0)에서는 본문 위 한 줄로. 둘 다 브라우저 테스트(기하·재생 순서)로
고정.

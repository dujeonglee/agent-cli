...
### ⑤ Main/Agent 통일 (서버 이벤트)
- 에이전트 루프가 내부 작업을 `assistant_turn`(+`task_id`)으로 방출
- `agent_msg` 는 왕래(in/out/question)만
- 프론트: 채널별 카드 목록 하나 — main 은 `task_id=""` 채널
- **TC 신규**: 에이전트 채널에도 생각·도구가 보이는지(지금은 안 보임),
  같은 렌더 코드가 양쪽을 그리는지, 왕래는 여전히 `agent_msg` 인지

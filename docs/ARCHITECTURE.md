...
│       ├── team_model.js    (228)  **Team 스윔레인 도출**: 순수 DOM-free — SSE 를 {lanes,agents,messages,oneshots,skillBands,mainSpans} 로 도출. **작업 스팬=scope 이벤트(권위 소스)**: `scope_start` task_id "{key}#{seq}"(begin_agent_work)=그 에이전트 레인의 work span, kind="skill"=호출자 밴드, 그 외(delegate/agent run)=main one-shot. agent_msg 는 **메시지 커넥터만**(out 정본+dedup). `build(events, now)` — 진행 중(열린 scope)이면 도메인·막대를 now 까지 확장(끝난 런은 마지막 이벤트, 우측 여백 없음), now 없으면 결정적(tMax). dual Node/browser export → Node 유닛(test_team_model)으로 뮤테이션 검증.
│       ├── team_view.js     (384)  **Team 스윔레인 렌더**: TeamModel→SVG(레인 칩·역할hue 작업바·peer 메시지 커넥터·**skill/run 은 호출자 레인 안 색구간**=블록 관계). 전 색상 var(--…) 테마 반응. 라벨은 항상-그리기 대신 **커스텀 100ms 툴팁**(hover; 우측 넘치면 좌측 flip). 시간축 **adaptive tick**(런 길이 비례). app.js 가 TeamView.ingest(dedup→재접속 replay 멱등, ready reset 없음) 로 급전, Timeline/Team 토글 상호배타. **이벤트 rAF + 5s 라이브 틱**(뷰 열렸을 때만; now 확장으로 진행 막대 성장). 브라우저 e2e=test_team_swimlane.

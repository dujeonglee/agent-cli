...
│   ├── shell.py             (175)  셸 명령 실행 + 위험 명령 (rm/rmdir/mv) y/n/a 확인 (decision + 선택적 코멘트, env `AGENT_CLI_DANGEROUS_SHELL_CONFIRM=0`로 비활성) → ToolResult. **프롬프트 가능 여부는 `get_renderer().can_confirm()` 로 판정** (구 `_is_tty()` 대체) — CLI는 TTY, web은 연결된 클라이언트(SSE+/api/input, TTY 불필요). 못 물어보면 hang 대신 명확한 refuse 에러. **`_confirm_lock`(모듈 threading.Lock)으로 확인 직렬화** — parallel delegate가 task별 워커 스레드로 돌기에, 락이 "한 번에 하나의 outstanding 프롬프트"를 보장해 응답이 물어본 워커로만 라우팅 (CLI `input()` 반환 / web `_input_queue` 단일 대기자). 락 안에서 `_session_allowlist` 재확인. 출력은 잘리지 않고 그대로 LLM observation으로 전달 (이전 shell_artifact 가드는 2026-05-19 제거 — head/tail 미리보기가 중간 디버깅 정보를 silent하게 누락시키는 사례 발견, 컨텍스트 budget은 compaction/FIFO가 처리)
...
2. 런타임 API 감지 (OpenAI 호환 `_detect_openai_compat_capabilities` — `/v1/models` 메타(`max_model_len`) + context overflow probe fallback + thinking 감지). 감지 시 `max_output = context // 4`; context < 16K(`MIN_CONTEXT_WINDOW`)면 `UnsupportedModelError`로 reject

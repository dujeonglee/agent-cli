...
├── turn_metrics.py          (106, v7.30.0)  **M2 동시성 계측** — `{session_dir}/turns.jsonl` 에 `event: turn|lock|compact|reject` 행을 추가 기록하는 프로세스 전역 emitter (`--turn-metrics`, 기본 off). P1(TTFT)·P2/P3(락 대기)·P4(공정성)·N1(동시 압축)·N3(귀속) 실험의 단일 데이터 소스. 파일은 복구 관측 `TurnRecorder` 와 공유(행 구별=`event` 키; TurnRecord 행은 키 없음) — 한 파일 = 한 타임라인이라 분석이 조인 없이 인과를 본다. **전역+기본 off 인 이유**: 발화 지점이 서로 다른 수명에 흩어짐 — 서버(enqueue/reject)·TurnRegistry(dispatch/complete/interrupt)·LLMCaller(first_token, depth==0 1회 래치 + **llm_call 콜별 usage(v7.30.0 P6)** — input/output/cache_read 토큰, turn_id·depth 귀속; usage 는 어디에도 영속되지 않으므로 N-병렬 토큰 비용 실측의 유일한 소스)·effect_lock(enqueue/acquire/release + wait_ms/held_ms; 스레드 이름 `agent-turn-t{n}` 이 곧 턴 귀속)·ContextManager(compact begin/commit/stale + 세대·`active_turns` 프로바이더, query_added 로 msg_id↔스레드 사슬) — 인스턴스 배선은 그 자체가 직렬 경로를 건드린다(effect_lock 전역 스코프와 같은 정당화: 한 프로세스=한 세션). off 면 emit=None 체크 no-op(직렬 경로 보존). 시계: 구간은 `mono_ms`(단조, 스레드 경계 유효), 대조는 ISO `timestamp`. 포크가 설계만 한 BENCH_TRACE 의 첫 구현. 프라이버시=구조 메타데이터만. 계약 테스트=`test_turn_metrics.py`(16) + e2e 사슬(`test_parallel_turns_e2e.py`) + 거부 게이트(`test_web_server.py::TestRejectContract`)

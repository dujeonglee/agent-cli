...
- **Delegate 전파**: `tool_delegate`가 `hooks_config`를 subagent `run_loop`에 그대로 전달. 에이전트는 자체 frontmatter hooks 필드를 가지지 않음 (YAGNI — 필요 시 skill.hooks 패턴을 재사용 가능). 즉 에이전트 전용 overlay는 없지만, 부모 계층의 전역/프로젝트/스킬 훅은 모두 상속.

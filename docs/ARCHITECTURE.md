...
├── directive_presets.py     (81)   DIRECTIVE 프리셋 라이브러리 — `~/.agent-cli/directive-presets/<name>.md` 홈 스토어(**모든 방 공유**, 항상-적용 전역 `~/.agent-cli/DIRECTIVE.md` 와 분리된 "골라 쓰는" 라이브러리). **stateless**(매 op 파일시스템 hit). `save(name,content)→id`(같은 이름 덮어씀)·`load(id)→body|None`·`list_presets()→[{id,label,source:user}]`(id 정렬)·`delete(id)→bool`. `_safe_name` 이 이름=파일명 stem 검증: 한글·공백·유니코드는 그대로 보존하되 `/`·`\`·`.`·`..`·leading-dot 거부(traversal 차단, id 는 URL 세그먼트+파일명 겸용이라 load/delete 에서도 재검증). `_presets_dir()` 는 테스트가 tmp 로 monkeypatch(홈 미오염). web `POST/GET/DELETE /api/directives/presets/library` 가 위임. (Directive 학습 DESIGN §5.2)

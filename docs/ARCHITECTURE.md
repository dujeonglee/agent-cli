...
모든 wire-format plugin이 반환하는 boundary 데이터타입. ReActFormat의 `parse_react`(같은 plugin 안)는 이 타입을 *직접* 반환하며, envelope 등 다른 plugin도 같은 타입을 사용해 loop이 plugin과 무관하게 동작.

...
│   ├── teammate_tool.py     (147)  teammate 도구 스키마 (P1) — mode enum(spawn/request/wait/status/kill), C7 `validate`(mode 조건부 필수 `_MODE_REQUIRED`), `render_oversized`(wait 의 큰 회신 → worker 가 이미 영속한 replies/reply-<seq>.md 가리키는 on-disk nudge + 더 좁은 재요청 유도). 실행은 subagent/teammate.tool_teammate — delegate 처럼 루프 인터셉트(tool_bridge `_invoke_teammate` 가 registry+runtime 주입). **레지스트리 없는 루프(서브에이전트·headless)에선 AgentLoop init 이 도구 자체를 strip** — P1 "teammate 안 teammate 금지"의 단일 가드이자 기존 테스트 무영향의 이유

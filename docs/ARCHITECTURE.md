...
│   ├── prompt.py            (138)  SystemPromptSvc + build_inspector_sections. **턴 스코핑 (v7.30, opt-in)**: `set_turn_scope(turn_id, author, query)` 가 "이 턴이 수행 중인 요청"을 `Turn Scope` 섹션으로 못 박는다 — 공유 트랜스크립트에서 모델이 **남의 동시 질문에 답하는** 의미론적 혼선의 완화이며, 구조적 귀속(`reply_to`, thread-local)은 이미 정확하므로 대상이 아니다. **두 겹 게이트**: `LoopConfig.turn_scoping`(CLI `--turn-scoping`, 기본 off — 효과 측정 전까지 출하 동작 무변) **AND** `origin_turn` 비어있지 않음(=병렬 계약). 직렬에 걸지 않는 이유는 거기서 실행 중 주입된 질문이 오히려 그 턴의 수행 대상이기 때문이다. 섹션은 `rebuild()` 가 **매번 다시 붙인다** — Inspector 의 DIRECTIVE 재빌드가 섹션을 처음부터 만들므로 재부착이 없으면 조용히 사라진다. `apply_hook_sections` 는 `Hook: ` 접두만 갈아끼우므로 스코프는 정적 섹션으로 보존. 인용은 `_SCOPE_QUOTE_LIMIT`(400자)로 자르고 공백을 평탄화한다(요청 원문은 이미 user 메시지에 있으므로 여기서는 **지목**만 하면 되고, 줄바꿈이 남으면 `> ` 인용 블록이 첫 줄만 인용하고 나머지가 평범한 지시문으로 샌다). 가드=`test_turn_scoping.py`(11)

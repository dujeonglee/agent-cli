...
├── main.py                  (852)  CLI 명령어: run, chat, setup, sessions + 공유 헬퍼
...
# VIRTUAL_TOOLS = frozenset({"complete", "ask", "run_skill", "read_artifact"}) — loop에서 인터셉트
...
```
build_system_prompt(capabilities, active_tools, include_delegate, skill_stack)
    │

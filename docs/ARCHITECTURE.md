...
│   └── prefix_md.py         (490)  PrefixMdFormat — 마크다운 H2 헤딩 wire format (`## Thought / ## Action / ## Input`). small-LLM이 XML envelope보다 자연스럽게 emit하도록 설계. parser: strict `^## X$` line-anchored 매칭, last-wins on `## Action`+`## Input` (sub-header drift 방어), action body는 단일 토큰 (`^[\w.-]+$`) 검증. **thought·action 둘 다 optional (`thought_required=False`, `action_required=False`)**: `## Thought` 헤딩 있으면 그 섹션, 없으면 `## Action` 앞 prose 를 thought 으로 간주; **action body 가 무효/빈, 또는 `## Action` 헤딩 자체가 없어도 trailing Input JSON(또는 bare dict)을 `_extract_json_dict` 로 추출해 action_input 을 보존** → loop 이 infer_action 으로 도구 복원 (react parity). ★ prefix_md 도입(3a47cbf)부터 빈 `## Action` 에서 Input 을 버려 dropped-action 복구가 불가였던 regression 수정 — 실전 1780718751 세션 18/188턴 NO_ACTION 이 전부 이 케이스. parse_stage: 0=복구불가(valid action body·trailing input 모두 없음), 1=valid action+Input, 2=valid action+Input 깨짐/없음, 3=action 없음/무효 but action_input 추출됨(infer 가능). provider_call_kwargs override (`skip_json_format=True`) — basic JSON 모드가 `{` 강제하는데 markdown은 `## `로 시작이라 충돌. 나머지 lifecycle은 ABC default 사용.

...
│   ├── virtual.py           (121)  가상 도구 Tool 서브클래스 (complete/ask/run_skill/ready_for_review) — loop이 인터셉트, **표준 키 유지** (prefix/추론 대상 아님). **`ask` 는 flat 단수 `{question}`**(질문 하나=op 하나; 여러 질문은 ask op 여러 개=read_file 식 배치) — 비-terminal 이라 응답이 observation 으로 accumulate. (legacy `questions[]` 도 `_extract_questions` 가 관용.)
...
│   ├── context.py           (418)  read_context 도구 — **history 를 SQL 로 질의**. 필터 파라미터 더미 대신 **단일 `query`(SQL SELECT)** 프리미티브: history.jsonl 을 인메모리 sqlite `history` 테이블로 온-디맨드 적재(컬럼 `session/loc/seq/kind/turn/ts/tools/files/author/text/content`)하고 LLM 이 SELECT 작성. `content` 는 원본(spill 레코드는 `{spill,output:[...]}` JSON 텍스트) → `json_extract(content,'$.output[N]')` 로 spill 청크 하나만 bounded 회수. 컬럼은 **읽기 시점에 `manager._classify_record`(kind/tools/text) + `extract_file_paths`(files) 로 유도** → 어떤 레코드 shape 든 동작, prefix-관습 재추측 없음. `turn`/`ts`/`author` 는 레코드에서. SQLite 는 `code_index._sqlite` shim 경유 **lazy 로드**(stdlib→pysqlite3 폴백; **코어 도구라 sqlite 부재여도 모듈 import 안 깨짐** — 쿼리 시 친절 에러). **읽기전용**: 비-SELECT prefix 거부 + 인메모리 DB 콜마다 재빌드·폐기(쓰기 무해)가 1차 가드, sqlite authorizer(SELECT/READ 외 거부)는 belt-and-suspenders(상수 없는 pysqlite3 빌드면 skip). 결과 50행 cap. `query` 생략 시 스키마+예시+세션목록(discovery). `sessions`(current/all/<id>) = 테이블에 적재할 데이터 범위. (구 mode=list/search/fetch + kind/tool/author/turn/scope 파라미터 → 전부 SQL 로 흡수, 736→362 LOC.) `files` 컬럼은 다음 BM25(FTS5)와 같은 sqlite 기반.
...
- **과대 도구 출력 spill (`_maybe_spill`, `add` 단일 지점)**: 관찰 출력이 `_SPILL_THRESHOLD_TOKENS`(50K) 초과면 `content = {"spill": True, "output": [guide, chunk1, ...]}`로 치환해 cache+history 저장. **청크·guide head 모두 문자(char) 기준** — `_chunk_text_by_tokens` 는 ~50K토큰=±200K자 창에서 줄경계 선호하되 **긴 줄은 하드분할**, guide head 는 `_SPILL_HEAD_TOKENS`(500)로 하드캡(무손실). (라인 기반이면 1MB 단일-줄 hashline 에서 guide·청크가 윈도우 초과 → `tokens_after=0` 재발했음. 문자 기준이라 단일 메시지가 절대 윈도우 초과 안 함.) **`_spill_view` 가 컨텍스트 가시면을 guide(output[0])로 통일** — `_estimate_message_tokens`·`get_messages`/`_convert_observation`·`_to_summary_text`·`_classify_record` 전부 guide 만 봄. 따라서 **단일 메시지가 윈도우를 못 넘김** → 압축 evict 슬라이스·요약 호출이 항상 윈도우 안(이전엔 800K 단일 관찰이 evict 전체를 먹어 `tokens_after=0`로 캐시 비우고, 요약 호출도 윈도우 초과로 서버 관대함에 의존했음). 전체 청크는 history.jsonl 보존, 회수는 read_context `content`(JSON) 컬럼 + `json_extract('$.output[N]')`(turn 주소). `add()`가 저장 메시지를 반환해 `_append_observation` 이 **저장값을 렌더**(라이브 카드=ctx=resume 일관; 거대 출력은 guide 카드). 관찰 렌더는 `_append_observation` 단일 지점 통합(`render` 플래그; recovery 는 `render_recovery` 가 이미 렌더해 False). multi-op 관찰 결과는 flush 합본 1카드(action 카드는 op별 라이브).
...
Tool output은 **잘림(truncation) 없이 전체를 그대로** LLM에 전달합니다 — 단, 한 관찰이 `_SPILL_THRESHOLD_TOKENS`(50K)를 넘는 병적 대용량(예: 레포 전체 `find`, 전 심볼 `code_index` 덤프)이면 **spill**됩니다: 전체는 history.jsonl 에 무손실 청크로 보존하고 컨텍스트엔 guide(head + read_context 회수법)만 들어가, 한 메시지가 윈도우를 넘겨 압축을 깨뜨리는 걸 방지(§5.4 spill 참조). 이전에는 context window의 3% 비율로 잘랐으나(`tools/truncation.py`, 삭제됨), LLM이 불완전한 정보로 판단하는 성능 열화가 확인되어 제거. context가 budget의 90%를 넘으면 `context/manager.py`의 compaction이 oldest 절반을 LLM 요약으로 흡수하고, 실패/미충족이면 belt-and-braces로 FIFO drop이 메시지 단위로 떨궈냄.

...
│       ├── boot.js          (130)  **SSE 입장 게이트 (v7.5.0)** — index.html 이 app.js 대신 이걸 로드. tab-presence 채널 프로토콜 단일 소유: responder(ping→`{pong,nonce,path,held}` — 파킹 탭은 `held:false` 라 한도 카운트에서 제외, held 필드 없는 pong=구버전 탭·board 대시보드는 항상 보유로 집계) + `countHeld()`(150ms 수집) + `window.AgentCliPresence{countHeld,setHeld}` 공유 표면. 흐름: 카운트 → `MAX_HELD_TABS`(5) 미만이면 app.js 동적 로드(=SSE 연결, 슬롯 선점 held=true), 이상이면 `#conn-parked` 전면 파킹 + 5s+지터 자동 재시도 — "보유 ≤5" 불변식 자가 강제로 1슬롯 상시 확보(파킹 탭 로드도 안 굶음). BroadcastChannel 미지원이면 게이트 없이 통과(종전 동작).

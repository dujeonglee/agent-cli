...
│   ├── __init__.py
│   ├── directives.py        (~310, v4.45.0 C3) DIRECTIVE.md 섹션/zone 조작(_strip/_replace_managed_section·3축 _zone_get/_zone_set·_AXIS_TEMPLATES) — 순수 도메인 로직, FastAPI import 0(테스트 가드), server → directives 단방향. (세션 학습 증류(📥 learn)는 v5.3.0 에서 auto-review 와 함께 제거 — learned 축은 수동 편집/프리셋 전용으로 존속.) **U-C 스코프 상호작용**: `_append_before_scope_markers` — learned(`## 학습된 지침`) append 를 첫 `## @main`/`## @agents` 마커 **앞**(common 끝)에 삽입해 세션 교훈이 마지막 스코프 블록에 빨려 들어가는 것을 방지(무마커 파일은 종전 EOF append); 페르소나는 prepend 라 자연히 common. task 프리셋 로드는 '나머지 전부' 의미론 그대로 스코프 블록 포함 교체(task zone 소유)
...
build_system_prompt(capabilities, active_tools, skill_stack, session_id, agent_role, agent_registry, ...)
    ...
    └─ Execution Context (skill_stack/agent_stack이 있을 때만 — Recency 마지막)
        ├─ "Call stack: main → agent:code-reviewer → skill:plan"

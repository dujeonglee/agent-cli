...
├── prompts/                        프롬프트 템플릿
│   ├── __init__.py          (1)
│   ├── session_state.py     (104)  세션 상태 블록 (프롬프트 꼬리) — 휘발 상태(컨텍스트 사용량·턴·Live Agents·Session Memory·압축 경고)의 순수 렌더. 조립은 `LLMCaller._build_session_state`, 전달은 `ContextManager.get_messages` 꼬리 append (미영속). 위치 근거는 §KV 프리픽스.

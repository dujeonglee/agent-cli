...
| 도구 | 설명 | 필수 입력 | 출력 |
|------|------|----------|------|
| `read_file` | 파일 읽기 (hashline 포맷). 모드: `stat` (메타데이터 + 앞 20줄), `search` (정규식 grep), `line_start/line_end` (부분 범위 — 범위가 파일 전체를 덮으면 whole-file read), 또는 mode 없이 full read (크기 제한 없음). | `path` | `LINE#HASH:content` 형식 |

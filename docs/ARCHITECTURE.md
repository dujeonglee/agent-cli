...
│       ├── team_view.js     (384)  **Team 스윔레인 렌더**: TeamModel→SVG(레인 칩·역할hue 작업바·peer 메시지 커넥터·**skill/run 은 호출자 레인 안 색구간**=블록 관계). 전 색상 var(--…) 테마 반응. 라벨은 항상-그리기 대신 **커스텀 100ms 툴팁**(hover; 우측 넘치면 좌측 flip). 시간축 **adaptive tick**(런 길이 비례). app.js 가 TeamView.ingest(dedup→재접속 replay 멱등, ready reset 없음) 로 급전, Timeline/Team 토글 상호배타. **이벤트 rAF + 5s 라이브 틱**(뷰 열렸을 때만; now 확장으로 진행 막대 성장). **resume 복구**: scope 이벤트는 `scopes.jsonl`→`replay_scopes` 로 재방출(`replay:true`), 프런트는 막대만 그리고 timeline 접이 카드는 스킵. 브라우저 e2e=test_team_swimlane(TestResumeScopeReplay 포함).

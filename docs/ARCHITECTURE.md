...
│   ├── context.py           (736)  read_context 도구 (list / search / fetch). **search = history.jsonl 레코드 구조화 JSON 쿼리**: 필터 `keyword`(text 부분일치)·`kind`(query/action/observation/final/raw/system)·`tool`(툴명 멤버십)·`author`(닉네임)·`turn`(int 또는 {from,to}) — 자유 조합, ≥1 필수. + `sessions`(current/all/<id>). 각 레코드를 **읽기 시점에 `manager._classify_record` 로 분류**(쓰기 enrich 와 동일 함수 — 어떤 shape 든 동작, prefix-관습 재추측 없음), 영속된 `turn`/`author` 는 레코드에서 읽음. 결과: loc+kind/turn/tools/author+text 프리뷰, 50 cap. (구 scope[reasoning/tool/observation/query]+keyword-필수 폐기; 구 tool-scope 가 단수 `action` 만 읽어 ops-모양 액션을 놓치던 버그도 `text` 기반으로 해소.) fetch: loc+range

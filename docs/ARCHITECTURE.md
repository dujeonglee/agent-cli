...
├── memory.py                (238)  세션 메모리 스토어 — LLM 이 큐레이션한 **compaction-immune** salient 노트. `<session_dir>/memory.jsonl`(read_context history 와 같은 세션 디렉토리, resume 복원). **stateless**(매 op JSONL fresh read → resume 정합, 캐시 sync 없음). `add/get/update/delete/list_entries` + `render_index`(상시 `## Session Memory` 섹션 — 요약만, detail 제외 → 토큰 저렴 + recall) + `format_entry`(get 전체). type enum=`failure`⚠/`discovery`💡/`decision`🔀/`note`📝(검증·아이콘 단일 출처). id=max+1 monotonic(삭제해도 재사용 X). 인덱스 `_INDEX_CAP=30` 소프트 캡(초과분 "생략" 꼬리). `_io_lock` 로 read-modify-write 직렬화. `mark_memory_dirty`/`consume_memory_reload`(mutating op → 다음 턴 loop rebuild). **왜 별도 스토어**: 시스템 프롬프트는 `ctx.get_messages()` 밖이라 압축 대상 아님 — 롤링 컨텍스트에 넣으면 정작 필요할 때 요약/드롭됨 + 자기-출력 재공급 mimicry 위험(docs/session-memory/DESIGN.md).
├── directive_presets.py     (127)  **축별** DIRECTIVE 프리셋 라이브러리 — `~/.agent-cli/directive-presets/<axis>/<name>.md` 홈 스토어(axis∈`persona|task|learned`=`AXES`, **모든 방 공유**, 항상-적용 전역 `~/.agent-cli/DIRECTIVE.md` 와 분리된 "골라 쓰는" 라이브러리). **stateless**(매 op 파일시스템 hit). 모든 함수가 `axis` 를 선인자로: `save(axis,name,content)→id`(같은 이름 덮어씀)·`load(axis,id)→body|None`·`list_presets(axis)→[{id,label,source}]`(id 정렬)·`delete(axis,id)→bool`. `_safe_axis` 가 axis 를 고정 enum 으로 검증(URL 유래라도 서브디렉토리 탈출 불가), `_safe_name` 이 이름=파일명 stem 검증(한글·공백·유니코드 보존, `/`·`\`·`.`·`..`·leading-dot 거부 → traversal 차단, load/delete 에서도 재검증). **Built-in 프리셋**: 패키지 동봉 `agent_cli/directive_presets_builtin/<axis>/*.md`(persona 2·task 3, wheel 포함=pyproject package-data)을 `_BUILTIN_ROOT` 에서 읽어 `list_presets` 가 `source:"builtin"` 로 병합·`load` 가 폴백 — **읽기전용**(save/delete 는 홈 스토어만, 같은 이름 user 프리셋이 built-in 을 shadow). `_presets_root()`·`_BUILTIN_ROOT` 는 테스트가 tmp 로 monkeypatch(홈·shipped 미오염). web `GET/POST/{id}/apply/DELETE /api/directives/presets/library?axis=` 가 위임(POST 저장분=그 축의 zone body 만; 성격/업무/지침 세 축 동일 API). (Directive 학습 DESIGN §5.2)
...
│   ├── shell.py             (219)  셸 명령 실행 (**flat-native, Step 3**: 스키마 `{command, timeout?}` — `shell_` prefix 제거, `wrap_single_op`=identity; shell 이 마지막 flat 전환이라 이로써 **모든 builtin 도구 flat**) + 위험 명령 (rm/rmdir/mv) y/n/a 확인 (decision + 선택적 코멘트, env `AGENT_CLI_DANGEROUS_SHELL_CONFIRM=0`로 비활성) → ToolResult. **위험가드 통과 후 `_confine.guard(extract_shell_paths(cmd), …)` 로 워크스페이스-밖 경로 게이트**(별개 축 — 파괴적이지 않아도 밖이면 물음). **프롬프트 가능 여부는 `get_renderer().can_prompt()` 로 판정** (구 `_is_tty()` 대체) — CLI는 TTY, web은 연결된 클라이언트(SSE+/api/input, TTY 불필요). 못 물어보면 hang 대신 명확한 refuse 에러. **확인 직렬화는 렌더 레이어의 공유 `interactive_lock`(RLock)** 사용 (confirm·ask 공통) — parallel delegate가 task별 워커 스레드로 돌기에 "한 번에 하나의 outstanding 프롬프트"를 보장해 응답이 물어본 워커로만 라우팅. shell은 락을 잡고 `_session_allowlist` 재확인 후 `renderer.confirm`을 호출(같은 스레드 RLock 재진입). `ask`(`_handle_ask`)도 동일 `can_prompt` 게이트 — 못 띄우면 `"(no response)"` 치환. 위험 확인 별칭: `y`(+yes/ok/okay/yep/yeah/sure), `a`(+always/**allow**), `n`(+no/nope) — 긍정 의도가 안전 기본값 deny로 오인되지 않게 확장(특히 프롬프트 라벨이 "always allow"라 `allow`→a). 출력은 잘리지 않고 그대로 LLM observation으로 전달 — 단 마스터 캡(`context_window//10`) 초과 시 `ShellTool.render_oversized` 가 **전체 출력을 `session_dir/shell-output-<hash>.txt` 에 lazy 저장(over-cap 때만)** 하고 그 파일을 가리키는 on-disk nudge(read/search·delegate 섹션 팬아웃)로 치환(headless=tee 폴백). **전체는 디스크에 보존·경로 announce** 라 옛 shell_artifact head/tail 의 silent 누락 문제(2026-05-19 제거 사유)와 무관 — 정보 유실 없이 컨텍스트만 보호. (이전 shell_artifact 가드는 head/tail 미리보기가 중간 디버깅 정보를 silent하게 누락시키는 사례로 제거.)
│   ├── fetch.py             (313)  웹 페이지 fetch → 마크다운 변환 → ToolResult (+ FetchTool). **flat-native (Step 3 완결, v4.38.0)**: 스키마 = flat `{url, depth?}` (required `url`) — 마지막까지 `fetch_` prefix 를 쓰던 builtin 이라 `claims`/`infer_action` 이 fetch 에만 live 였고 "모든 builtin flat-native" 불변식을 거짓으로 만들던 것을 flatten 으로 완결. `wrap_single_op`=identity; `key_prefix` 기본 유지 → 레거시 `fetch_url` emission(구 세션 재공급 prior)은 `strip_prefix` 가 계속 관용. **`render_oversized` override**: over-cap 내용을 `session_dir/fetch-output-<hash>.txt` 로 lazy 저장→가리키는 on-disk nudge(read range/search·delegate 섹션 팬아웃·더 좁은 URL/얕은 depth); headless/저장실패=제네릭 폴백
│   ├── delegate/            (922 총, v4.44.0 C2 패키지化) in-process 서브에이전트 패키지 — loop/ 와 동형 패턴, `__init__` 재수출로 기존 import 표면 무변경. 테스트 monkeypatch 는 사용 서브모듈 patch(_run_single·_load_agent→exec). `_reset_agent_loader`(테스트 전용 전역 mutator)는 prod 삭제 — 테스트가 `agents._agent_loader` 직접 교체 + conftest autouse 스냅샷/복원(구식부터 있던 순서-의존 누수 클래스 근본 수리).
│   │   ├── agents.py        (67)   에이전트 정의 로딩(_AGENT_SEARCH_PATHS·frontmatter 파싱·빌트인 경로 — 패키지化로 __file__ 3-parent)
│   │   ├── report.py        (249)  실행 결과의 표현: DelegateResult·활동로그 추출(iter_record_ops)·dir명/result.md 영속·출력 포맷팅
│   │   ├── exec.py          (444)  실행 엔진: _run_single(**delegate 고유분만** — 빈 task/provider 가드·사이클/깊이 체크·delegate dir 명명·리포트 조립; 실행 몸통은 subagent/runner 호출, teammate P0 v4.53.0 — 관찰 문구·result.md 바이트 불변)·_run_parallel(worker 스레드·renderer lifecycle)·tool_delegate 진입
│   │   └── tool.py          (114)  DelegateTool(스키마·parallel_safe·render_oversized) — 기존 delegate.py 설명은 아래 유지: in-process 서브에이전트 (fork/none, 병렬 + Live 상태 패널은 render.minimal `FrameClock` reuse, subdir, agent_stack, stop_event). **flat-native (consolidation Step 3)**: `DelegateTool` 스키마 = flat 단일 task `{task, context?, tools?, agent?}` (required `task`) — `delegate_tasks` 배치 배열·prefix 제거, `wrap_single_op`=identity. **`parallel_safe=True`** (유일) → loop `_dispatch_parallel_batch` 가 한 턴의 연속 delegate op 들을 모아 `{tasks:[...]}` 로 조립→`tool_delegate`→`_run_parallel`(병렬). 병렬 엔진(`tool_delegate`/`_run_parallel`/`_run_single`, `{tasks:[...]}` 소비)은 **기능적이라 보존**(read/code_index 의 batch 와 달리 삭제 안 함). `_invoke_delegate` 가 단일 flat op 을 `{tasks:[op]}` 로 정규화(agent 등 전 필드 보존). ContextManager는 순환 회피 위해 dispatch 시점 런타임 import. **`render_oversized` override**: 서브에이전트 답변이 마스터 캡 초과면, `tool_delegate` 가 이미 저장한 `<delegate_dir>/result.md`(=`ToolResult.artifact`)를 가리키는 on-disk nudge — read range/search·delegate 로 result.md 섹션 팬아웃·**re-delegate-narrower**(근본 원인=task 과대). artifact/session_dir 없으면(병렬 경로·headless) 제네릭 폴백
│   ├── teammate_tool.py     (~150)  teammate 도구 스키마 — mode enum(spawn/request/status/resume/kill — **wait 는 U-A v4.63.0 제거**: stop_event 미감시 결함+자동 배달 완전 대체), C7 `validate`(mode 조건부 필수 `_MODE_REQUIRED`), `instructions` 파라미터(instant-agent U4 — 인라인 즉석 역할, `compose_role_prompt` 로 파일 본문→인라인 순 합성, manifest 에 원본·합성본 영속 → resume/부활 동일 system prompt). 실행은 subagent/teammate.tool_teammate — delegate 처럼 루프 인터셉트(tool_bridge `_invoke_teammate` 가 registry+runtime 주입). **레지스트리 없는 루프(서브에이전트·headless)에선 AgentLoop init 이 도구 자체를 strip** — P1 "teammate 안 teammate 금지"의 단일 가드이자 기존 테스트 무영향의 이유. (agent 통합 로드맵: docs/agent-unification/DESIGN.md — 5.0.0 에서 AgentTool 로 개명 예정)
...
│   ├── records.py           (121, v4.47.0 C5) on-disk record shape 의 계약 — iter_record_ops(양쪽 assistant shape 단일 리더)·_classify_record(kind/tools/text 분류)·_op_summary. delegate report·review·tools/read_context 가 공개 소비(예전 manager private 침범의 정당화)
...
├── prompts/                        프롬프트 템플릿
│   ├── __init__.py          (1)
│   └── system_prompt.py     (998)  Attention 최적화 시스템 프롬프트 빌더. **`build_system_prompt_sections()`** = 단일 조립 지점 — (이름, 텍스트) 섹션 리스트 반환(Role/Context Discipline/Task Guidelines/Response Format/Available Tools/[MCP Tools]/[Skills]/[Agents]/Environment/[Context Recovery]/[Directives]/[Session Memory]/[Execution Context]); `build_system_prompt()`는 그 join wrapper(바이트-동일). **[Session Memory]** = `session_dir` 있고 `memory.jsonl` 비어있지 않을 때 `agent_cli.memory.render_index(session_dir)` 로 상시 인덱스(요약만) 주입 — 매 빌드 fresh read 라 `memory add` 후 rebuild 시 반영·resume 복원. loop 이 `consume_memory_reload()`(directives 와 같은 dirty-flag 패턴)로 mutating op 후 rebuild + `notify_memory_applied()`(→ SSE `memory_changed`)로 **열린 Prompt Inspector 의 prompt 뷰 live 재fetch**(메모리는 에디터 없어 prompt-only). 인덱스 스캐폴딩(헤더·hidden 꼬리)은 영어(시스템 프롬프트 일관, 요약 본문은 LLM 작성 언어). 섹션 이름은 조립 시점에 부여 — 합쳐진 문자열의 `##` 재파싱은 본문 헤딩(도구 가이드·format_rules 예시) 때문에 불가하므로 구조를 원천에서 노출(Prompt Inspector 소비) (Primacy/Middle/Recency, Role 상속, Context Recovery Guide). **format-aware 툴 가이드 (멀티-op 2단계 — DESIGN §5)**: 4개 인라인 빌더(read_file/edit_file/code_index/delegate)가 `wire_format.multi_op` 분기 — multi_op 면 per-tool 배치 prose·예시를 생략하고 단일-대상(op 하나=파일/edit/query/task 하나) 예시를 `render_action_input` 경유 flat 렌더 (op 배열이 곧 배치 — 배치 중첩이 27B 90% 깨뜨린 실측 근거). **read_file 빌더는 flat-native(Step 3) 이후 예시가 항상 flat 단일파일** — `multi_op` 분기는 intro 문구(멀티-op: "한 턴에 read_file op 여러 개" / 단수: "한 호출=한 파일")만 가르고, `{reads:[...]}`·"5. Batch" prose 는 제거(read_file 에 batch 셰이프 자체가 없어짐). `_ASK_INLINE_NO_COMPLETE` = `exposes_complete=False` 포맷용 ask 가이드 변형 (`complete` 호출 대신 thought-only finish 로 표현). **react 도 Step 2(2026-06-13)부터 multi_op=True** 라 multi_op 분기를 탐 — 출하 포맷 둘 다(react/md_array) multi-op 이고, else(단수) 경로는 synthetic 포맷만 도달(정리는 roadmap Step 3/4). 스냅샷 가드(tests/snapshots/tools_section_react.txt)는 이제 react 의 **multi-op flat** 렌더를 고정. `build_system_prompt(wire_format=…)` — Response Format 섹션은 `wire_format.format_rules()`, 스킬·에이전트 호출 예시는 `wire_format.render_full_example(thought=None, ...)`, 도구 inline 가이드의 action_input 단편은 표준 키 dict로 작성되어 `wire_format.render_action_input(dict)`이 wire별 직렬화 (react·md_array 둘 다 prefixed dict→flat op `{action, ...plain params}` 로 변환; 비-JSON plugin이 swap), 도구별 `{tool}_` prefix는 `Tool.add_prefix`로 적용 — 가이드는 표준 키로 쓰고 prefix·직렬화는 단일 출처(`_rai_prefixed`)에서. 인라인 예시는 wire 셰이프로 감싸지 않음 — 와이어 셰이프 학습은 Format Rules + skill/agent 예시(각 1번)에서 일어나고, 인라인은 mode 분기 / 의미론 학습. Recency 순서: Environment → Recovery → Directives → Execution Context (passive→active, persistent→immediate; Execution Context만 동적이라 끝에 배치 → 앞 3개 KV cache 안정). Tool inline 가이드는 `_build_tool_inline_guides(active_tools, wire_format)` 가 매 호출마다 빌드 — `read_file` 가이드의 Flow 문장이 `code_index` 활성 여부에 따라 분기 (활성 시 supported 확장자 파일은 `code_index mode='list'`로 우회 — 확장자 목록은 `code_index.languages.get_supported_extensions()` 단일 출처에서 가져와 walker 추가가 자동 전파). code_index 가이드는 per-file (list/fetch) vs index-wide (lookup/kind/file/refs/callers/callees/slice) scope 경계를 명시, on-demand parse fallback 위치도 안내. **code_index 도 flat-native(Step 3)**: `rai` 헬퍼가 항상 flat 단일쿼리 렌더, `{queries:[...]}` batch 예시·"LIST" prose 제거(read_file 와 동형). edit_file 가이드(`_build_edit_file_inline` — op 시맨틱·hashline·constraints 는 wire 공통 텍스트, flat 단일편집 예시만 `render_action_input`으로 wire별 렌더)는 (1) 편집 직전에 CURRENT turn에서 read 하도록 요구(code_index mode='fetch'도 fresh read로 카운트) (2) hash mismatch를 failure가 아닌 guardrail로 reframe해 모델이 panic 없이 re-read/retry 하도록 톤 조정 (3) **파일 CONTENT 작성은 write_file/edit_file — shell heredoc(`cat <<EOF`) 금지** nudge: 코드가 shell+JSON 이중 escape 돼 NO_JSON 빈발(세션 1782027249 실측 지배)이라, write_file 는 본문을 전용 필드에 담아 escape 한 겹. **edit_file flat-native(Step 3)**: 한 op=한 편집(`edits` 배열 제거), `multi_op` 분기는 framing 문구만 가름. **same-file 배치 안내(multi_op 분기)**: "같은 파일 다중편집은 한 턴에 연속 edit_file op 으로 — ref 는 마지막 read 기준 그대로(줄번호 미보정), 함께 bottom-up 적용되어 stale 안 됨, overlap 은 배치 거부, 같은-파일 op 은 인접 유지"로 **허용**을 안내(루프 `_dispatch_edit_batch` 가 실제 처리). 옛 "턴을 나눠 re-read" 문구를 대체. 단수(react 아닌 synthetic) 분기는 한 턴 1-op 이라 여전히 턴 분리 안내.
│
├── input_queue.py           (118)  공용 입력 큐 (P5) — web WebServer 와 CLI run 큐 펌프가 공유하는 골격 (deque+Condition, SHUTDOWN sentinel identity 계약, timeout dequeue, on_change 콜백). 아이템 {id, conn_id, nickname, text} 는 web 큐 표시·cancel 소유권 계약
├── subagent/                       서브에이전트 실행 계층 (teammate P0 v4.53.0 / P1 v4.54.0 — docs/teammate/DESIGN.md)
│   ├── __init__.py          (6)    docstring only — 소비자는 서브모듈 직접 import (가변 전역 재수출 금지, C2 교훈)
│   ├── runner.py            (161)  delegate(일회성)와 teammate(상주)가 공유하는 3단계: `apply_role_overrides`(역할 md config 오버레이 — **로더-불가지론적**: delegate=agents/·teammate=teammates/ 로 로더는 분리(D11), 파싱된 config dict 만 소화. hooks 는 caller 위에 병합) · `create_subagent_ctx`(none/fork ctx 생성 + wire_format·예산 상속 + **현재 스레드의** 인스펙터 스코프에 ctx 등록 v4.52.0) · `run_subagent_message`(ctx 위에서 run_loop 1회, depth+1 — delegate 는 1회 후 ctx 폐기, teammate 는 같은 ctx 로 반복: 이 차이가 두 도구의 전부). 무거운 의존은 함수-내부 import (registry→delegate→context.manager 순환 회피, 테스트로 고정)
│   ├── roles.py             (77)   teammate 역할 md 로더 (D11(b)) — 전용 검색 경로 `.agent-cli/teammates/` → `~/.agent-cli/teammates/` → **패키지 내장 `teammates/builtin/`** (v4.59.0). ResourceLoader 공유, `_load_agent` 동형 계약 `(role_prompt, config, error)`. **`available_roles()`**: 역할 발견 표면 — 시스템 프롬프트 광고용(name+description, `disable-model-invocation` 제외)과 auto-spawn 스캔용(include_meta=True, 전체+meta) 두 뷰
│   └── teammate.py          (590)  **상주 세션 에이전트** (P1 코어): `Teammate`(key·역할·영속 ctx·inbox(SimpleQueue)·자기 stop_event·데몬 worker — 상태 전이 starting→idle→busy→…→dead 는 worker 루프 한 곳) + `TeammateRegistry`(main 루프 수명 소유, spawn/request/wait_reply/drain_replies/kill/shutdown_all + 회신 mailbox=Condition+list, `runner` DI seam 으로 테스트가 LLM 없이 스레딩 검증, 상한 env `AGENT_CLI_MAX_TEAMMATES`=4) + `build_reply_record`(배달 레코드 — **`tool:"teammate"`+additive `source:"teammate_reply"`; `tool:""` 금지**(v4.51.0 형식-개입 마커 오인 방지, 테스트 고정); over-cap 회신은 디스크 포인터 치환 — 전문은 worker 가 `teammates/<key>/replies/reply-<seq>.md` 에 항상 영속=P3 토대) + `tool_teammate`(mode 디스패치 — tool_bridge 인터셉트 진입점). worker 는 시작 시 `begin_prompt_scope(key)` 로 **상시** 인스펙터 스코프를 열고(요청 사이에도 칩 생존) 종료 시에만 고정 — ctx 도 worker 스레드에서 생성해 스코프 등록이 올바른 스레드에 떨어짐. 인터럽트 분리: main stop 은 teammate 무영향, kill/세션종료만 종료. **P2 ask→main 라우팅 (v4.55.0)**: worker 가 `_make_ask_handler(tm)` 을 서브루프에 주입(runner→run_loop→LoopConfig.ask_handler) — teammate 의 ask 는 사용자 대신 질문을 mailbox 에 `kind:"question"` 으로 push 하고 inbox 의 **다음 도착 메시지를 답으로 소비**(도착 순서가 답 — main 의 request 든 P4 인간 개입이든; 비-main 화자는 `[author]:` 접두). 대기 상태 `waiting_ask`, _SHUTDOWN 은 즉시 해제+재게시. `build_reply_record` 가 질문을 답변-op 안내 포함 관찰로 포맷(`source:"teammate_question"`), `wait_reply` 는 kind 불문 반환(질문 건너뛰면 상호 대기 교착). dispatch `_handle_ask(questions, handler=)` 는 handler 시 사용자 프롬프트 생략·동일 Q/A shape, **handler 없으면 종전 단일-인자 호출**(기존 patcher 표면 보존). **P3 resume 재생성 (v4.56.0, D7)**: `teammates.json`(fsio 원자 교체) = manifest(역할 프롬프트 통째 저장 — 역할 md 소실에도 무관, 논리 생사: kill/에러만 영구 dead·세션 종료는 idle 유지) + 미배달 pending 디스크 미러(push/drain/wait 마다 갱신 — `_cv` 는 RLock 이라 재진입 저장 안전). 부트스트랩(run/web)이 `restore(parent_ctx)` 무조건 호출 — fresh 세션은 파일 부재로 no-op, resume 세션은 revivable teammate 를 ctx `resume` 모드(runner 의 3번째 모드 — 자기 history 이어받기, LLM 비노출)로 재기동 + seq 이어가기(reply-N 충돌 방지) + kill 툼스톤 복원(status 가시성) + pending 복원(질문은 stale 마킹 — 재시작으로 답변 대기가 아님을 배달 문구가 안내). `tool_teammate` 가 매 호출 runtime 갱신 — restore 된 teammate 도 첫 request 부터 현 세션 provider 로. **P4 WebUI (v4.57.0, D8/D3)**: 렌더러 표면 `teammate_roster`(sticky — 재접속 snapshot 복원)+`teammate_message`(persistent — replay 로 창 복원, direction in/out/question) 를 registry 가 spawn/kill/request/회신/질문/상태 전이마다 호출. **인간 개입 비배달 규칙**: `tm.current_author` 가 "main" 이 아니면(웹 창 발신) 회신·질문을 main mailbox 에 넣지 않는다 — 창(SSE)에만. 엔드포인트 `POST /api/teammate/{key}/input`(닉네임 attribution `user:<nick>`, waiting_ask 면 답으로 소비)·`/kill`(join 오프로드) — `server.teammate_registry` 를 worker 부트가 주입. **idle 자동 재기동 = `MailWaker`**(subagent/teammate.py — web 무의존 순수 조율자): on_mail(idle 시 합성 wake 를 입력 큐에)·on_run_end(마지막 턴 경계 이후 도착분 레이스 봉합)·handle_dequeued(잔여 없으면 "skip" — 빈 run 방지)·armed 중복 방지. 프런트는 🤝 드로어(roster 칩+대화 버블+입력, app.js IIFE — 메인 SSE 가 CustomEvent 브리지, auto_review 패턴). **전문가 역할 (v4.59.0)**: 시스템 프롬프트에 `build_teammate_role_descriptions`(Teammate Roles 섹션 — `"teammate" in active_tools` 게이트, delegate 의 Agents 섹션과 동형)로 역할 광고 → 모델이 스스로 전문가 소집. 내장 역할 `teammates/builtin/`(researcher·code-reviewer, package-data 포함), `/create-teammate` 내장 스킬(disable-model-invocation), `auto_spawn()`(frontmatter `auto-spawn: true` → 부트스트랩이 restore 후 호출, 살아있는 동일 역할 dedup), 부트스트랩 runtime **프리필**(restore/auto-spawn 분이 스폰 도구 호출 없이 웹 창 첫 접촉을 받아도 provider 배선 보유). **worker 사망 통지 (Q4)**: 비정상 종료(ctx 생성 실패·기계 예외 — BaseException 포착)는 `kind:"died"` mail → `source:"teammate_died"` 관찰로 main 에 능동 통지(+MailWaker wake)·revivable=False; kill/세션종료(stop_event set)는 통지 없음. **다중 인스턴스+Live 광고 (v4.60.0)**: 같은 역할 중복 spawn 허용(원래 가능 — 구분 수단 신설), spawn `name`(인스턴스 라벨, `[a-zA-Z0-9_-]{1,24}`) → `format_teammate_label` "agt-x (coder · ui)" 가 회신/질문/died 헤더·status·스코프 칩·web roster·카드에 일관 표시, manifest 저장(resume 보존). **`build_live_teammates_section`**(system_prompt.py): 상주 멤버십(key·역할·name·description≤140자)을 main 시스템 프롬프트에 상시 광고 — compaction 면역+auto-spawn/resume 커버, "같은 작업 스레드는 request 재사용/병렬 독립 작업만 추가 인스턴스" 유도. 이중 게이트(`"teammate" in active_tools` + registry)라 teammate 서브루프 자신에겐 미노출. 재조립 트리거=`notify/consume_teammates_reload`(멤버십 변화만 — spawn/kill/died/restore; busy/idle 상태 전이는 제외로 KV 프리픽스 보호) — `_execute_turn` 의 directives/memory 플래그와 같은 자리에서 소비, 인스펙터 스냅샷도 동시 갱신. 내장 `coder` 역할(파일 스코프 규율 — 병렬 분담용). **mode:"resume" (v4.61.0)**: `resume_teammate(key)` — dead(kill/사망/restore 툼스톤) teammate 를 같은 key·fresh worker·ctx resume 모드(P3 기계)로 부활: 이전 문답 전부 기억, instance_name/description 보존, seq 이어가기(reply-N 비충돌), error 리셋, revivable 복귀(세션 resume 자동 재생성 대상). 가드: 산 사람 거부(409)·상한 존중. DIED 통지가 resume op 을 안내, 웹 dead 칩 ↻ = `POST /api/teammate/{key}/resume`. 이름은 세션 `--resume`·ctx resume 모드와 동일 의미론("이력 이어받아 재개")이라 채택 — respawn(초기화 오해)·restore(내부 API 충돌) 기각. **@명령 (범위 B, v4.62.0)**: `_try_dispatch_teammate_command`(main.py — try_dispatch_agent_or_skill 의 @분기 최선두 + run 경로의 @블록) — `@teammates` roster / `@agt-<key>` 상태 / `@agt-<key> <msg>` 직접 request(author="user" → D8 비배달). DispatchOutput 에 list_teammates/teammate_dispatch_result 표면(base 는 안전 no-op — 기존 어댑터 무파손). CLI 수신 = `MinimalRenderer.teammate_message`(to≠main 인 out/question 만 콘솔 라인, markup=False — main 발신분은 관찰 배달과 이중 표시 방지) — teammate_message 에 **`to` 필드**(수신자) 추가, worker 가 out=요청 화자/question=current_author 로 태깅. run 경로는 @agt 처리 후 quiescence 대기(회신 콘솔 출력까지 생존). **멤버십 즉시 인스펙터 반영**: `WebRenderer.update_prompt_section(scope,name,text)`(스냅샷 외과 갱신 — 교체/Teammate Roles 뒤 삽입/빈값 제거, 총계 재계산) + transient `prompt_changed`(열린 인스펙터 loadPrompt refetch — memory_changed 패턴)를 `notify_teammates_changed(registry)` 가 전 멤버십 변화 지점에서 구동 — idle 중 대화 창 kill 도 즉시 반영, 실제 프롬프트 재조립은 여전히 플래그가 다음 턴에 보장. **P5 큐 공용화 (v4.58.0, 사용자 지시 "web 공용화 우선")**: WebServer 의 pending deque+Condition 골격을 `agent_cli/input_queue.py` **InputQueue** 로 추출(enqueue/dequeue_blocking[timeout 지원]/nowait/cancel 소유권/snapshot/shutdown-drains-FIFO-first + on_change 콜백=web 큐 브로드캐스트 자리, `WebServer.SHUTDOWN is InputQueue.SHUTDOWN` identity 계약 유지 — 서버 메서드는 thin wrapper, web 테스트 무수정 통과가 게이트). CLI `run` 은 **큐 펌프**(`main._run_message_pump`)가 됨 — CLI 에 REPL 이 없다는 실측으로 설계 재해석: 초기 질의를 큐에 넣고, teammate 회신의 wake 도 같은 큐로, **정지=큐 비고+`registry.has_active_work()` 없음**(busy·큐잉·미배달; waiting_ask 는 교착 방지로 제외 — `waiting_ask_keys()` 로 경고 후 종료, resume 시 STALE). teammate 미사용 run 은 1회 실행 후 즉시 종료(무변경)
├── skills/                         프롬프트 스킬 시스템
...
tools/delegate.py   → tools/result, resource_loader, subagent/runner (실행 몸통, teammate P0)
subagent/runner.py  → (전부 lazy: context/manager, loop, render, hooks — 순환 회피)
subagent/roles.py   → resource_loader
subagent/teammate.py→ tools/result (+ lazy: subagent/runner·roles, render, context/token_estimator)
tools/teammate_tool → tools/base, tools/result
...
**편집 후 hashline echo (`_updated_region_echo`).** edit 는 원래 diff 만 반환했다 — diff 는 **무엇이** 바뀌었나는 보여주지만 `LINE#HASH` 태그가 없어, 모델이 방금 편집한 자리를 이어서 편집하려면 `read_file` 재왕복이 필요했다(회귀가 아니라 처음부터 없던 비대칭 — `write_file` 은 이미 rewrite 시 hashline echo 로 이 왕복을 없앴다). 이제 성공한 편집은 diff 뒤에 바뀐 영역 ±3줄의 **fresh hashline 블록**을 붙여(절대 줄번호), 후속 edit 을 read 없이 체이닝할 수 있다. 전체 파일 hashline 덤프(=edit 의 비용 이점 파괴)는 `_MAX_REGION_LINES` 상한으로 막고 나머지는 read_file 로 안내한다. mimicry 우려는 무관 — 이건 **관찰-side**(도구 결과) 변형이라 action 재공급이 아니다(`feedback_refeed_own_output_mimicry` 는 관찰-side 는 안전이라 명시, write 가 이미 안전하게 함).
...
### 6.5.1 Review tool-calls builder (`_format_tool_calls_for_review`)

> **NOTE (v4.4.0):** `ready_for_review` 가상 도구는 **제거**됐습니다 — 모델이
> complete 전에 자발적으로 호출하라는 self-review 도구였으나 실전 사용률 0
> (297레코드 세션 0회). `_format_tool_calls_for_review` 는 **보존** —
> auto-review(v4.5.0, web `🔍 Review` 토글)가 reviewer 컨텍스트로 재활용
> (`review.py::build_reviewer_task`). 함께 남겨뒀던 self-review 프롬프트 합성기
> `_build_review_observation` 은 실 호출자가 없어 v4.35.1 에서 삭제
> (reviewer 프롬프트 합성은 `build_reviewer_task` 가 담당).
...
레코드 순회는 `manager.iter_record_ops` — **멀티-op `ops` 레코드와 단수 legacy
`action` 레코드 양쪽**을 읽습니다 (이전엔 top-level `action` 만 읽어 md_array/react
가 `ops` 로 저장하기 시작한 뒤 섹션이 항상 비어 있던 회귀를 v4.35.1 에서 수리).
virtual tools(`complete` / `ask`)는 op 단위로 제외. 30개 초과 시 최근 30개만

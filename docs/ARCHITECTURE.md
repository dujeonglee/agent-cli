...
> 최종 업데이트: 2026-05-25
> 버전: 2.0.0-dev
> 총 소스: ~38,500 LOC (130 Python 파일) + ~55,000 LOC 테스트 (137 파일)
> 총 테스트: ~3,570 유닛
...
├── main.py                  (2540) CLI 명령어: run, web, setup, sessions, update. **C4 공용 부트스트랩 (v4.46.0)**: `SessionBootstrap`(frozen — provider 6-튜플+wire format+예산) + `_bootstrap_provider`(셋업→fail-fast 포맷 해석(v5.19.0: `resolve_wire_format` 체인 — 명시 `--response-format`(default None=미지정 감지) > resume 세션 메타(`session_format=` kwarg) > models.json 모델 바인딩 > DEFAULT; web 의 대화형 최근-세션 resume 은 부트 후 결정이라 caller 가 기록 포맷으로 재해석+`dataclasses.replace`)→70% 예산 폴백) + `_load_resume_session`(핸드셰이크 전 fail-fast) + `_build_context`(ContextManager 단일 조립) 를 run/web 이 공유 — 세션 획득 UX(run=명시 --resume 만, web=+최근세션 프롬프트)·renderer·worker 만 커맨드별. 예산 폴백은 `compute_token_budget`=(context×7)//10 로 **통일**(이전엔 run/web 상이 — C4 감사 발견). **`run --resume <id>`**(v4.46.0): web 과 같은 on-disk 세션 로드 → 복원 컨텍스트 위에 QUERY 주입(run↔web 상호 이어가기). **web MCP 배선**(v4.46.0): `_setup_mcp`+TOOLS.update+worker `run_loop(mcp_manager=)`+`capture_startup_system_prompt(mcp_manager=)`+shutdown `disconnect_all` — 이전엔 web 만 미배선. --style, --response-format, resume preview. **`update`**: `gh release view`로 최신 릴리스 태그 확인 → `_parse_version` 비교 → 새 버전이면 `gh release download`(wheel)+`pip install --upgrade`(`--check` 확인만, `-y` 확인생략). editable/dev 설치는 `_is_editable_install` 감지해 거부+`git pull` 안내(`--force` 우회). **`_prompt_model_capabilities` wire-format 질문 (바인딩 UX ②, v5.22.0)**: 대화형 모델 등록(감지 실패 폴백)에 "Wire format [auto]" 한 줄 — auto/빈 입력=필드 미기록(해석 체인 위임), `list_names()` 등록명만 수용(오타 재질문 — D2 조용한 폴백 금지), 선택 시 entry 에 `wire_format` 키 추가. **세션 표시 공유 헬퍼**: `_print_session(meta)` 가 id·시각 + `session_summary` 기반 `↳ 마지막 요청` / `→ 마지막 결과`(또는 `(in progress)`)를 한 블록으로 출력 — `sessions` 명령과 resume 프롬프트가 동일 포맷 공유. **`_maybe_resume_recent(workspace, response_format, prompt_fn)`**: `--resume` 없이 `web` 진입 시 가장 최근 세션을 보여주고 `[y/N]` 질의 — `y` 면 `load_session` 으로 이어가고(`is_resume=True`) 그 외엔 `create_session` 으로 새로 시작. `prompt_fn` 은 TTY 면 `input`, 비대화(파이프/cron)면 `None` → 묻지 않고 항상 새 세션 (안전 기본값). **`DispatchOutput` Protocol + `_ConsoleDispatchOutput` + `try_dispatch_agent_or_skill`** — `@<agent>`/`/<skill>` 접두사 처리 (listing, invocation, not-found) 공유 dispatcher. `run` 은 `_ConsoleDispatchOutput`(Rich 색상), `agent-cli web` worker 는 `web.server.WebDispatchOutput`(observation 이벤트) 어댑터 주입. unknown `@`/`/` 명령은 LLM으로 통과하지 않고 error observation 발사 (오타로 인한 사고성 LLM round-trip 방지). **`web` 명령**: `--resume <id>` 지원 — provider 핸드셰이크 전에 `load_session` pre-check로 unknown ID fail-fast, `ContextManager(resume=True)` 로 캐시 복구 후 `renderer.replay_from_history(ctx)` 한 번 호출해 persistent event buffer를 재구성 → 이후 새 SSE 연결의 snapshot replay로 이전 turn이 그대로 UI에 복원. **graceful shutdown**: `uvicorn.Server(config).run()` 직접 호출 + `KeyboardInterrupt` swallow + `finally` 블록에서 `renderer.shutdown_all_connections()` → `server.shutdown()` → `worker.join(timeout=2s)` → `finalize_session(...)` 순서로 정리 (lifespan shutdown 훅이 SSE generator를 먼저 닫아도 idempotent). **A1 병렬 디스패처 (v7.29.0, opt-in)**: `web --concurrency-contract parallel` 이면 `_worker_loop` 이 큐에서 꺼낸 메시지를 `TurnRegistry.submit` 으로 넘기고 **즉시 다음 메시지를 받으러 돌아간다**(worker_busy 를 켜지 않는다 — 그 플래그는 프런트 Send 게이팅이라 켜면 '돌고 있어도 더 보낼 수 있다'는 요점이 사라진다). 턴별 러너는 자기 `stop_event`·`origin_turn` 으로 `run_loop` 을 돌리되 **`dequeue_user_message=None`** — 병렬에서 큐의 주인은 디스패처이고, 턴이 직접 꺼내면 다음 사용자의 메시지를 자기 턴 안으로 삼켜 새 턴이 열리지 않는다. 직렬 본문은 **무수정**이고 배선 kwargs 중복은 의도(공통 함수로 접으면 delicate 한 클로저·stop_event 수명을 함께 흔든다). 알 수 없는 계약 이름은 Exit(2) — 조용한 직렬 폴백 금지(G1 동형). **관전 토큰 해석**: `--view-token` 은 `--spectators` 를 함의하고, `--token` 과 **같은 문자열이면 Exit(2)** — `_token_role` 이 full 을 먼저 보므로 같은 값을 두 역할에 주면 관전 링크가 조용히 전권을 나눠준다(읽기 전용이 아닌 관전 URL 은 없는 것보다 나쁘다). 계약 테스트 2층: 옵션 **게이트**(거부·기본값 해석)는 무거운 배선 이전에서 끊어 재는 `test_web_command.py::TestConcurrencyOptionValidation`, 게이트를 통과한 값이 **무엇에 연결되는가**는 `web()` 을 uvicorn 직전까지 실제로 부팅시키는 `test_web_boot_wiring.py`(21) — 후자가 없으면 부품 테스트가 전부 green 인 채 조립만 어긋나 병렬 계약이 조용히 직렬로 되돌아간다(레지스트리 생성·cap·게이트·`/api/stop` 훅·`server.turn_registry`·계측 프로바이더·on_change→렌더러·큐→submit·`origin_turn`·종료 수거·관전 토큰까지 배선 단위로 고정; 각 항목은 해당 배선을 제거하면 실패하는 것을 확인).
├── resource_loader.py       (144)  ResourceLoader — 파일 검색/우선순위 (스킬/에이전트/지시사항)
├── fsio.py                  (147, v4.47.0 · v7.29.0 append 직렬화)  **파일 저장 패턴의 단일 소유자** — ① 원자 교체 atomic_write_text/json(유니크 mkstemp→os.replace; 고정 tmp 는 v4.27.1 status.json 레이스 실측으로 금지, 부모 소실은 실패 시 mkdir+재시도) ② 가드 append append_line(history.jsonl 가드의 일반화) — **스레드 안전(v7.29.0)**: 경로 해시 스트라이프 락(`_APPEND_LOCKS`, 64개) 하에 open/write/close+재시도를 한 임계영역으로 묶는다. **근거는 실측**: 로컬 ext4 는 append 1회=write(2) 1회(8MB 페이로드도 BufferedWriter 가 raw 로 그대로 흘림)라 O_APPEND 원자성이 성립해 무손상이지만, **WSL 의 Windows 드라이브 마운트(/mnt/*, drvfs)** 는 O_APPEND 를 원자적으로 구현하지 않아 **4KB 페이로드에서도** 16스레드×8회 동시 append 가 128줄 중 45줄만 남기고 15줄이 깨진 JSON(1MB→28/128); 같은 조건에 락을 걸면 128/128·corrupt 0. 세션 디렉토리는 cwd 를 따라가므로 사용자가 Windows 드라이브에서 작업하면 history.jsonl 이 바로 그 마운트에 놓인다(NFS 도 같은 부류). 오염된 줄은 `store.load_records` 의 "깨진 줄 건너뜀" 정책에 걸려 **조용히 사라져** resume 시 원인 없이 기록이 증발한다. 스트라이핑을 쓴 이유 = 경로별 dict 는 서브에이전트 run 디렉토리마다 Lock 이 무한 누적(해시 충돌은 무관한 두 파일이 잠깐 줄 서는 것뿐, 정확성 무영향). 프로세스 로컬로 충분 = 한 세션 한 프로세스(web spawn-or-attach)·서브에이전트는 스레드. 데드락 무관 = 락 구간에서 open/write/close 만 호출(효과 락 M4 와 획득 순서 무관). 계약 테스트 `test_fsio.py::TestAppendLineConcurrency` 는 **비원자 쓰기를 주입**해(`_SplitWriteFile`) ext4 에서도 락 제거 시 실패하도록 — 호스트 FS 에 의존하면 가드 구실을 못 하기 때문 ③ 직접 쓰기는 도구 산출물 의미론이라 의도적 비사용. 소비: compaction.json(고정 tmp 교정)·web.json(비원자 결함 발견-수리 — 보드가 half-write 읽을 수 있었음)·status.json(자체 mkstemp 수렴)·models/config·memory.jsonl rewrite·session meta·DIRECTIVE.md·directive presets·turns.jsonl/raw appends. code_index builder 는 자체 원자 규약 보유(비수렴)
├── turn_metrics.py          (106, v7.30.0)  **M2 동시성 계측** — `{session_dir}/turns.jsonl` 에 `event: turn|lock|compact|reject|ctx` 행을 추가 기록하는 프로세스 전역 emitter (`--turn-metrics`, 기본 off). P1(TTFT)·P2/P3(락 대기)·P4(공정성)·N1(동시 압축)·N3(귀속) 실험의 단일 데이터 소스. 파일은 복구 관측 `TurnRecorder` 와 공유(행 구별=`event` 키; TurnRecord 행은 키 없음) — 한 파일 = 한 타임라인이라 분석이 조인 없이 인과를 본다. **전역+기본 off 인 이유**: 발화 지점이 서로 다른 수명에 흩어짐 — 서버(enqueue/reject)·TurnRegistry(dispatch/complete/interrupt)·LLMCaller(first_token, depth==0 1회 래치 + **llm_call 콜별 usage(v7.30.0 P6)** — input/output/cache_read 토큰, turn_id·depth 귀속; usage 는 어디에도 영속되지 않으므로 N-병렬 토큰 비용 실측의 유일한 소스)·effect_lock(enqueue/acquire/release + wait_ms/held_ms; 스레드 이름 `agent-turn-t{n}` 이 곧 턴 귀속)·ContextManager(compact begin/commit/stale + 세대·`active_turns` 프로바이더, query_added 로 msg_id↔스레드 사슬, **ctx snapshot/append/commit(v7.30.2 N5)** — 변형 카운터 `_commit_seq` 를 실어 스텝 staleness = (커밋 seq−1)−(스냅샷 seq) 가 조인 없이 산술로 나옴) — 인스턴스 배선은 그 자체가 직렬 경로를 건드린다(effect_lock 전역 스코프와 같은 정당화: 한 프로세스=한 세션). off 면 emit=None 체크 no-op(직렬 경로 보존). 시계: 구간은 `mono_ms`(단조, 스레드 경계 유효), 대조는 ISO `timestamp`. 포크가 설계만 한 BENCH_TRACE 의 첫 구현. 프라이버시=구조 메타데이터만. 계약 테스트=`test_turn_metrics.py`(22) + e2e 사슬(`test_parallel_turns_e2e.py`) + 거부 게이트(`test_web_server.py::TestRejectContract`) + `--turn-metrics` 배선(`test_web_boot_wiring.py::TestTurnMetricsWiring` — 파일 존재로는 판정 불가하므로 `event` 키 유무로 계측 행을 가려낸다)
...
│   ├── prompt.py            (138)  SystemPromptSvc + build_inspector_sections. **턴 스코핑 (v7.30 도입, 이후 병렬 기본 on)**: `set_turn_scope(turn_id, author, query)` 가 "이 턴이 수행 중인 요청"을 `Turn Scope` 섹션으로 못 박는다 — 공유 트랜스크립트에서 모델이 **남의 동시 질문에 답하는** 의미론적 혼선의 완화이며, 구조적 귀속(`reply_to`, thread-local)은 이미 정확하므로 대상이 아니다. **두 겹 게이트**: `LoopConfig.turn_scoping`(CLI `--turn-scoping/--no-turn-scoping`, **기본 on** — 라이브 측정 56/80 → 0/80 뒤 출하 기본값이 됐고 절제는 명시적 off) **AND** `origin_turn` 비어있지 않음(=병렬 계약). 직렬에 걸지 않는 이유는 거기서 실행 중 주입된 질문이 오히려 그 턴의 수행 대상이기 때문이다. 섹션은 `rebuild()` 가 **매번 다시 붙인다** — Inspector 의 DIRECTIVE 재빌드가 섹션을 처음부터 만들므로 재부착이 없으면 조용히 사라진다. `apply_hook_sections` 는 `Hook: ` 접두만 갈아끼우므로 스코프는 정적 섹션으로 보존. 인용은 `_SCOPE_QUOTE_LIMIT`(400자)로 자르고 공백을 평탄화한다(요청 원문은 이미 user 메시지에 있으므로 여기서는 **지목**만 하면 되고, 줄바꿈이 남으면 `> ` 인용 블록이 첫 줄만 인용하고 나머지가 평범한 지시문으로 샌다). 가드=`test_turn_scoping.py`(12)
...
│   ├── turns.py             (363, v7.29.0)  **동시 병렬 턴 레지스트리 (A1 코어)** — 포크 `pi.ts` 의 `RuntimeHandle`·`pumpConcurrent` 이식. `TurnRegistry(runner, max_concurrent, on_change)`: `submit()` 이 cap 여유가 있으면 같은 호출에서 즉시 디스패치, 초과분은 FIFO 대기. **turnId 는 디스패치 시점 발급**(`t1`,`t2`… — `pi.ts:240-244`: 그래야 seq 가 실제 디스패치 순서와 일치하고 `interrupt(turnId)` 가 '실제로 시작된 턴'만 가리킨다. 대기 항목은 id 가 없다). 완료/**실패 양쪽**에서 슬롯 회수 후 재펌프(한쪽만 회수하면 실패 턴이 슬롯을 영구 점유해 세션이 굳는다). `interrupt(turn_id, conn_id=)` = 지정 턴만(소유자 검증 옵션), `interrupt_all()` = 세션 전역 `/api/stop` 의 병렬판, `active_count()`(inflight) / `pending_count()`(cap 대기) / `is_busy()`(둘 다 — idle-reaper 용) / `wait_idle()` / `shutdown()`. 락 구간에서는 자료구조 조작 + `Thread.start()` 만 하고 **runner 는 절대 락 아래서 부르지 않는다**(ctx·renderer·효과 락과의 순환 차단). **직렬 모드에서는 이 모듈이 생성조차 되지 않는다** — 포크의 '이중 경로 완전 분리'(`pi.ts:559-568`) 복제. **A5 공정 큐 (v7.29.0 M5)**: 펌프가 FIFO pop 이 아니라 **eligible-scan splice** — 큐를 머리→꼬리로 보며 "conn_id 가 비었거나 그 conn_id 의 활성 턴이 없는" 첫 항목을 고른다(포크 `pumpConcurrent` pi.ts:226-237 + `hasActiveUser` 212-218). 한 사용자가 큐에 N건을 쌓아도 per-user=1 이라 1건만 활성이 되고 나머지는 스킵돼 **다른 사용자가 슬롯을 얻는다**(기아 방지). 대기열 순서는 보존 — 스킵은 건너뛰기지 재정렬이 아니다. **빈 conn_id 는 게이트 면제**(CLI·MailWaker wake 처럼 사람에게 귀속되지 않는 턴) — 빈 값끼리 매칭시키면 그것들이 서로를 막아 병렬이 통째로 무너진다. `_active` 스캔이 per-user 활성의 **단일 진실원천**(별도 카운터 맵은 취소·실패 경로에서 어긋나 영구 차단을 만든다). 효과 락의 엄격 FIFO(추월 금지)와 방향이 반대인 것은 의도 — 거기선 **같은** 자원을 다투므로 추월이 기아를 만들고, 여기선 **다른** 사용자에게 기회를 주는 것이 공정성이다. 미포함: 라운드로빈 가중치. **per-user 게이트 ablation 스위치 (v7.30.0)**: `TurnRegistry(per_user_gate=False)` + CLI `--no-per-user-gate` — 순수 FIFO+cap 대조군(P4 벤치 전용; 기본 on 무변). 가드=`test_turn_registry.py::TestPerUserGateAblation`. **`on_change` 배달은 전용 `_notify_lock` 으로 직렬화 (v7.30.1)**: 콜백은 인자가 없고 구독자가 `active_count()` 로 현재 값을 스스로 읽으므로, 두 턴이 동시에 끝나면 '읽기→(선점)→쓰기' 사이에 남이 끼어들어 **낡은 카운트가 최종값으로 굳는다** — 레지스트리는 비었는데 `WebRenderer.worker_is_busy` 가 영원히 True 라 idle self-reap(`--idle-timeout`)이 끝내 발동하지 않고 status.json 도 계속 거짓말을 한다. 배달 락 아래서 콜백이 값을 **다시 읽기** 때문에 마지막 배달이 항상 최신 상태를 본다. `_lock` 이 아니라 전용 락인 것은 규율 유지 — 콜백은 여전히 `_lock` 밖에서 돌아 renderer 락과 중첩되지 않는다(획득 순서 `_notify_lock` → renderer `_lock`, 역방향 없음). 가드=`test_turn_registry.py::TestStatusNotificationOrdering`(락 제거 시 결정적 실패). 부수효과 직렬화는 M4 효과 락 소관 — 여기서 병렬화되는 것은 **추론**이다. **`snapshot()` 는 `/api/turns` 페이로드**(턴 id·작성자·연결 id) — **메시지 본문은 싣지 않는다**. 이 표면은 라우트 표상 `read` 라 관전 토큰으로도 읽히므로, 필드를 더할 때 "스트림이 이미 보여주는 것 이상을 노출하지 않는다"는 경계를 함께 확인해야 한다(본문은 렌더러 표시 규칙을 거쳐 나가는데 여기로 새면 그 규칙을 우회한다). 가드=`TestSnapshot` — 필드 집합을 **완전 일치**로 못박아 `text` 추가 시 결정적으로 실패한다(서버 쪽 테스트는 author/id 의 존재만 보므로 그 경로로는 안 잡힌다). 계약 테스트=`test_turn_registry.py`(34) + `test_parallel_turns_e2e.py`(12) + 부트 배선 `test_web_boot_wiring.py`(21)
...
│   ├── effect_lock.py       (252, v7.29.0)  **부수효과 계층 락 (M4/A3)** — 포크 `sandboxLock.ts` 이식. A1 이 추론을 병렬화했다면 여기서는 그 턴들의 부수효과를 **충돌 단위**로 직렬화해 동시 파일 쓰기 진입을 0으로 만든다. `hold(intent)` 컨텍스트 매니저 + 워크스페이스별 `{running[], queue[]}` 상태. **엄격 FIFO(추월 금지)** — 큐 머리가 막히면 뒤도 대기해 SHELL 이 파일 작업에 굶지 않는다(공정성을 위해 동시성을 조금 포기; 포크 검증 선택). 스코프 3종: `off`(기본, ablation 대조군 = 기존 동작 보존) / `workspace`(포크 v1 단순 mutex 재현 — 다른 파일도 줄 세워 E2-B 에서 1.07× 로 붕괴한 팔) / `conflict`(행렬 적용, 실사용). **프로세스 전역**인 이유 = 락은 한 워크스페이스에 대한 것이고 서브에이전트는 프로세스가 아니라 스레드다; 루프별 설정이면 어떤 루프는 잠그고 어떤 루프는 안 잠가 보호가 무의미해진다. **UNKNOWN 은 잠그지 않는다(M1 숙제의 해소)** — UNKNOWN 이 성격이 다른 셋을 뭉뚱그리고 있었다: ①복합 도구(`agent`/`run_skill`)는 중첩 루프의 잎이 각자 락을 잡으므로 부모가 배타를 쥐면 **교착**(자식은 다른 스레드라 재진입 불가) ②`ask` 는 사람 대기 중 전체 정지 ③`memory`/`read_context`/`code_index`/`fetch` 는 워크스페이스 경로가 아닌 자기 상태를 만지며 각자 가드 보유. 셋 다 "이 락으로 정렬할 효과가 없다" → 운용 의미를 **정렬 대상 아님**으로 확정. `is_exclusive` 는 "잠근다면 배타인가"만 답하고 **잠글지**는 락이 정한다. 훅 지점 2곳: `tool_bridge._invoke_regular`(잎 도구 전부) + `dispatch._dispatch_edit_batch`(순수 배치 함수를 직접 불러 앞 경로를 안 타므로 별도 — 빠뜨리면 배치 편집만 보호 밖으로 샌다). 경로 정규화는 `abspath(normpath())` — 같은 파일을 상대/절대로 부르면 키가 갈려 보호가 새기 때문(win32 소문자화는 Linux 기준 결정으로 미포함). 계약 테스트=`test_effect_lock.py`(29, 락 무력화 시 실패로 하중 확인 + lost-update ablation + `TestEditBatchPath` 로 배치 훅 지점 고정) + `test_parallel_turns_e2e.py::TestEffectLockUnderRealParallelTurns`(실 `run_loop` 배선 E2E — 같은 경로 peak=1 / 다른 경로 peak>1 / `workspace` 스코프 peak=1 / **`off` 스코프 peak>1 음성 대조**로 '락이 아니라 우연한 직렬 실행을 재고 있을' 가능성을 배제)
│   ├── effect.py            (78, v7.29.0)  **부수효과 인텐트 어휘** — `EffectKind`(FILE_READ/FILE_WRITE/FILE_DELETE/SHELL/PACKAGE/UNKNOWN, `str` 혼합이라 JSON/로그에 값 그대로) + `EffectIntent(kind, path)` frozen dataclass + `is_exclusive` 술어. 포크(Coagora) `sandboxLock.ts:16-22` 의 **호환성 행렬**을 옮긴 것: FILE_WRITE/READ 는 **경로가 다르면 병렬·같으면 직렬**, 그 외 전부 배타. 빈 경로는 락 키로 신뢰 불가라 배타(`turn.ts:113-114`). FILE_DELETE 가 배타인 이유 = `rm -r src/` 와 `write src/x.py` 가 경로 키가 달라 병렬 진입하면 **새 ENOENT 레이스 클래스**가 생김. **FILE_DELETE/PACKAGE 는 어휘로만 존재** — 본류엔 파일 삭제 전용 도구가 없고(삭제·패키지 설치 전부 `shell`=이미 배타) 명령 문자열을 추측 파싱해 만들어내지 않는다. **경로 정규화는 여기서 안 함** — 락 키 동일성은 M4(락 모듈) 책임, 포크도 `normalizeLockPath` 를 락이 소유(`touched_paths` 가 원본 경로를 그대로 돌려주는 것과 같은 규율). 근거: docs/research/11-upstream-merge-plan.md §2 M1, 10-agent-cli-gap-analysis.md §1 A3

...
│   └── openai.py            (200)  OpenAI 호환 API (function calling + reasoning_content + streaming + TTFT). **`response_format={"type":"json_object"}` 는 `supports_structured_output` AND `not skip_json_format` 일 때만 전송** — wire plugin 이 비-JSON shape(prefix_md 의 markdown 섹션)면 `provider_call_kwargs`로 `skip_json_format=True`를 보내고, 이를 honor 해야 함. JSON 모드를 markdown prompt 에 강제하면 omlx/mlx 가 degenerate(`[2025]`/`[1000,1000]` 깨진 토큰)하는 버그가 있었음 (prefix_md 기본 전환이 노출 — bakeoff 는 provider 우회라 못 잡음).
│
├── tools/                          도구 시스템

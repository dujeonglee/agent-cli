...
│   ├── react.py             (688)  ReActFormat — 기본 plugin. ReAct-shape 문자열 (JSON `{thought, action, action_input}`) + recovery wording + 3-stage fallback parser (`parse_react`) + stage-2 JSON repair helper (`repair_json`) 모두 self-contained. WireFormat ABC 상속해 lifecycle default 사용 — format-specific 메서드만 정의. **`thought_required=False`·`action_required=False` (prefix_md 와 통일)**: `_normalize_action_input` 이 action 없어도 non-reserved sibling 을 action_input 으로 bundle 해 dropped-action 을 infer 로 복구 (Layer 1 virtual-tool alias 는 action 있을 때만). (이전 EnvelopeFormat은 2026-05-10 측정 후 폐기 — Phase 1 bakeoff에서 mistral 0% / qwen thought 9.5%로 wire-shape 결정성 약점 확인)
│   ├── md_array.py          (457)  **MdArrayFormat (기본 wire format — 멀티-op, docs/inputs-array-schema/DESIGN.md)** — 마크다운 envelope(`## Thought`/`## Action`) + `## Action` 본문 = flat `{action, ...params}` op 들의 JSON 배열. 한 턴 여러 INDEPENDENT op(배열 원소), plain 키(no prefix), **op 하나=대상 하나(per-tool 배치 금지 — 배치 중첩이 27B 90% 깨뜨린 실측)**. bare 객체=1-op 관용. **종료=thought-only**(`## Action` 생략) lenient 파싱: 빈/`None` marker 본문, bare `{result}` 객체, 헤더 없는 평문, **`## Input` 잔재 꼬리·빈 op(`{}`/`[{}]`)·빈 배열(`[]`)** (Phase-2 + 실전 실측 — 종료 시점에 prefix_md prior `## Action`+`## Input {}`, 또는 ready_for_review 후 "Decision: complete"+`## Action\n[]` 를 내면 각각 NO_ACTION·NO_JSON 루프化했음; 단 thought 가 있을 때만 — `[1,2,3]` 같은 비-dict payload 는 malformed=parse 실패 유지) 모두 terminal(답=thought/result). NO_ACTION recovery 문구에도 "끝났으면 `## Action` 생략" 탈출구 포함. `multi_op=True`/`exposes_complete=False`(complete 미노출 — 단 모델이 발명한 complete op 은 루프가 관용). **`parse_turn` override**가 본 경로(ParsedTurn: N ops/terminal); `parse` 는 1st-op 단수 투영(ABC 호환용). **history 직렬화 override**: ops 레코드 `{role, thought, ops:[{action, action_input}]}` / terminal 레코드 `{role, thought, terminal}` — render 가 wire 모양 재방출(round-trip 테스트로 고정). sanitize_thought(줄단독 `##` 헤더 제거)·is_degenerate(빈 헤더 ≥2) 는 prefix_md 동형. **2026-06-11 기본 전환** (Phase-2 95.2%=react + 실전 150턴 0.7% 검증) — `DEFAULT_WIRE_FORMAT`. **format_rules 능동 배치 유도(B, DESIGN §6)**: 독립 op 를 한 턴에 묶도록 결정 휴리스틱+3-op read 예시로 적극 steering — 단 의존-분리(뒤 op 가 앞 결과 필요시 턴 분리)·중첩 금지(list-in-op) 두 가드레일 동등 비중 유지(과유도 회귀 방지). read_file 인라인 가이드에도 same-turn 배치 힌트(code_index 는 기존 보유). 실전 DOOM 분석: 연속 readonly 단일op 런 23개=상한 ~38턴(24%) 배치 기회.
│   └── prefix_md.py         (526)  PrefixMdFormat — 마크다운 H2 헤딩 wire format (`## Thought / ## Action / ## Input`). small-LLM이 XML envelope보다 자연스럽게 emit하도록 설계. parser: strict `^## X$` line-anchored 매칭, last-wins on `## Action`+`## Input` (sub-header drift 방어), action body는 단일 토큰 (`^[\w.-]+$`) 검증. **thought·action 둘 다 optional (`thought_required=False`, `action_required=False`)**: `## Thought` 헤딩 있으면 그 섹션, 없으면 `## Action` 앞 prose 를 thought 으로 간주; **action body 가 무효/빈, 또는 `## Action` 헤딩 자체가 없어도 trailing Input JSON(또는 bare dict)을 `_extract_json_dict` 로 추출해 action_input 을 보존** → loop 이 infer_action 으로 도구 복원 (react parity). ★ prefix_md 도입(3a47cbf)부터 빈 `## Action` 에서 Input 을 버려 dropped-action 복구가 불가였던 regression 수정 — 실전 1780718751 세션 18/188턴 NO_ACTION 이 전부 이 케이스. parse_stage: 0=복구불가(valid action body·trailing input 모두 없음), 1=valid action+Input, 2=valid action+Input 깨짐/없음, 3=action 없음/무효 but action_input 추출됨(infer 가능). provider_call_kwargs override = `{"json_mode": False}` — JSON 모드는 `{` 강제라 markdown(`## `)과 충돌해 항상 끔. **`is_degenerate`** override(`_DEGEN_RUNAWAY`): **빈 wire 블록**(헤더+공백만, body 없음) ≥2 = format runaway. lookahead 로 연속 빈 헤더(`## Thought ## Action ## Action ## Action`)를 모두 카운트; 정상은 헤더 사이 body 있어 0, thought optional 1개는 허용. streaming 중 break(degeneration_check) + 최종 라벨(FAILURE_DEGENERATE). 단일 stop 문자열로는 "반복"·"빈 블록"을 못 보고 thought-optional 정상을 오탐해서 drop함. **`sanitize_thought`** override: thought 내 줄단독 `## Thought/## Action/## Input` 라인 제거(`the ## Thought section` 같은 inline 언급은 보존) — 모델이 reasoning 에 흘린 sentinel 이 prior 로 재주입돼 mimicry→runaway 되는 **근본 고리 차단**(degeneration guard 가 증상 억제, 이게 원인 제거). 나머지 lifecycle은 ABC default 사용.
...
│   ├── http.py              (214)  post_with_retry (Timeout/ConnectionError 재시도, pre-stream only, 고정 1초 백오프) + **`interruptible_lines(r, interrupt_check, poll=0.2)`** — 스트리밍 read 를 no-data gap 에도 중단 가능하게: `r.iter_lines()` 는 다음 바이트까지 블로킹이라 "chunk 마다 flag 확인"으로는 **첫 토큰 전 TTFT 창**(큰 프롬프트는 수 초 stall)을 못 깬다. 소켓 read timeout 도 안 됨(requests 는 read timeout 을 terminal 로 처리, 스트림 재개 불가). 그래서 블로킹 read 를 데몬 reader 스레드가 돌려 queue 로 넘기고, 제너레이터가 `poll` 주기로 queue 를 폴링하며 빈 폴링마다(+진입 시 1회) `interrupt_check` 확인 → True 면 `r.close()`(reader 의 blocked recv 를 read 소유 측에서 깨움) 후 중단. interrupt_check 없으면 plain `iter_lines()` pass-through(스레드 없음). iter_lines 의 진짜 에러는 전파, interrupt 로 인한 자기 close 에러는 무시. 양 provider 공유(둘 다 requests 기반).

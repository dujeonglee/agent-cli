...
│   ├── core.py              (~865) AgentLoop 오케스트레이터(라이프사이클·시그널·큐주입·_execute_turn 지휘 + 위임/property 표면). **wire_format None 폴백은 ctx-우선** (v5.19.0 G2 수리): `ctx.wire_format if ctx else 전역 기본` — 종전엔 무조건 전역 기본이라 비기본 포맷 세션의 서브에이전트가 ctx(히스토리 렌더)/loop(파서·프롬프트) split-brain (I2 불변식: cfg.wire_format ≡ ctx.wire_format)

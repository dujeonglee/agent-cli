...
│   ├── symbols.py           (~545) read_symbols 도구 — tree-sitter 기반 구조 인지 reader (mode='list' outline / mode='fetch' 단일 심볼 body, hashline 포맷 출력 → fetch→edit_file 직결). 언어: Python·JS/TS·C/C++·Markdown (.c/.h 포함 모두 C++ grammar). 정의 우선(declaration vs definition), nested 표기는 언어별 관습 (`Foo.bar` / `ns::Foo::bar` / `## Setup`). `get_supported_extensions()`가 prompt inline guide + error 메시지 single source
│
├── context/                        컨텍스트 관리

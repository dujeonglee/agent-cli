...
├── prompts/                        프롬프트 템플릿
│   ├── __init__.py          (1)
│   ├── session_state.py     (117)  세션 상태 블록 (프롬프트 꼬리) — 휘발 상태(컨텍스트 사용량·턴·Live Agents·Session Memory·압축 경고) + **Task Guidelines 전체 (v8.52.0)** 의 순수 렌더. 가이드라인 이동 근거: Harbor tb21 실측에서 35B-A3B 가 시스템 프롬프트 Primacy 의 원칙(백업·간결)을 0/1 무시, 같은 문장을 마지막 메시지 꼬리에 두면 3/3 준수 — 비용은 매턴 ~0.4K 재프리필(꼬리는 어차피 비캐시 구간). 조립은 `LLMCaller._build_session_state`, 전달은 `ContextManager.get_messages` 꼬리 append (미영속). 위치 근거는 §KV 프리픽스.

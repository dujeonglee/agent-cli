...
│   ├── dispatch.py          (1257) TurnDispatcher(+_op_* 4헬퍼) + 관찰조립·ask 자유함수군(_append_observation/_handle_ask/_extract_questions/_try_echo_as_final/…)
│   ├── skill_invoke.py      (168)  _handle_run_skill(깊이/사이클 가드 포함)
│   ├── core.py              (859)  AgentLoop 오케스트레이터(라이프사이클·시그널·큐주입·_execute_turn 지휘 + 위임/property 표면)

...
│   ├── context.py           (362)  read_context 도구 — **history 를 SQL 로 질의**. 필터 파라미터 더미 대신 **단일 `query`(SQL SELECT)** 프리미티브: history.jsonl 을 인메모리 sqlite `history` 테이블로 온-디맨드 적재(컬럼 `session/loc/seq/kind/turn/ts/tools/files/author/text`)하고 LLM 이 SELECT 작성. 컬럼은 **읽기 시점에 `manager._classify_record`(kind/tools/text) + `extract_file_paths`(files) 로 유도** → 어떤 레코드 shape 든 동작, prefix-관습 재추측 없음. `turn`/`ts`/`author` 는 레코드에서. **읽기전용**: sqlite authorizer 가 SELECT/READ 외 거부 + 인메모리 DB 는 콜마다 재빌드·폐기(쓰기 무해), 비-SELECT prefix 거부. 결과 50행 cap. `query` 생략 시 스키마+예시+세션목록(discovery). `sessions`(current/all/<id>) = 테이블에 적재할 데이터 범위. (구 mode=list/search/fetch + kind/tool/author/turn/scope 파라미터 → 전부 SQL 로 흡수, 736→362 LOC.) `files` 컬럼은 다음 BM25(FTS5)와 같은 sqlite 기반.

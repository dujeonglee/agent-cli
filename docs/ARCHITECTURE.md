...
│   ├── turns.py             (257, v7.29.0)  **동시 병렬 턴 레지스트리 (A1 코어)** — 포크 `pi.ts` 의 `RuntimeHandle`·`pumpConcurrent` 이식. `TurnRegistry(runner, max_concurrent, on_change)`: `submit()` 이 cap 여유가 있으면 같은 호출에서 즉시 디스패치, 초과분은 FIFO 대기. **turnId 는 디스패치 시점 발급**(`t1`,`t2`… — `pi.ts:240-244`: 그래야 seq 가 실제 디스패치 순서와 일치하고 `interrupt(turnId)` 가 '실제로 시작된 턴'만 가리킨다. 대기 항목은 id 가 없다). 완료/**실패 양쪽**에서 슬롯 회수 후 재펌프(한쪽만 회수하면 실패 턴이 슬롯을 영구 점유해 세션이 굳는다). `interrupt(turn_id, conn_id=)` = 지정 턴만(소유자 검증 옵션), `interrupt_all()` = 세션 전역 `/api/stop` 의 병렬판, `active_count()`(inflight) / `pending_count()`(cap 대기) / `is_busy()`(둘 다 — idle-reaper 용) / `wait_idle()` / `shutdown()`. 락 구간에서는 자료구조 조작 + `Thread.start()` 만 하고 **runner 는 절대 락 아래서 부르지 않는다**(ctx·renderer·효과 락과의 순환 차단). **직렬 모드에서는 이 모듈이 생성조차 되지 않는다** — 포크의 '이중 경로 완전 분리'(`pi.ts:559-568`) 복제. 미포함(M5 소관): per-user 1활성턴 게이트·라운드로빈 공정 큐·`/api/turn/{id}/interrupt` 엔드포인트. 부수효과 직렬화는 M4 효과 락 소관 — 여기서 병렬화되는 것은 **추론**이다. 계약 테스트=`test_turn_registry.py`(19) + `test_parallel_turns_e2e.py`(7)

...
├── paths.py                 (47)  **`.agent-cli` 경로쌍 단일 소스 (v8.40.0, 리뷰 §4.5)**: `scoped_paths(*parts)` = `[cwd/.agent-cli/<x>, home/.agent-cli/<x>]` — 선언은 항상 [프로젝트, 사용자] 순=우선순위 순. **`sessions_dir()` (v8.50.0)**: 세션 루트의 단일 소스 — `AGENT_CLI_SESSIONS_DIR` env 면 그 경로(`~` 확장), 아니면 종전과 동일한 cwd 상대 `.agent-cli/sessions`(상대 유지 — 기록·표시 경로 형태 불변). 종전 3곳(context/session·tools/context·main web 인스턴스 파일)의 손-조립 리터럴을 흡수, 소비 모듈은 `_SESSIONS_DIR` 모듈 상수로 import 고정(테스트 monkeypatch seam; 구 `_SESSIONS_BASE` 는 sessions 접미 유무가 모듈마다 달라 폐기). 등가성=test_paths.py::TestSessionsDir. 소비 7모듈(config config.json/models.json·mcp.json(역순 소비는 `reversed(...)` 파생)·hooks.json·hooks/ 디렉토리·skills·agents 프로파일·DIRECTIVE 베이스)이 모듈-레벨 상수를 여기서 파생(cwd 는 종전과 동일하게 import 시점 고정, 테스트 monkeypatch seam 유지). **hooks.json 병합 규칙 통일 (v8.40.0, 유일한 의도 동작 변경)**: 종전 first-found-only(프로젝트 파일 존재 시 사용자 전역 통째 무시) → 이벤트별 연결 병합(프로젝트 matcher 먼저, 둘 다 발화 — Python 훅 디렉토리·스킬 frontmatter 훅과 동형 철학). 등가성=tests/test_paths.py(8상수 핀)+test_hooks.py::TestLoadHooksMergesBothScopes

...
> 최종 업데이트: 2026-03-26
> 버전: 2.0.0-dev
> 총 소스: 6,486 LOC (46 Python 파일) + 6,555 LOC 테스트 (26 파일)
> 총 테스트: 463 유닛 + 65 통합 = 528개
...
│   ├── manager.py           (301)  ContextManager (세션별 scratchpad, 구조화 요약)
│   ├── scratchpad.py        (395)  Scratchpad + Artifact + ContextBudget + 세션 격리
...
└── planning/                       Planning Mode
    ...
    └── executor.py          (204)  Phase 3: 단계별 실행 (Tool RAG)
...
planning/generator  → parsing/plan_parser, planning/models, prompts/system_prompt,
                      providers/base, providers/compat, render, tools (TOOLS, VIRTUAL_TOOLS)
planning/executor   → loop, planning/models, providers/base, providers/compat,
                      render, tools (TOOLS, VIRTUAL_TOOLS)
...
| (매칭 없음) | 전체 실제 도구 (VIRTUAL_TOOLS 제외, 폴백) |
...
| 분류 | 파일 수 | 테스트 수 | 실행 방법 |
|------|---------|----------|----------|
| 유닛 테스트 | 26 | 463 | `pytest tests/ -m "not ollama_integration"` |
| 통합 테스트 | 1 | 65 | `pytest tests/test_integration.py` |
| **전체** | **26** | **528** | `pytest tests/` |

...
│       ├── team_model.js    (219)  **Team 스윔레인 도출 (Phase 3)**: 순수 DOM-free 모듈 — SSE(agent_roster→레인·agent_msg in→out→작업구간/메시지·scope_start kind→skill밴드/one-shot)를 {lanes,agents,messages,oneshots,skillBands} 로 도출. dual Node/browser export 라 Node 유닛(test_team_model)으로 뮤테이션 검증.
│       ├── team_view.js     (316)  **Team 스윔레인 렌더 (Phase 3)**: TeamModel→SVG(레인 칩·역할hue 작업바·peer 메시지 커넥터·skill 밴드·one-shot 서브트랙·Fit/Live window). 전 색상 var(--…) 테마 반응. app.js 가 TeamView.ingest 로 급전, Timeline/Team 토글로 표면 전환(상호배타). 브라우저 e2e=test_team_swimlane.

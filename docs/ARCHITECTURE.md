...
├── web/                            agent-cli web 서버 (optional dep, `pip install agent-cli[web]`)
│   ├── __init__.py
│   └── server.py            (~200) FastAPI app + 4 endpoint. `GET /api/health` (auth 없음), `GET /api/stream` (SSE, 토큰 인증, takeover-aware), `POST /api/input` (chat/prompt/confirm 통합), `POST /api/abort` (`prompt_user`/`confirm` 인터럽트). 단일 active client — takeover 모델 (`__close__` sentinel로 깔끔). 토큰은 `secrets.compare_digest` 상수시간 비교. `stream_events` async generator가 snapshot replay → live loop 순서로 yield, `__close__` sentinel 만나면 break

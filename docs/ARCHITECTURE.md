...
│   ├── anthropic.py         (227)  Anthropic Messages API (tool_use + thinking blocks + streaming + TTFT + prompt cache via cache_control). **`interrupt_check`** kwarg(zero-arg predicate) 가 있으면 `_handle_stream` 이 text chunk 마다 polling → True 면 stream 닫고 `stop_reason="interrupted"` 반환(loop 이 partial 폐기). openai 와 parity (단 degeneration 조기-break 은 openai 전용 — anthropic 은 downstream 라벨에 의존).

...
│   ├── prompt.py            (138)  SystemPromptSvc + build_inspector_sections. **턴 스코핑 (v7.30 도입, 병렬 기본 on)**: `set_turn_scope(turn_id, author, query)` 가 현재 요청을 `Turn Scope` 섹션으로 지정한다. 이는 모델 순응에 의존하는 의미적 완화이며 파일 격리 보장이 아니다. P1 capability 모드는 별도의 `Turn File Capability` 섹션으로 승인 경로, staging, shell/복합 도구 차단을 알린다. 두 섹션 모두 `rebuild()` 뒤 재부착된다. 가드=`test_turn_scoping.py`, `test_turn_isolation.py`.

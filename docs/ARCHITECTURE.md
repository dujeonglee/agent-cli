...
│   ├── anthropic.py         (256)  Anthropic Messages API (tool_use + thinking blocks + streaming + TTFT + prompt cache via cache_control). **`degeneration_check`**(= `wire.is_degenerate`, provider-독립)·**`interrupt_check`**(zero-arg) 두 predicate 를 `_handle_stream` 이 text chunk 마다 적용 → degeneration 은 `'#'` 게이팅 후 True 면 `stop_reason="degenerate_runaway"`(loop 이 라벨·복구), interrupt 는 매 chunk True 면 `stop_reason="interrupted"`(loop 이 partial 폐기). openai 와 조기-break 동작 동일 — loop 이 두 provider 에 같은 predicate 를 넘기므로 최적화도 대칭(이전엔 anthropic 이 degeneration_check 를 받고도 버리는 비대칭 부채였음).
│   └── openai.py            (244)  OpenAI 호환 API (function calling + reasoning_content + streaming + TTFT). **`degeneration_check`** kwarg(= `wire.is_degenerate`)가 있으면 `_handle_stream` 이 누적 텍스트에 적용 → True 면 stream 을 닫고 break(format-runaway 조기 중단, `'#'` 포함 chunk 에서만 검사해 O(headers)). truncated content 는 `stop_reason="degenerate_runaway"` 로 반환돼 downstream 에서 parse·라벨. **`interrupt_check`** kwarg(zero-arg predicate, = loop `_interrupt_check` → `stop_event.is_set()`) 가 있으면 chunk 마다(게이트 없이 — bool read 라 cheap) polling → True 면 `r.close()` + `stop_reason="interrupted"`. degeneration partial 과 달리 loop 이 이 partial 을 **파싱·기록 없이 폐기**(사용자가 방향 전환). close 는 `r` 을 소유한 읽기 스레드에서 일어남 — 시그널핸들러/타스레드가 읽는 중인 소켓을 닫는 reentrant/race 를 피하려고, loop 은 flag(`stop_event`)만 세팅하고 닫기는 스트리밍 루프가 스스로. **`response_format={"type":"json_object"}` 는 `kwargs["json_mode"]` 가 True 일 때만 전송 — provider 는 capability 를 직접 안 본다.** `json_mode` 는 **`WireFormat.provider_call_kwargs(capabilities)`** 가 결정(wire ⨯ capability 단일 결정점): JSON-shaped wire(react)는 `capabilities.supports_structured_output`, prefix_md(markdown)는 capability 무관 항상 `False`. 이전엔 provider 가 capability 와 wire 의 `skip_json_format` 을 직접 조합하다 prefix_md 에 JSON 강제 → omlx/mlx degenerate(`[2025]`/`[1000,1000]`)하는 버그가 있었음 (prefix_md 기본 전환이 노출 — bakeoff 는 provider 우회라 못 잡음). 이제 새 wire plugin 은 `provider_call_kwargs` 만 정의하면 provider 가 잘못 조합할 여지가 없음.
│
├── tools/                          도구 시스템

...
├── constants.py             (83)  공유 상수 (timeout, observation 템플릿, INTERRUPT_NOTICE, QUEUED_REQUEST_NOTICE[런 중 주입되는 큐 요청 앞에 붙어 "미완 이전 요청도 함께 처리" 지시 — v8.14.2]). 외부 모듈 의존 없음 — 저층 레이어. wire-format-specific 상수 (FORMAT_RULES, RETRY_HINT_*, SYSTEM_USER_PREFIXES) 는 ``wire_formats/`` 의 plugin이 소유

...
├── constants.py             (83)  공유 상수 (timeout, observation 템플릿, INTERRUPT_NOTICE, QUEUED_REQUEST_NOTICE[런 중 주입되는 큐 요청 앞에 붙어 "미완 이전 요청도 함께 처리" 지시 — v8.14.2]). 외부 모듈 의존 없음 — 저층 레이어. wire-format-specific 상수 (FORMAT_RULES, RETRY_HINT_*, SYSTEM_USER_PREFIXES) 는 ``wire_formats/`` 의 plugin이 소유 **v8.60.0**: STREAM_MAX_ATTEMPTS(총 시도 4)·STREAM_MAX_ATTEMPTS_MAX(10) — 구 STREAM_MAX_RECONNECTS 제거; 조기 종료 예산은 http.py 사유 상수(`_BROKEN_STREAM_ATTEMPTS`)
...
│   ├── core.py              (899) AgentLoop 오케스트레이터(라이프사이클·시그널·큐주입·_execute_turn 지휘 + 위임/property 표면). **큐 drain-all (v8.16.1)**: `_inject_queued_messages` 가 턴 경계에서 `while` 로 **큐를 전부 drain** — 대기 중 요청이 다음 턴 LLM 컨텍스트에 함께 들어간다(Claude Code 식 묶음 반영; 종전 v8.11 은 턴당 1건). FIFO 순서 보존, 라우팅 명령은 각각 실행. **주입 framing (v8.14.2)**: `QUEUED_REQUEST_NOTICE`(role=user)를 drain 배치의 첫 plain-chat 직전에 **한 번** 넣는다 — 첫 요청이 final 전에 다음 요청이 주입돼도 모델이 최신 것만 답하고 이전 요청을 흘리던 실측 드롭(세션 e35a) 방지. notice 는 INTERRUPT_NOTICE 동형(별도 user 턴, 원문 무오염, `all_system_user_prefixes` 로 resume 프리뷰서 필터). 시스템 프롬프트 Task Guidelines 지침으로도 보강. **wire_format None 폴백은 ctx-우선** (v5.19.0 G2 수리): `ctx.wire_format if ctx else 전역 기본` — 종전엔 무조건 전역 기본이라 비기본 포맷 세션의 서브에이전트가 ctx(히스토리 렌더)/loop(파서·프롬프트) split-brain (I2 불변식: cfg.wire_format ≡ ctx.wire_format)

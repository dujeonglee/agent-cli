...
│   └── react.py             (733)  ReActFormat — 유일 builtin plugin. ReAct-shape 문자열 + recovery wording + history pipeline 3 메서드 + 3-stage fallback parser (`parse_react`) + stage-2 JSON repair helper (`repair_json`) 모두 self-contained. parse_react는 ParsedAction을 직접 반환 (별도 dataclass 없음). `render_assistant_from_history`는 `tools/action_summary.summarize_action_args`를 직접 사용해 자연어 요약 생성. 폴더 째 삭제 가능 boundary — 외부에서 ReAct에 의존하는 코드 0건. (이전 EnvelopeFormat은 2026-05-10 측정 후 폐기 — Phase 1 bakeoff에서 mistral 0% / qwen thought 9.5%로 wire-shape 결정성 약점 확인)

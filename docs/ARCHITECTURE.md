...
│   ├── base.py              (140)  `Tool` ABC — schema(name/description/parameters) + dispatch(`_run`) + wire-key prefix(`key_prefix`/`strip_prefix`/`add_prefix`) + `claims`(prefix 매칭) + **`touched_paths`/`summary_arg`**(compaction 시 file-list 기여 + action 라벨 — 각 도구가 자기 prefix/배열 입력 셰이프를 알고 `strip_prefix`로 표준 키 읽음; base 기본=빈 list / 첫 string fallback, path·command·agent 도구가 override). **`wrap_single_op(flat)`** (멀티-op 3b): 멀티-op 포맷의 flat 단일-대상 op(`{"path": x}`)을 자기 캐노니컬 prefixed 입력(`{"read_file_reads": [{...}]}`)으로 재포장 — 기존 validate→strip→run 파이프라인을 무변경으로 재사용하는 전제. 기본=add_prefix(구조 변화 없음 — shell 등 flat 도구용); 배치 4개(read_file/edit_file/code_index/delegate)만 override, 이미-배치 입력은 idempotent 통과. 멀티-op 디스패치 경로에서만 호출(단수 포맷 우회). `run()`이 strip_prefix 후 `_run` 호출. 각 도구는 `name`만 정하면 prefix/strip/claims 자동
│   ├── virtual.py           (121)  가상 도구 Tool 서브클래스 (complete/ask/run_skill/ready_for_review) — loop이 인터셉트, **표준 키 유지** (prefix/추론 대상 아님)
│   ├── result.py            (15)   ToolResult 데이터클래스 (success, output, error, artifact)
│   ├── registry.py          (415)  12개 Tool 인스턴스 수집 → `TOOLS`(= `TOOL_SCHEMAS` alias), `_execute_tool`(tool.run), **`infer_action`**(action_input 키 prefix → 정확히 1개 도구가 claims 하면 복원, 0/2+는 None), `validate_tool_input`(3-tuple), `get_tool_descriptions(..., wire_format=None)` — **format-aware**: wire 가 `multi_op` 면 각 도구 description·param 키에서 자기 prefix(`{tool}_`)를 strip (flat `{action, params}` 컨벤션). **`_multi_op_flat_params`**: multi_op 일 때 배치 배열 param(`read_file_reads`/`code_index_queries`/`delegate_tasks`, edit_file 은 scalar `path` + `edit_file_edits`)을 **item-object 필드로 unwrap** 해 Input JSON 을 flat 단일-op 모양으로 노출 — `Tool.wrap_single_op` 의 flat→batch 매핑과 정확히 대칭. item 스키마는 schema 의 `items.properties` 에 이미 존재(per-tool 선언 불필요). **이걸 안 했더니 27B 가 광고된 `reads` 배열을 그대로 베껴 옛 `read_file_reads` wrapper 를 md_array 에서 emit 했음(DESIGN Exp 8 — root cause; inline 가이드만 flat 로 고치고 스키마 렌더를 안 고친 누락)**. **`_MULTI_OP_DESC_REWRITES`**: read_file/code_index description 의 배치 문장("Provide … as a list", "one or more files" 등)을 multi_op 에서 중립화(prefix-strip 후 적용; 테스트가 잔존 배치 표현 0 단언 → description 편집이 키를 outdate 하면 loud fail). `exposes_complete=False` 면 `_ALWAYS_INCLUDE` 의 `complete` 를 생략 + ready_for_review description 의 "BEFORE complete" 표현을 중립 re-phrase. 기본(None/단수 포맷)은 바이트-동일 (스냅샷 테스트 가드). **`render_param_value`**(JSON-Schema property → `Input JSON` 값: type + required 마커 + 중첩 `array<object{k1, k2?, ...}>` 항목 키. MCP adapter 와 공유 — 두 도구 표면 렌더 일관). tool 모듈을 import하므로 `detectors`는 `validate_tool_input`을 lazy import (순환 회피)
...
2. 런타임 API 감지 (OpenAI 호환 `_detect_openai_capabilities` — `/v1/models` 메타(`max_model_len`) + context overflow probe fallback + thinking 감지). 감지 시 `max_output = context // 4`; context < 16K(`MIN_CONTEXT_WINDOW`)면 `UnsupportedModelError`로 reject
...
| 도구 | 설명 | 필수 입력 | 출력 |
|------|------|----------|------|
| `read_file` | 파일 읽기 (hashline 포맷). `read_file_reads` 배열로 여러 파일/구간 배치 (단일도 1개 배열). 각 item 모드: `stat` (메타데이터 + 앞 20줄), `search` (정규식 grep), `line_start/line_end` (부분 범위), 또는 mode 없이 full read (크기 제한 없음). 부분 실패는 전부 실패할 때만 에러. | `read_file_reads[]` (item: `path` 필수, `line_start?`/`line_end?`/`search?`/`context?`/`stat?`) | `LINE#HASH:content` 형식. 다중은 파일별 헤더로 합성 |
...
agent-cli web [options]
  ...
  @<agent> <task>    에이전트에 위임
```
다중 뷰어 + first-come-keeps-control. 상세는 server.py / web.py 엔트리 참조.

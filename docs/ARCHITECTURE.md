...
├── directive_presets.py     (98)   **축별** DIRECTIVE 프리셋 라이브러리 — `~/.agent-cli/directive-presets/<axis>/<name>.md` 홈 스토어(axis∈`persona|task|learned`=`AXES`, **모든 방 공유**, 항상-적용 전역 `~/.agent-cli/DIRECTIVE.md` 와 분리된 "골라 쓰는" 라이브러리). **stateless**(매 op 파일시스템 hit). 모든 함수가 `axis` 를 선인자로: `save(axis,name,content)→id`(같은 이름 덮어씀)·`load(axis,id)→body|None`·`list_presets(axis)→[{id,label,source:user}]`(id 정렬)·`delete(axis,id)→bool`. `_safe_axis` 가 axis 를 고정 enum 으로 검증(URL 유래라도 서브디렉토리 탈출 불가), `_safe_name` 이 이름=파일명 stem 검증(한글·공백·유니코드 보존, `/`·`\`·`.`·`..`·leading-dot 거부 → traversal 차단, load/delete 에서도 재검증). `_presets_root()` 는 테스트가 tmp 로 monkeypatch(홈 미오염). web `GET/POST/{id}/apply/DELETE /api/directives/presets/library?axis=` 가 위임(POST 저장분=그 축의 zone body 만; 성격/업무/지침 세 축 동일 API). (Directive 학습 DESIGN §5.2)

...
│   ├── base.py              (140)  `Tool` ABC — schema(name/description/parameters) + dispatch(`_run`) + wire-key prefix(`key_prefix`/`strip_prefix`/`add_prefix`) + `claims`(prefix 매칭) + **`touched_paths`/`summary_arg`**(compaction 시 file-list 기여 + action 라벨 — 각 도구가 자기 prefix/배열 입력 셰이프를 알고 `strip_prefix`로 표준 키 읽음; base 기본=빈 list / 첫 string fallback, path·command·agent 도구가 override). **`wrap_single_op(flat)`** (멀티-op 3b): 멀티-op 포맷의 flat 단일-대상 op 을 자기 캐노니컬 입력으로 재포장 — 기존 validate→strip→run 파이프라인을 무변경으로 재사용하는 전제. 기본=add_prefix(구조 변화 없음 — shell 등 flat 도구용); **flat-native 도구(write_file/read_file/edit_file, consolidation Step 3)는 identity override**(스키마 자체가 flat 이라 재포장 불요); 아직 배치인 2개(code_index/delegate)만 `{...flat}`→`{tool_xxx:[{...}]}` 배열 override, 이미-배치 입력은 idempotent 통과. 멀티-op 디스패치 경로에서만 호출(단수 포맷 우회). `run()`이 strip_prefix 후 `_run` 호출. 각 도구는 `name`만 정하면 prefix/strip/claims 자동
...
│   ├── registry.py          (420)  12개 Tool 인스턴스 수집 → `TOOLS`(= `TOOL_SCHEMAS` alias), `_execute_tool`(tool.run), **`infer_action`**(action_input 키 prefix → 정확히 1개 도구가 claims 하면 복원, 0/2+는 None), `validate_tool_input`(3-tuple), `get_tool_descriptions(..., wire_format=None)` — **format-aware**: wire 가 `multi_op` 면 각 도구 description·param 키에서 자기 prefix(`{tool}_`)를 strip (flat `{action, params}` 컨벤션). **`_multi_op_flat_params`**: multi_op 일 때 배치 배열 param(아직 배치인 `code_index_queries`/`delegate_tasks`)을 **item-object 필드로 unwrap** 해 Input JSON 을 flat 단일-op 모양으로 노출 — `Tool.wrap_single_op` 의 flat→batch 매핑과 정확히 대칭. flat-native 도구(write_file/read_file/edit_file, Step 3)는 스키마가 이미 flat 이라 else-분기로 그대로 통과. item 스키마는 schema 의 `items.properties` 에 이미 존재(per-tool 선언 불필요). **이걸 안 했더니 27B 가 광고된 배열을 그대로 베껴 옛 wrapper 를 md_array 에서 emit 했음(DESIGN Exp 8 — root cause; inline 가이드만 flat 로 고치고 스키마 렌더를 안 고친 누락)**. **`_MULTI_OP_DESC_REWRITES`**: 아직 배치인 도구(code_index)의 description 배치 문장("Provide … as a list" 등)을 multi_op 에서 중립화(prefix-strip 후 적용; 테스트가 잔존 배치 표현 0 단언 → description 편집이 키를 outdate 하면 loud fail). read_file 은 flat-native 라 description 이 이미 단일파일 → rewrite 항목 제거. `exposes_complete=False` 면 `_ALWAYS_INCLUDE` 의 `complete` 를 생략 + ready_for_review description 의 "BEFORE complete" 표현을 중립 re-phrase. 기본(None/단수 포맷)은 바이트-동일 (스냅샷 테스트 가드). **`render_param_value`**(JSON-Schema property → `Input JSON` 값: type + required 마커 + 중첩 `array<object{k1, k2?, ...}>` 항목 키. MCP adapter 와 공유 — 두 도구 표면 렌더 일관). tool 모듈을 import하므로 `detectors`는 `validate_tool_input`을 lazy import (순환 회피)
...
│   ├── edit_file.py         (230)  파일 편집 (hashline + 퍼지 매칭 + colored diff). ops: replace / append / prepend / delete (delete = pos..end 범위 제거, lines 없음 = replace+lines=[] 의 명시 형태) → ToolResult. **flat-native (consolidation Step 3)**: `EditFileTool` 스키마 = flat 단일편집 `{path, op, pos, end?, lines?}` (required `path,op,pos`) — `edit_file_edits` 배치 배열·`edit_file_` prefix 제거, `wrap_single_op`=identity. 한 op=한 편집; 여러 편집은 멀티-op 으로 edit_file op 을 여러 개 emit(단 ref 는 line#hash 앵커라 줄 이동 후 stale → **같은 파일 다중편집은 턴 분리+re-read**, 다른 파일은 같은 턴 OK). (옛 멀티-편집 머신러리 — 중복 ref/range overlap 거부·bottom-up sort·edits 배열 순회 — 는 read_file `_format_batch` 와 함께 제거: op-배열이 배치를 대신.) `key_prefix` 유지(latent)
...
**한 op = 한 편집 (flat-native, Step 3).** edit_file 은 `{path, op, pos, end?, lines?}` 단일 편집을 받는다 — 옛 `edits[]` 배열과 그에 딸렸던 멀티-편집 안전장치(중복 ref 거부 Layer 1, 범위 겹침 거부 Layer 2, bottom-up 정렬)는 배열과 함께 제거됐다(op-배열이 배치를 대신, read_file `_format_batch` 제거와 동형). **같은 파일 다중편집 주의**: hashline ref 는 줄번호+해시 앵커라 앞 편집이 줄을 밀면 뒤 편집의 ref 가 stale(fuzzy 도 위치 기반이라 content 검색 안 함) → 같은 파일 다중편집은 **턴을 나눠 사이에 re-read**, 다른 파일은 같은 턴에 여러 op 로 OK. (멀티-편집을 원자적으로 묶던 배열은 'op 안에 배열 중첩이 27B 90% 깨뜨림' 실측(DESIGN Exp 8) 때문에 채택 안 함.)

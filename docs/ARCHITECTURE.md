...
- **컨텍스트 관리**: 매 LLM 호출 직전 `C×r−S−St (r=compaction_ratio 0.8 기본, St=세션상태; **max_output 예약 제거, 요청-시 클램프로 대체 — v8.53.0**)`(S=system 실측) 초과 시 LLM 요약 compaction (recursive single-call), 실패/재구성 후 미충족 시 FIFO drop으로 belt-and-braces fallback, 호출 직후 서버 실측 `usage`로 reconcile (flow 1 예방); 추정이 빗나가 서버가 400(prompt too long)을 던지면 `force_fit`으로 compact→FIFO 사후 축소 후 bounded 재시도 (flow 2 반응); history.jsonl 영속화 + compaction.json (resume용 dynamic_start_index)
...
├── main.py                  (2292) CLI 명령어: run, web, setup, sessions, update. **C4 공용 부트스트랩 (v4.46.0)**: `SessionBootstrap`(frozen — provider 6-튜플+wire format+예산) + `_bootstrap_provider`(셋업→fail-fast 포맷 해석(v5.19.0: `resolve_wire_format` 체인 — 명시 `--response-format`(default None=미지정 감지) > resume 세션 메타(`session_format=` kwarg) > models.json 모델 바인딩 > DEFAULT; web 의 대화형 최근-세션 resume 은 부트 후 결정이라 caller 가 기록 포맷으로 재해석+`dataclasses.replace`)→70% 예산 폴백) + `_load_resume_session`(핸드셰이크 전 fail-fast) + `_build_context`(ContextManager 단일 조립) 를 run/web 이 공유 — 세션 획득 UX(run=명시 --resume 만, web=+최근세션 프롬프트)·renderer·worker 만 커맨드별. 예산 폴백은 `compute_token_budget`=(context×7)//10 로 **통일**(이전엔 run/web 상이 — C4 감사 발견). **`run --resume <id>`**(v4.46.0): web 과 같은 on-disk 세션 로드 → 복원 컨텍스트 위에 QUERY 주입(run↔web 상호 이어가기). **web MCP 배선**(v4.46.0): `_setup_mcp`+TOOLS.update+worker `run_loop(ports=…)`+`capture_startup_system_prompt(mcp_manager=)`+shutdown `disconnect_all` — 이전엔 web 만 미배선. --style, --response-format, resume preview. **`update`**: `gh release view`로 최신 릴리스 태그 확인 → `_parse_version` 비교 → 새 버전이면 `gh release download`(wheel)+`pip install --upgrade`(`--check` 확인만, `-y` 확인생략). editable/dev 설치는 `_is_editable_install` 감지해 거부+`git pull` 안내(`--force` 우회). **`_prompt_model_capabilities` wire-format 질문 (바인딩 UX ②, v5.22.0)**: 대화형 모델 등록(감지 실패 폴백)에 "Wire format [auto]" 한 줄 — auto/빈 입력=필드 미기록(해석 체인 위임), `list_names()` 등록명만 수용(오타 재질문 — D2 조용한 폴백 금지), 선택 시 entry 에 `wire_format` 키 추가. **세션 표시 공유 헬퍼**: `_print_session(meta)` 가 id·시각 + `session_summary` 기반 `↳ 마지막 요청` / `→ 마지막 결과`(또는 `(in progress)`)를 한 블록으로 출력 — `sessions` 명령과 resume 프롬프트가 동일 포맷 공유. **`_maybe_resume_recent(workspace, response_format, prompt_fn)`**: `--resume` 없이 `web` 진입 시 가장 최근 세션을 보여주고 `[y/N]` 질의 — `y` 면 `load_session` 으로 이어가고(`is_resume=True`) 그 외엔 `create_session` 으로 새로 시작. `prompt_fn` 은 TTY 면 `input`, 비대화(파이프/cron)면 `None` → 묻지 않고 항상 새 세션 (안전 기본값). **`DispatchOutput` Protocol + `_ConsoleDispatchOutput` + `try_dispatch_agent_or_skill`** — `@<agent>`/`/<skill>` 접두사 처리 (listing, invocation, not-found) 공유 dispatcher. `run` 은 `_ConsoleDispatchOutput`(Rich 색상), `agent-cli web` worker 는 `web.server.WebDispatchOutput`(observation 이벤트) 어댑터 주입. unknown `@`/`/` 명령은 LLM으로 통과하지 않고 error observation 발사 (오타로 인한 사고성 LLM round-trip 방지). **`web` 명령**: `--resume <id>` 지원 — provider 핸드셰이크 전에 `load_session` pre-check로 unknown ID fail-fast, `ContextManager(resume=True)` 로 캐시 복구 후 `renderer.replay_from_history(ctx)` 한 번 호출해 persistent event buffer를 재구성 → 이후 새 SSE 연결의 snapshot replay로 이전 turn이 그대로 UI에 복원. **graceful shutdown**: `uvicorn.Server(config).run()` 직접 호출 + `KeyboardInterrupt` swallow + `finally` 블록에서 `renderer.shutdown_all_connections()` → `server.shutdown()` → `worker.join(timeout=2s)` → **`runtime.teardown_session`**(registry 전원 종료→스피너→MCP 해제→`finalize_session`) 순서로 정리 (lifespan shutdown 훅이 SSE generator를 먼저 닫아도 idempotent). **run/web 조립기 (v8.39.0 — runtime.py)**: registry 생성(`build_agent_registry`)+메일 배선(`wire_agent_mail`)+종료(`teardown_session`)가 run/web 공용 — run 은 전 종료 경로(skill 조기-반환·@명령·@agent·메인 펌프·예외)가 **단일 finally 의 `_finalize_run`** 으로 수렴(종전 경로별 나열에서 skill 경로 registry/MCP·@agent 경로 MCP 정리가 누락돼 있던 것 수리). **web 디스크 훅 배선 수리 (v8.39.0)**: web 도 `load_hooks()` 를 로드해 worker `run_loop(hooks_config=)`+registry runtime 에 전달 — 종전엔 web 채팅 턴·상주 에이전트에서 hooks.json 의 PreToolUse/PostToolUse 가 조용히 미발화(run 만 배선; /skill·@agent 디스패치는 자체 로드라 web 내부에서도 갈렸다). **부팅 노브 인자 (v8.60.0~v8.61.0)**: `--stall`/`--stall-attempts`(무진전 한도×시도)·`--compaction-ratio`·`--max-agents` 를 run·web 공통으로 — 우선순위 **CLI 인자 > env > 기본값**이고, 인자는 `_build_context`(ctx 3축)와 `build_agent_registry(max_agents=)`(registry 1축)로 갈라진다. None = 미지정이라 생성자가 env/기본값을 고른다(0 은 명시값 — compaction 은 clamp, max-agents 는 무제한, stall 은 감지 끔). 종전엔 ⏳·🧠 만 부팅 주입 수단이 있고 🗜️·👥 는 웹 노브 전용이라 headless·harbor 가 매번 기본값으로 시작하던 비대칭의 해소. **세션 노브는 전부 세션 한정** — 창을 닫아도(sticky 스냅샷) 유지되지만 프로세스 재시작으로는 복원되지 않고 `--resume` 도 복원하지 않는다(SessionMeta 4필드에 없음; ⚡ 자동 승인처럼 안전 게이트를 끄는 값이 재시작을 넘으면 위험). `_parse_stall` 은 `600`·`10m`·`0` 을 받고 오타는 `typer.BadParameter` 로 즉시 실패(헤드리스에서 조용한 폴백 금지).
├── runtime.py               (356)  **run/web 부트스트랩·teardown 조립기 (v8.39.0, 리뷰 §4.1 P1)**: `AgentRuntime`(frozen dataclass) — 상주/서브 에이전트 provider 배선 13키의 단일 정의(종전 run/web/tool_bridge 3벌 dict 리터럴, 키 구성도 상이했음; `as_dict()` 는 얕은 사상으로 객체 동일성 보존, `from_loop_config(cfg, provider)` 가 tool_bridge 경로) + `build_agent_registry`(생성+main 슬롯 등록) / `wire_agent_mail`(waker+회신 훅+restore/auto_spawn — 생성과 분리해 run 의 skill-dispatch 앞 registry 생성 순서 보존) + `teardown_session`(경고→registry 종료→스피너→MCP 해제→세션 저장 — 모든 run/web 종료 경로가 수렴하는 단일 소유자). 등가성 계약=tests/test_runtime_assembly.py (HEAD 3벌 dict 키·값 매핑 핀 + teardown 시퀀스 + CliRunner 실구동 경로 검증) + **루프 포트 빌더 다섯 (v9.14.0, docs/wiring §4.4)**: `ports_for_run`/`_web`/`_skill`/`_oneshot`/`_resident` — 조립 지점마다 흩어져 있던 포트 나열의 단일 정의. `LoopPorts` 는 **기본값이 없어** 포트를 빠뜨리면 생성 시 `TypeError`(배선 누락이 릴리스 둘을 깨뜨린 뒤 도입); `None` 으로 두려면 `unwired` 에 사유를 적어야 한다(패리티 계약=tests/test_loop_ports.py). 웹은 `dequeue_user_message`/`route_message` 가 반복마다 새 클로저라 **메시지마다** 포트를 새로 짓는다.
├── paths.py                 (~60) **`.agent-cli` 경로의 단일 소스 — 설정의 집은 하나 (v9.0.0, docs/config-scopes)**: `project_dir()`=`cwd/.agent-cli` · `user_dir()`=`~/.agent-cli` · `sessions_dir()`(v8.50.0, `AGENT_CLI_SESSIONS_DIR` 이전). **`scoped_paths()` 은퇴** — 종전(v8.40.0)엔 [프로젝트, 사용자] 쌍을 돌려주고 7개 모듈이 각자 병합(config 필드별·mcp 이름별·hooks 누적·DIRECTIVE 연결)했는데, "둘 다 읽고 프로젝트 우선"은 말하기는 쉬워도 어느 파일이 이겼는지 매번 따져야 했고 cwd 기준이라 하위 디렉토리에서 조용히 달라졌다. 이제 각 설정은 정확히 한 스코프: **유저** config.json·models.json(머신 사실) / **프로젝트** mcp·skills·agents·hooks·hooks.json·DIRECTIVE / **sessions_dir()** sessions·chat_history. 소비 모듈은 `project_dir() / "mcp.json"` 처럼 조립하되 종전대로 모듈-레벨 상수(import 시점 cwd 고정, 테스트 monkeypatch seam). 알고 받아들인 손실(개인 자산 축 없음·전역 정책 훅 없음·프로젝트 밖 실행은 빈껍데기·마이그레이션 경고 없음)은 DESIGN §3. 계약: `test_paths.py`(상수 단일 스코프 핀 + 홈 직접 조립 소스 가드 + 런타임 무시 검증) · `test_config_scopes_e2e.py`(**새 프로세스**에서 실제 HOME/cwd 로 import-시점 배선 검증 — 인프로세스 몽키패치로는 못 잡는 층)
...
├── memory.py                (249)  세션 메모리 스토어 — LLM 이 큐레이션한 **compaction-immune** salient 노트. `<session_dir>/memory.jsonl`(read_context history 와 같은 세션 디렉토리, resume 복원). **stateless**(매 op JSONL fresh read → resume 정합, 캐시 sync 없음). `add/get/update/delete/list_entries` + `render_index`(상시 `## Session Memory` 섹션 — 요약만, detail 제외 → 토큰 저렴 + recall) + `format_entry`(get 전체). type enum=`failure`⚠/`discovery`💡/`decision`🔀/`note`📝(검증·아이콘 단일 출처). id=max+1 monotonic(삭제해도 재사용 X). 인덱스 `_INDEX_CAP=30` 소프트 캡(초과분 "생략" 꼬리). `_io_lock` 로 read-modify-write 직렬화. `mark_memory_dirty`/`consume_memory_reload`(mutating op → 다음 턴 loop rebuild). **왜 별도 스토어**: 시스템 프롬프트는 `ctx.get_messages()` 밖이라 압축 대상 아님 — 롤링 컨텍스트에 넣으면 정작 필요할 때 요약/드롭됨 + 자기-출력 재공급 mimicry 위험(docs/session-memory/DESIGN.md).
├── config.py                (217)  config.json 3레이어 로딩 + models.json 레지스트리 **v9.5.0**: `_PROVIDER_FALLBACKS` 의 **모델 이름 추측 제거** — `gpt-4o`/`claude-sonnet-4-…` 는 로컬 서버(omlx·vLLM·LM Studio)에서 절대 맞을 수 없고, "모델을 안 골랐다"를 "없는 모델 404"로 바꿔 원인을 가렸다. 주소만 남는 `_PROVIDER_FALLBACK_URLS`; 모델은 빈 문자열 = "고른 적 없음". `save_config_value(key, value)` — 검증이 대화형으로 고른 모델을 그 자리에서 저장(다른 키는 읽어 되쓰므로 수동 편집분 보존).
├── model_check.py           (189)  **부팅 시 모델 검증 (v9.5.0, docs/model-resolution)**. 조회 결과를 **삼치**로 본다 — `목록O+이름O`→진행 / `목록O+이름X`→실패 / **`목록X`→경고 후 진행**. 마지막이 핵심: 구 `setup._list_models` 는 실패를 전부 `[]` 로 뭉개 오프라인·`/models` 미지원 서버를 "모델 0개"로 읽었고, 그대로 쓰면 부팅이 막혀 원래 고장보다 나쁜 회귀가 된다. `ModelListing(models, available, reason)` · `verify_model()` (`ModelNotFound`/`NoModelSelected`) · `suggest()`(difflib 오타 구제) · `is_interactive()`(보드가 띄운 인스턴스는 stdin 이 파이프라 물으면 멈춘다 — v9.1.0 `agent-cli mcp` 함정과 동형). `(base_url, provider)` 프로세스 캐시로 부팅+spawn 이 같은 목록을 재조회하지 않는다. 검증 지점 둘: `main._setup_provider`(모든 진입점의 단일 지점, provider 생성 전)와 `AgentRegistry.spawn`(역할 md 의 `model:` 이 상속을 덮었을 때만 — 안 보면 그 에이전트 **첫 턴**에 가서야 404 가 드러난다, 제보된 증상의 경로).
...
├── constants.py             (83)  공유 상수 (timeout, observation 템플릿, INTERRUPT_NOTICE). **`QUEUED_REQUEST_NOTICE` 는 v9.18.0 에 제거** — 꼬리의 `## Open Requests` + Task Guidelines 규칙과 같은 말을 하는 세 번째 사본인데 혼자 `ctx.add` 라 history 에 박혔고(그래서 프리뷰 필터가 필요했다), 위치도 꼬리보다 뒤였다(꼬리는 갓 주입된 요청 **뒤**에 붙는다). 강제도 조언이 아니라 독촉(v9.17.0)이 한다. 접두(`wire_formats._FORMAT_AGNOSTIC_USER_PREFIXES`)만 남긴다 — 9.17 이하 세션을 resume 하면 그 문구가 history 에 있다. 외부 모듈 의존 없음 — 저층 레이어. wire-format-specific 상수 (FORMAT_RULES, RETRY_HINT_*, SYSTEM_USER_PREFIXES) 는 ``wire_formats/`` 의 plugin이 소유 **v8.60.0**: STREAM_MAX_ATTEMPTS(총 시도 4)·STREAM_MAX_ATTEMPTS_MAX(10) — 구 STREAM_MAX_RECONNECTS 제거; 조기 종료 예산은 http.py 사유 상수(`_BROKEN_STREAM_ATTEMPTS`)
...
├── agent_icon.py           (143, v9.9.0)  상주 에이전트의 결정적 **시각 정체성** — `agent_icon(key)`(이모지 24종) + `agent_nickname(key)`("날쌘 여우" — 아이콘과 **같은 인덱스**의 동물 24 × 독립 해시의 수식어 16) + `agent_display_name(key, profile, name)`(구체적인 것부터: 인스턴스 이름 > 프로파일 > 별명). 프론트(`app.js` 의 `OV_AGENT_*`/`ovAgentHash`/`ovAgentIcon`/`ovAgentNickname`/`ovAgentDisplayName`)와 **풀·순서·해시가 동일**해야 한다 — `tests/test_app_markdown.py::TestAgentIconParity` 가 node 로 JS 를 실행해 값을 대조(아이콘·별명 2축 + 풀 길이/내용 일치). key 는 ASCII 라 `ord` == `charCodeAt`. **별명은 표시 전용**: 주소는 언제나 key(`@agt-<key>`, 도구 `{"key":…}`)라 화면은 이름 옆에 key 를 흐리게 병기한다(`.ag-key`, 채널 칩·ask 트레이). 별명을 주소로 받으면 유일성(같은 이름 둘)·경로(`agents/<key>/`)·rename 이 따라오는데 key 가 uuid 라 그 셋이 구조적으로 해결돼 있다. **순수 함수**(로스터 미참조) — `#N` 중복 회피를 넣으면 같은 key 가 접속마다 다른 이름이 되어 resume·재접속에서 얼굴이 바뀐다(결정성 > 중복 회피; 24×16 충돌은 곁의 key 가 가른다). 별명이 한국어인 것도 의도 — 사람 닉네임 풀(`render/web.py::_NICKNAMES`, "Funky Gecko")이 영어라 **사람과 에이전트가 한눈에 갈린다**(아이콘 접두에 더한 2차 구분).
...
│   ├── wf_recovery.py       (~108) WF-aware Intervention factory — `format_no_json_retry` (A1a), `format_no_action_retry` (A3). plugin의 framing/reminder/static fallback 사용. WF 의존이 한 파일에 모여 audit 용이. **`format_no_json_retry(syntax_error=…)`**: 옵셔널 구문 진단(`diagnose_syntax_error` 결과)을 framing 다음 줄에 끼워 *어디서* 깨졌는지 노출 + primitives 에 `diagnose_json_error` 추가. 미지정(기본 None)이면 메시지·primitives bit-for-bit 불변(기존 호출/테스트 보존).
│   ├── detectors.py         (~250) 감지기 모음. stateful: `ActionLoopDetector` (B1, turn 간 (action, args) 추적). stateless: `detect_unknown_tool` (A4), `detect_schema_mismatch` (A5, `validate_tool_input` wrap), `detect_nested_envelope` (A6, complete 결과의 이중 래핑 감지 — 관찰 전용).
│   ├── intervention.py      (~30)  `Intervention` dataclass — primitive 합성 결과 (message + 적용된 primitive 이름)
│   ├── observability.py     (196)  `TurnRecorder` — 세션별 `turns.jsonl` 추가-only writer; `TurnRecord` 스키마(model, timestamp, parse_stage, failure_signal, primitives_applied — timestamp 가 row 정렬; 구 `seq` 는 run-local 충돌로 제거 — **+ 턴별 토큰 사용량 4필드 (v8.49.0)**: input_tokens/output_tokens/cache_read_input_tokens/cache_creation_input_tokens 를 `TokenUsage` 에서 verbatim 복사(usage 없으면 0, 키는 항상 존재). 세션 비용·캐시 적중률의 **유일한 on-disk 원천** — 루프는 라이브 readout 용 출력 누계만 메모리에 두고, 합산은 소비자(KPI·보드·벤치 하니스)가 행을 더해 파생. 배관: core `_run_turn` 이 `response.usage` 를 `_handle_text_path(llm_text, usage)` 로 넘기고 dispatch 의 finally-record 가 실어 보냄 — 절단(`length`)·중단 턴은 종전과 같이 record 없음). FAILURE_* 라벨 8종 (NO_JSON / NO_OUTPUT / NO_ACTION / UNKNOWN_TOOL / SCHEMA_MISMATCH / NESTED_ENVELOPE / ACTION_LOOP / **DEGENERATE**=wire shape 반복 runaway, 라벨만·dispatch 진행)
│   └── primitives.py        (~109) format-agnostic 회복 primitive (`echo_prior_output`, `probe_progress`, `restate_task`) — provider/모델/채널/wire format 이름 모름. ReAct-shape constraint reminders는 ``ReActFormat`` 가 소유 **v9.23.2**: `echo_prior_output` 는 `bounded_excerpt` 로 앞뒤만 인용(`ECHO_MAX_CHARS`/`HEAD`/`TAIL`) — 전문 인용은 폭주 출력을 컨텍스트에 복제했다.
...
│   ├── ports.py             (81)   **LoopPorts (v9.14.0, docs/wiring §4)** — 루프에 주입되는 능력 포트 8개(questions·message_handler·agent_registry·monitor_registry·mcp_manager·hook_runner·route_message·dequeue_user_message) + `owner`("main"|"agent:<key>", 모니터 소유자 라우팅이 C2 에서 소비) + `unwired` 사유 맵. frozen·kw_only·**무기본값**. `handler_resources()` 가 `Tool.requires_handler` 뷰를 **파생**(종전 core.py 안 3키 dict 리터럴). `ctx`(다섯 호스트 전부가 넘기는 기반)와 `stop_event`(가변 LoopState+setter)는 포트가 **아니다**
│   ├── state.py             (103)  LoopConfig(frozen 불변 배선)·LoopState(공유 가변)·센티널(_CONTINUE/_RETRY/_NOT_HANDLED). **`LoopState.run_requests` (v9.15.0)**: 이 런이 응답해야 할 사용자 요청 `{id, author, text}` — 런 스타터 1건 + 턴 경계 drain N건. 웹 큐가 이미 `enqueue` 에서 id 를 발급하는데(`cancel_pending` 이 쓴다) `item.get("text")` 한 줄에서 버려지고 있었다. drain-all 이 요청 셋을 한 턴에 합치고 `complete` 하나가 나가면 **무엇이 답해졌는지 아무도 몰랐다**(`run_authors` 는 *누가* 물었는지만 안다) — 두 사람이 서로 다른 걸 물었는데 모델이 하나만 답해도 조용했다. `complete` 의 선택적 `answers:[id]` 가 주장을 받고, 주장 안 된 요청은 최종 답 말미에 붙는다. **회계지 강제가 아니다**: `complete` 을 붙잡지 않는다(붙잡으면 일을 시킨 쪽이 무관한 요청이 풀릴 때까지 결과를 못 받는다 — agent-ask 3판에서 한 번 틀렸던 자리). 주장의 정직성은 검증 불가(ask 와 동일 — asker 가 읽고 판단한다); 검출되는 것은 **아무도 주장하지 않은 요청**뿐이다. `answers` 부재 = 전부 주장(하위호환). 라우팅 명령·합성 wake 는 요청으로 안 센다. CLI 는 요청이 하나라 비어 있다. **모델이 id 를 보는 경로**(`constants.outstanding_requests_block` → `prompts/session_state.py` 의 **per-turn tail**, v9.16.0): `## Open Requests` 목록이 매 턴 마지막 메시지 꼬리에 실린다 — 요청이 하나여도 싣는다(모델이 id 어휘를 배울 기회가 실측 90% 의 단건 런에 있다). 드레인 직후 **한 번만** 주입하던 v9.15.0 은 실패했다: 주입은 턴이 자라면 뒤로 밀려 `complete` 을 부르는 순간과 멀어진다(라이브 cgyx7z — 목록을 보고도 `answers` 를 생략). 꼬리는 매 턴 다시 읽히고 persist 되지 않으며 KV 접두 캐시를 깨지 않는다. 이 한 줄이 없으면 `answers` 는 **도달 불가**다: 모델이 받는 것은 `[nickname]: text` 뿐이고 큐 id 는 루프 경계에서 죽는다. **되돌림**(`dispatch._require_answers`, v9.16.0 → v9.22.0 단건 포함): `## Open Requests` 가 있는 런에서 `answers` 없이 `complete` 하면 — 요청 수 무관 — **런당 한 번** 형식 교정으로 되돌린다(단건 면제는 message/ask 가 섞인 런에서 '결과가 곧 그 답' 이 거짓이라 제거) — 꼬리가 매 턴 요구해도 모델이 생략했다(라이브 xrnway — 생각에 id 까지 적고 필드는 비움). 붙잡기가 **아니다**: 기다리는 상대 없이 같은 산출물을 형식만 고쳐 다시 내라는 것이라 `complete` 은 여전히 지연되지 않는다. 끝내 생략하면 받아주고 각주로 적는다. **각주 세 축**(`dispatch._with_unanswered_notice`): 모델이 밝힌 미답 → `⏳ … were not answered`(건수 무관 — 단건 런의 `answers: []` 도 모델 자신의 진술이라 삼키지 않는다) / 생략 + 합쳐진 런 → `📋 … is undeclared`(생략은 '전부 답함' 도 '전부 미답' 도 아닌 **모름**이다 — 실측에서 둘 다 답했는데 '미답' 이라 단언해 거짓 보고가 났다) (되돌림 뒤에도 생략하면 건수 무관 `📋`). **미답은 런을 닫지 못한다**(`dispatch._settle_requests`/`_should_nag`/`_nag_open_requests`, v9.17.0): 주장된 id 는 `run_requests` 에서 **지워지고**(주장=닫힘), 남은 것이 있으면 최종답을 렌더·기록한 **뒤** `_CONTINUE` 를 돌려 루프를 이어간다 — 붙잡기가 아니다(답은 이미 나갔고 기다리는 사람이 없다). 종전엔 각주 한 줄로 끝났고 그 요청은 다음 런으로도 안 넘어갔다(런 수명 상태, 이월 없음) — 다음 런이 온다는 보장도 없어 세션이 유휴로 들어가면 사라졌다. 독촉은 **요청당 한 번**(`LoopState.requests_nagged`) — 상한이 없으면 고집 센 모델과 물려 런을 태운다. 생략에는 독촉하지 않는다(무엇이 남았는지 모른다). **주장은 history 에 남는다**(`wire_format.serialize_terminal_for_history(answers=…)`, v9.17.0): 이 레코드는 `(thought, result)` 로 **재구성**되던 것이라 `answers` 가 통째로 버려졌고, 인스펙터·resume·감사 어디서도 무엇을 답했다고 했는지 볼 수 없었다(라이브 조사가 그 재구성본을 '모델이 안 보냈다' 는 증거로 읽었다). 부재와 빈 주장은 구분해 저장한다(모름 vs. '아무것도 안 답함'). **최종답이 무엇에 대한 답인지**(v9.19.0): `_op_complete` 이 `_settle_requests` **전에** 주장분을 뽑아 `render_step("final", …, requests=[{id,author,text}])` 로 넘긴다(정산이 지운 뒤엔 알 수 없다). 웹은 `assistant_turn.requests` additive — 이벤트의 `answers`(닉네임 귀속)와 **다른 필드**다(이름을 겹치면 `complete` 의 `answers`(요청 id)까지 셋이 된다). resume 은 user 레코드의 **`request_id`**(`_add_user_message`, v9.19.0 — 회계의 빠져 있던 연결고리: id 가 `run_requests` 까지만 가고 디스크엔 안 찍혀 어떤 것도 대조할 수 없었다)로 `_replay_requests` 맵을 쌓고 터미널 레코드의 `answers` id 를 풀어 같은 칩을 낸다(`_replay_authors` 와 같은 모양). v9.16 이하 세션은 `answers` 가 없어 칩도 없다 — 위치로 추측하지 않는다
│   ├── prompt.py             (79)  SystemPromptSvc — 시스템 프롬프트 소유(섹션 리스트가 단일 진실, system 은 join 파생)
│   ├── tool_bridge.py       (601)  ToolBridge(hooks·invoke·RunContext·관찰 seam) + _normalize_input(소비자가 bridge 뿐이라 이주)
│   ├── llm.py               (361)  LLMCaller(`_decoding_grammar` v9.24.0 — 콜마다 `CallSettings.grammar` 채움[think 열림/닫힘 두 변형 캐시], 인스펙터 스냅샷엔 섹션이 아니라 `render_system_prompt_snapshot(..., grammar=(thinking_open, ebnf))` 별도 인자로 — 프롬프트 토큰 합계에 안 섞임[서버가 `kind=grammar` 섹션으로 목록 끝에 붙임], 턴 통계에 `grammar` 플래그; `_llm_compact_summarize` 는 문법 없음 — 요약은 산문; _call_llm·압축요약콜) + _build_token_stats + _MAX_OVERFLOW_RETRIES. 압축 목표 비율은 상수 대신 `self.ctx.compaction_ratio`(라이브, web 슬라이더 반영)를 매 콜 읽음(flow1 예방 target·flow2 overflow retry 양쪽, 5.14)
│   ├── dispatch.py          (~1314) `BATCH_OP_SKIPPED_NOTE`(v9.24.2 — 배치 op 가 실행 전에 거부될 때[모르는 도구·스키마 불일치·반복 호출] 셋이 쓰는 한 문장, 원인과 다른 줄: "다른 op 는 그래도 시도됐다 — 각자 결과를 보라"; 종전 "already ran" 은 전부 실패한 배치에서도 그렇게 말했다) · TurnDispatcher(+_op_* 4헬퍼) + 관찰조립·ask 자유함수군(_append_observation/_handle_ask/_extract_questions/_try_echo_as_final/…). **산문-only 종결 제거 (v8.4.0)**: v7.14 의 `prose_completion`(action-less 순수 산문 → synthetic complete) 은 프로덕션 반례(plan 스킬이 전환 서술 "Now let me write the plan document:" 35자를 결과로 조기 종료 — turns.jsonl `primitives_applied=["prose_complete"]` 로 확정)로 제거. 완료는 도구-입력 의미론이고 의미론은 엄격(v8.0.0 선) — action-less 턴은 parse_stage 불문 전부 NO_ACTION 넛지로 가고, 넛지 문구(`constraint_reminder_action_required`, 양 포맷 self-contained)가 '산문이 답이었으면 complete result 로 재방출'을 직접 안내해 1턴 수렴. 계약=test_multi_op.TestProseRequiresExplicitComplete(6, 실루프 양 포맷) 뮤테이션 2/2. **foreign-format 구제 (Phase 3, v5.21.0)**: parse 직후 0-op 이면 `wire_formats.try_foreign_parse` 1회 — 성공 시 그 turn 으로 정상 진행, 라벨 `FAILURE_FOREIGN_FORMAT`+primitives `foreign_parse:<src>`, 직렬화는 `outcome["corrected_record"]`(ops shape) 경유(단일-op `_dispatch_op` corrected 우선순위: foreign>inference, 멀티-op `_flush_op_results(corrected_record=)` 스레딩) → prior 는 바인딩 포맷 캐노니컬 재렌더 = 누출 raw 재공급 없는 자기 교정. **개입 통일 (v8.37.0)**: 5개 개입 블록(A7 NO_THOUGHT[v9.23.3 제거]/B1 action-loop/A4 unknown/A5 schema/`_recover_unparsed`)이 `_intervene()` 단일 헬퍼로 — render_recovery→관찰 append(render=False)→outcome 기록→**턴 비계수**(통일 규칙: 개입은 도구 미실행 회복 넛지라 max_turns 예산 미소모; 종전 A4/A5 만 계수하던 비일관 제거, 반복 폭주는 B1 detector level≥3 하드페일이 상한). recovery_kind: format 4곳 fold 대상, B1 은 빈 값(파싱 성공≠루프 해소라 비fold). **재시도는 기록하지 않는다 — 형식 경로로 확장 (v9.23.2)**: NO_ACTION/NO_JSON(`_recover_unparsed`)·단일 op A4/A5·출력 길이 절단(`core._on_output_truncated`) 이 `store_emission=False` — 실패 원문은 저장하지 않고 넛지가 `echo_prior_output` 로 **앞뒤만**(`bounded_excerpt`: 2000자 이하 전문, 넘으면 머리 1400 + `[… N characters omitted …]` + 꼬리 600) 인용한다. 종전엔 저장 + 전문 인용이라 폭주 한 번이 두 벌로 쌓였고, 재시도가 계속 실패하면 fold 기회도 없었다(Harbor extract-elf: 32K자 폭주 → 입력 +2만 토큰 → 모델이 깨진 조각을 22턴 흉내). 길이 절단 경로는 종전에 fold 대상도 아니어서 최대 32K 토큰이 영구히 남았다. 저장이 맞는 경로(수락된 complete 뒤 독촉, 배치의 부분 실패, 도구 실패·거부)는 그대로. **B1 로 확장 (v9.23.4)**: 행동 루프(같은 도구+같은 인자 연속) 넛지도 막힌 호출을 저장하지 않는다(`store_emission=False` — 넛지 문구가 `shell({...}) N times` 로 무엇을 반복했는지 이미 말하고, B1 은 fold 대상이 아니라 종전엔 실행 안 된 호출이 영구히 쌓였다). 배치 안에서 걸리면 A4/A5 와 같이 그 op 만 `FAILED … This op did NOT run` 으로 적고 나머지 op 를 실행한다 — 종전엔 단일 op 전용 `_intervene` 을 불러 뒤의 새 op 들까지 `aborted` 로 취소했다. **parallel_safe 트랩 봉인 (v8.37.0)**: 배치 수집이 `_PARALLEL_BATCH_ENGINES`(현재 {agent}) 게이트 — 엔진 미배선 parallel_safe 도구는 묶지 않고 순차 per-op 로(종전 NotImplementedError 런 크래시). **정책 속성 파생 (v8.38.0)**: 턴-종결 flush 분기는 `Tool.terminal`(complete/run_skill), core 의 tools_list 구성은 `depth_gated`/`requires_handler`/`force_mount` 속성에서 파생 — 도구명 문자열 하드코딩 대체(T3 선언화), 등가성은 60조합 매트릭스 테스트로 고정
...
│       ├── index.html       (228)  단일 HTML 셸 — `<head>` 의 인라인 테마 스크립트(첫 페인트 전 `data-theme` 설정 = localStorage `agentcli_theme` 의 테마 id, 미지정 시 amber[시스템 light 선호면 light] — FOUC 방지), **칩 헤더(v7.1.0)**: 좌측 의미 칩 3개 — 모델 `#info`(max 28ch ellipsis, hover=provider·전체명) / ctx 게이지 칩 `#chip-ctx`(`#ctx-gauge`+`#ctx-pct`; 클릭 → `#ctx-popover` 팝오버에 기존 `#token-usage`·`#compaction-wrap`·`#maxagents-wrap` 이 **id 보존 이동** — IIFE·테스트 계약 무변경) / 워크스페이스 칩 `#chip-ws`(`📋 …/마지막 2세그먼트`, 내부 `.hd-chip-text` 36ch ellipsis[flex 컨테이너엔 ellipsis 미적용이라 텍스트 span 이 담당], **칩 자체가 클릭=경로 복사** — 별도 버튼은 칩 높이보다 커 세로 클리핑됐던 실피드백 수리), 우측 아이콘(🎨 테마 피커[`#theme-wrap` > 버튼 + `#theme-menu` 드롭다운] / ⚡ Inspector / 📤 Export / 📁 Files 버튼 — 공통 borderless-emoji 스타일 + 접속자 로스터 옆 ✎ `#rename-btn` 닉네임 재설정 버튼[기본 hidden, 로스터 합류 시 노출]) / name-bar(첫 접속·✎ 공용 닉네임 입력바) / messages / export-bar(선택 모드 액션바 — Jira 폼에 `#export-jira-http-warn` 평문 경고 span 포함) / download-drawer(우측: 파일트리·count·⬇ zip + 업로드 드롭존) / footer + textarea. JS는 부트스트랩 `?token=` 을 주소창에서 strip 후 쿠키로 인증(요청 URL 에 토큰 미부착), SSE 연결. **헤더 개편(v8.57.0)**: 구 ctx 게이지 칩·팝오버(`#chip-ctx`/`#ctx-popover`) 폐기 → 토큰 상세 `#token-usage`(`#tok-base`+`#tok-think`) 헤더 상시 노출, 세션 노브 4종은 값-배지 칩(`.knob` + `.knob-btn` + `data-pop` 팝업; compaction/maxagents/stall/thinking, 내부 입력 id 보존) — **v8.60.0**: ⏳ stall 칩이 **두 입력**(`#stall-input` 한도 + `#stall-attempts` 총 시도)과 파생값 줄(`#stall-derived` = 한도×시도 최대 대기)을 한 팝업에 담는다(🧠 의 2입력 선례). 칩을 늘리지 않은 건 레이아웃 판단 — 헤더는 이미 4칩+토큰으로 빠듯하고, 두 값은 따로가 아니라 곱으로만 의미가 있다. 배지는 `10m×4`(곱만 보이면 어느 축을 고칠지 모르고, 한 축만 보이면 최대 대기를 암산해야 한다), 한도 0 이면 `off`+시도 입력 비활성, 좁은 창은 flex-wrap 자동 줄바꿈으로 모든 칩 상시 노출(보따리 없음). **v9.2.0 🔌 칩** `#mcp-wrap`(5번째 노브, 🧠 뒤): 읽기 전용 — 팝업에 입력 요소 없음(테스트 핀), 배지 `붙은/전체`, 등록 서버 없으면 hidden.
│       ├── app.js           (4274) `obsSummary(content, failed)`(v9.24.2 — 성공은 마지막 줄, 실패는 원인 줄[배치면 첫 FAILED op 의 다음 줄]; 종전엔 실패도 끝 줄이라 `} This op did NOT run` 이 떴다) · 📐 문법 칩 IIFE(v9.24.0 — 배지 강제/끔/미지원/확인 중, sticky 동기화) · 생성 중 줄·토큰 줄의 📐 · 인스펙터 `INSP_KINDS`(system/dynamic/tail/grammar 그룹 제목 — grammar 는 총합 막대 없이 `insp-tag` 생각 열림/닫힘); SSE 이벤트 디스패치 + DOM 렌더링. **런 블록 (v9.23.0, docs/chat-ui §12)**: 상주 스코프(`ctx_dir: agents/<key>`)의 `scope_start` 가 `.card-run`(머리 `run-head`·몸통 `run-body`·꼬리) 을 채널 루트에 열고(`ensureRunBlock`), 그 `task_id` 의 카드는 `appendToTimeline` 이 몸통으로; `scope_end` → `closeRunBlock`(최종답 카드가 꼬리 `run-final`+`run-meta`, 없으면 `run-foot`). `agent_msg`: in → `runBySeq[key][seq]` 의 블록 머리(`runItem`) 또는 `⏳ 대기`(`queuedIn`, `scope_start.seqs` 로 회수 — 구 세션은 `[index+1]`+직전 블록 폴백); out → `fallback` 만 `⚠ 폴백` 줄(`pendingFallback` 로 순서 무관); question → 트레이 갱신만. 수신 줄·블록은 `d.task_id`(보낸 쪽 런) 가 아니라 `key` 로 루트에. 발신 도구(`SEND_TOOLS`)는 아이콘 → · 꼬리 `sendPeerChip`(글자, 점프 없음). 삭제: `ovRenderAgentMsg`/`ovPeerInfo`/`lastFinalByChannel`/영수증/`.card-msg`/peer 점프. kill(`ovOnAgentCleared`)은 `agent_msg` 에서 나온 것(머리 `run-item`·`card-fallback`·`card-queued`)만 지운다 — 블록은 서버 버퍼에 남는 스코프·턴 이벤트라 지우면 resume 재생 수신 줄이 붙을 곳이 없다(v9.23.1). **`renderEmphasis` CommonMark flanking (v9.22.3)**: 여는 `*`/`**` 뒤·닫는 것 앞에 공백이면 강조가 아니다 — 종전엔 사용자 말풍선의 cron 식 `0 9 * * 1-5` 가 `<em> </em>` 으로 별표를 잃었다(어시스턴트 텍스트도 동일 파이프라인 `escapeAndFormat`). **tab-presence 비콘 (v7.3.0, v7.7.0 에서 유일한 연결-가드 표면으로 단순화)**: EOF IIFE 가 BroadcastChannel `agentcli_tab_presence` 의 ping 에 `{pong, nonce, path}` 응답 — board 열기 게이트(항상 board 경유 운용 전제)의 카운트·재사용 판정 데이터 소스. 페이지 자체 입장 게이트(v7.5 파킹/v7.6 Web Locks)는 제거 — Web Locks 는 secure context 전용이라 LAN http 에서 무동작이었고, URL 직접 진입은 운용 정책상 범위 밖(세션 복원 잔여 리스크는 문서화 수용). **confirm 무응답 가시화 (v7.2.0 ⓔ)**: `submitConfirm` 이 `CONFIRM_STALL_MS`(3s) 타이머를 걸고 미해결이면 버튼 줄에 `#confirm-stall` 경고(연결 정체 안내) 표시 — 브라우저 origin 당 6연결 고갈 시 클릭이 조용히 브라우저 큐에 갇히던 실사고의 가시화; `setInputMode` 전환 시 타이머·경고 정리. postInput 409(이미 해결된 프롬프트) 수신 시 confirm/ANSWERING stale UI 를 chat 모드로 접음. **ctx 칩/팝오버(v7.1.0)**: `token_usage` 핸들러가 `#ctx-pct`(%)·`#ctx-gauge-fill`(width) 갱신+칩 노출, EOF 의 팝오버 IIFE 가 theme-menu 패턴(토글·외부클릭/Esc 닫기·내부클릭 stopPropagation=슬라이더 조작 보호·aria-expanded)으로 `#ctx-popover` 개폐; ready 핸들러가 ws 칩(마지막 2세그먼트+클릭 복사 📋→✓ 플래시) 배선. event_buffer (snapshot) replay → live. 카드 종류: user_message (우측 파란 bubble), assistant_turn (thought + final OR action), observation (✓/✗ + tool_name), error, streaming (점선, 토큰 누적). prune 이벤트 시 가장 오래된 N개 카드 DOM에서 제거. input mode 3개 (chat / prompt / confirm). confirm 모드는 ConfirmOption.label 버튼 + 코멘트 텍스트. confirm/ask에 `input_required.agent/reasoning/action` 가 있으면 `buildPromptMetaEl`이 `↳ from <agent> · 💭 reasoning · ⚡ action`(.prompt-meta) 블록을 버튼/답변영역 위에 렌더 — 서브에이전트 출처 표시. **identity/roster**: `identity` 이벤트로 자기 conn_id 수신(접속자 로스터 "(you)"·큐 소유). 모든 연결 동등하게 입력 가능. 입력 POST 에 `conn_id` 동봉(큐 소유 식별). **닉네임 입력**: 접속 시 `#name-bar`(기본값 채워진 입력)로 이름 설정(localStorage 기억; 저장값 있으면 자동 적용·바 미표시) → POST /api/nickname. **닉네임 중간 변경(✎)**: `openNameBar(current)` 공유 헬퍼가 first-connect 프롬프트와 ✎ 진입점 양쪽에서 name-bar 를 prefill·포커스 재노출 — 헤더 `#rename-btn`(✎)은 `viewers` 이벤트에서 내가 로스터에 있을 때만 노출(`myNickname` 도 갱신해 prefill), 클릭 시 현재 닉네임으로 바 재오픈 → 기존 `applyNickname` 경로 재사용(POST + localStorage 갱신 + 바 숨김). 백엔드 무변경(`set_nickname` 이 로스터 재브로드캐스트, ephemeral·미영속). **메시지 큐 UI**: `queue` 이벤트 → `#queue-list` 에 대기 메시지(닉네임)·자기 항목 ✕(POST /api/queue/cancel). send 는 항상 큐잉(busy 면 대기); Stop 은 별도 `#chat-stop` 버튼(busy 시 노출, POST /api/stop). **markdown 헬퍼 (`escapeAndFormat` → `extractCodeFences` → `markdownInline` → `restoreCodeFences`)** — 의존성 0의 자체 미니 파서: 헤더(`#`/`##`/`###` → `<h1>`/`<h2>`/`<h3>`), GFM 파이프 표(헤더 행 + `---` separator + body), 순서/비순서 리스트 (`-`/`*`/`1.` 연속 라인 ↔ `<ul>`/`<ol>`), `**bold**`/`*italic*`, 인라인 코드, 펜스 코드(```` ``` ````). **XSS 안전(NFR-MD-2)**: `escapeHtml`이 가장 먼저 실행되어 `<`를 `&lt;`로 치환, 펜스를 placeholder로 빼낸 후 markdown 패스를 stripped body에 적용, 마지막에 pre-rendered `<pre><code>`로 복원 — markdown 패스가 사용자 입력 HTML을 실행 가능 토큰으로 되돌릴 경로가 없음. write_file/edit_file 의 plain diff 는 `colorizeDiffBody` 가 observation 본문에서 `--- a/` 블록 이후 라인을 첫 char 별 `rich-*` span 으로 색상 (diff 데이터는 plain — 색상은 렌더 시점). **`failed_turn` 핸들러**는 `finalizeStreamingAsFailed`로 live streaming 카드를 `.card-failed`로 마감(제거 X)+streamingText 리셋 → 잘못된 응답 / intervention(observation) / 재발화가 **3개 카드로 분리**(이전엔 하나의 카드에 재발화까지 누적되다 정상 응답에서야 교체). **Export 기능(별도 IIFE — main 렌더 루프 무수정)**: 📤 버튼이 선택 모드 토글 → `#messages > .card` 를 class 로 분류(user/assistant/observation/error/agent; streaming·failed 제외)해 per-card 체크박스 부착(MutationObserver 로 실행 중 도착 카드도). 선택 entry(`{kind,label,body=innerText,mono}`)를 `POST /api/export/html`(Blob 다운로드) 또는 `POST /api/export/jira`(인스턴스 드롭다운은 `GET …/targets`로 채움[deployment 포함, 0개여도 폼 표시=zero-config] + **편집 가능한 base_url 필드**[config target 선택 시 prefill, 직접 타이핑 가능, localStorage `agentcli_jira_url` 마지막 URL 기억; `updateJiraHttpWarn` 가 `input`/`change` 마다 URL 이 `http://` 면 `#export-jira-http-warn` 평문 경고를 인라인 표시(차단 아님, https/빈값이면 숨김)] + Cloud/Server 토글 + 본인 계정·토큰[localStorage `agentcli_jira_cred_{base_url}` — URL 별 prefill] + issue key 폼, body 에 `base_url`+`auth:{user,secret}`+deployment 동봉)로 전송. Inspector 와 동일한 "헤더 버튼 → 별도 IIFE" 패턴. **v8.13.2**: 선택 체크박스는 `#messages` 카드에 붙는데 개요/흐름 모드에선 `#messages` 가 닫힌 전문 드로어 안이라 안 보인다 → `enter()` 가 `window.__showTimeline()`(메인 IIFE 훅 = `setViewMode('detail')`)을 호출해 타임라인을 띄운 뒤 체크박스를 붙인다('export 눌러도 뭘 고를지 안 나옴' 수리). **Download 기능(별도 IIFE)**: 📥 버튼이 우측 드로어 토글 → `GET /api/workspace/tree`로 루트 목록을 받아 lazy 트리 렌더(디렉토리 ▶ 클릭 시 하위 fetch·펼침, 노드별 체크박스, 파일·디렉토리 size 표시). 선택 경로(또는 All) → `POST /api/workspace/download` → 응답 zip Blob 을 anchor click 으로 저장. **압축 슬라이더(별도 IIFE, 5.14)**: `#compaction-range` — 시작 시 `GET /api/compaction` 으로 현재값+범위(%로 표시) 로드 후 노출, `change` 에 `POST /api/compaction`(clamp 결과 되반영), `agentcli:compaction`(sticky 중계)로 타 뷰어 동기화. 메인 렌더 루프 무수정. **테마 피커(별도 IIFE)**: 🎨 `#theme-btn` 이 `#theme-menu` 드롭다운을 토글 — 5개 테마 목록(`THEMES` = id/name/swatch[bg+accent] 단일 출처)에서 항목별 스와치+이름+현재 ✓ 를 렌더, 클릭 시 `<html data-theme>` 설정 + localStorage `agentcli_theme` 저장 + 메뉴 닫기(외부 클릭/Esc 도 닫음). 기본 amber. 초기 테마는 `<head>` 인라인 스크립트가 이미 설정(FOUC 0); CSS 토큰만 갈리고 렌더 로직 무관 — 메인 IIFE 와 격리. **Directives 드로어(Inspector IIFE, 5.4.0 스코프 탭)**: 📝 에디터 = 청중 탭 3개(공통/Main/서브에이전트 — 파일의 U-C 스코프와 1:1, 내용 있는 탭 ● 뱃지) + 탭별 버퍼(`dirBuffers`, 전환 시 sync) + ✨ 생성 입력줄(`generateDirective`→`POST …/generate`, 진행 중 버튼 잠금, 결과는 활성 탭에 미저장 반영) + 저장(`{scopes}` POST — 서버 조립)/취소(`dirDirty` 가드). 구 3축 행·프리셋 드롭다운·💾 저장 모달은 폐지. **레벨 컨트롤 + 개요 뷰 (v8.12.0, presentation-only)**: 헤더 세그먼트 `개요·흐름·전문`(`#vt-overview`/`#vt-flow`/`#vt-detail-toggle`, role=tab·aria-selected). **v8.14.0 — 전문 = base 위 사이드 오버레이(배타 탭 아님)**: `setBaseView(mode)`(개요↔흐름 base 전환: `#overview` hidden·`#team-view` display·`body.mode-overview`)와 `setDrawer(open)`(전문 드로어 열기: `.open`+`body.drawer-open`)를 분리하고 `syncTabs()`가 탭 상태를 반영(개요/흐름=배타 base, 전문=드로어 열림 반영이라 base 와 동시 활성 가능). 개요·흐름 모두 `body.drawer-open` 시 `margin-right:var(--drawer-w)` 로 드로어 폭만큼 양보해 **겹침 없이 나란히**(요약/스윔레인 + 상세 대화 동시 확인; `--drawer-w`=드로어 폭·base margin 단일 출처). 흐름(v8.14.1)은 `#team-view` 의 ResizeObserver 가 폭 변화 시 SVG width 를 재계산하므로 margin 만으로 스윔레인이 **압축**된다(가로 스크롤/오버레이 아님; transition 은 생략해 슬라이드 중 rAF 반복 리렌더 회피). `setViewMode(mode)`는 하위호환 진입점(`'detail'`→`setDrawer(true)`, 그 외→`setBaseView`); `#vt-detail-toggle`=`setDrawer(!drawerOpen)` 토글, `#td-close`=`setDrawer(false)`(base 유지). Export IIFE 의 `window.__showTimeline`도 `setDrawer(true)`. **개요 외과적 갱신(v8.47.0 — 실사고)**: 병합만으로는 부족했다. `ovRender` 는 `$overview.innerHTML` 를 통째로 갈아끼우는데 `ovOnStream`(청크마다)·`ovOnAction`(툴콜마다)이 그걸 호출해, 실제로 바뀌는 건 맨 아래 활동 스트립 한 줄뿐인데도 초당 ~60회 `.ov-block` 이 파괴·재생성됐다. 결과로 **[⧉ 복사]/[▤ 전체 대화] 버튼이 클릭 불가** — click 은 mousedown 과 mouseup 이 같은 엘리먼트에 떨어져야 발생하므로 그 사이에 노드가 사라지면 브라우저가 click 을 아예 만들지 않는다(그래서 `$overview` 위임으로도 못 살린다). 덤으로 `.ov-pulse`(1.1s 무한 CSS 애니메이션)가 매 프레임 t=0 에서 재시작해 멎어 보였고, `<details>` 열림·텍스트 선택·`:hover` 도 매 프레임 리셋됐다. 수리: `ovActHtml` 에서 본문(`.ov-act-body`)을 분리하고 스트립만 패치하는 `ovRenderAct`/`scheduleOvActRender` 를 신설, 그 두 이벤트만 그 경로로 보낸다(나머지 9개 호출부는 실제로 목록이 바뀌므로 전체 렌더 유지). 스트립 첫 등장 시 플레이스홀더를 걷어야 하는 경우만 전체 렌더에 위임. 계약은 `tests/browser/test_overview_activity.py`(구버전 app.js 로 되돌리면 실패함을 확인) + `test_web_server.TestRenderCoalescing` 가 잠근다. **렌더 병합(v8.42.0 — 리뷰 §4.6 효율)**: 이벤트 훅 경로의 `ovRender`/`scrollToBottom` 은 `scheduleOvRender`/`scheduleScroll`(rAF 당 1회 병합) 경유 — 스냅샷 재생·스트리밍의 동기 이벤트 폭주에서 개요 전체 재렌더(호출×14항목 마크다운)와 scrollTop 쓰기(전체 레이아웃 강제)가 O(N²) 였던 것 수리(888이벤트 스냅샷 실측: 블로킹 1012ms→277ms, ovRender 585ms→20ms; 4x 스로틀 ~4.3s→~1s). 직접 호출은 사용자-조작 경로(setViewMode·ovSetChannel)만 — 계약=test_web_server.TestRenderCoalescing. **개요 렌더 모듈(`ov*`, 기존 이벤트 재사용·서버 무변경) — 플랫 로그 모델(v8.15.0)**: 종전의 상태적 짝짓기(`ovDone`/`ovLive`/`ovPending`)를 폐기하고 **append-only 플랫 로그 `ovEntries`**(`{kind:'user'…}` | `{kind:'resp'…}`)로 교체. 훅은 전부 append 만: `user_message`→user 엔트리(`ovOnUserMsg`, `^[닉]:` 접두 제거), `stream_chunk`(main=task_id 없음)→gen resp 엔트리 스트리밍(`ovOnStream`, 참조 `ovGen`), main `assistant_turn{final}`→resp 확정(`ovOnFinal`, reasoning=`d.thought`), **top-level(depth0) 스코프 `assistant_turn{final,task_id}`→dispatched resp 엔트리(`ovOnScopedFinal`, `ovTopScopes` 로 depth0 만; 모델 B — 직접 `/skill`·`@agent` 결과를 요약에 표시)**, `scope_start`(depth0→`ovTopScopes`, skill→ambient), `token_usage`→ctx%, `agent_roster`→dot. **렌더는 항목별 독립(v8.15.0, 그룹핑 폐기)**: `ovRender` 가 `ovEntries` 를 순회하며 user=`ovUserHtml`(독립 평문 줄)·resp=`ovRespHtml`(독립 블록)로 그린다 — **그룹핑도 "↳ 요청자" 귀속도 없다**(짝지어 보이던 원인 제거; 사용자 리뷰 반영). 마지막 resp = hero(accent). 도착 순서 그대로의 순수 로그라 "응답 대기" 정체·쿼리↔응답 오귀속이 구조적으로 불가능. **회신 주체 배지(v8.23.0)**: resp 엔트리에 `source`(main final=`"main"`, scoped final=`ovScopeSrc[task_id]`=scope_start 의 `d.agent`/skill 이름)를 달고 `ovRespHtml` 이 카드 헤더에 `.ov-src`(main=muted·agent=accent) 배지로 렌더 — 다중 에이전트 시나리오에서 응답 주체 식별(짝짓기 귀속 아님, 카드 자기-라벨). **대화 채널(agent-channels 2·4단계, v8.25.0·v8.28.0)**: 입력 위 `#ov-channels` **채널 칩 바**(v8.28.0 — 4단계에서 select 드롭박스 교체)로 대화 상대(`main` | agent key)를 고르면 개요가 그 채널만 렌더(`ovActiveChannel`; main=`ovEntries`, agent=`ovChannels[key]`=agent_msg 스트림) → 여러 대화가 한 스트림에 섞이던 것 해소(④). `submitChatOrPrompt` 가 agent 채널+chat 이면 `/api/agent/<key>/input` 로 라우팅(prompt/confirm 은 main). 발신엔 대상 배지(`.ov-umsg-to`), 회신엔 주체 배지 — **라벨은 ovRender 시점에 roster 로 해소**(replay 가 roster 보다 먼저 와도 키로 고정 안 됨). **에이전트별 고유 아이콘(v8.32.0)**: `ovAgentLabel` 이 `ovAgentIcon(key)`(key 문자합 해시 → 24종 이모지 풀)로 라벨을 짓는다 — 서버 `agent_cli/agent_icon.py`(스윔레인·scope_start `d.agent`·CLI)와 **동일 풀·해시**(test_app_markdown parity 로 고정, key 는 ASCII 라 ord==charCodeAt) → 같은 agent 가 서버/웹에서 같은 아이콘. **런-레벨 실패 요약 표시(v8.44.0)**: `turn_error`(LLM 호출 실패·워커 예외·워커 크래시 — 이 3곳뿐, 전부 '이 런은 끝났다'는 치명 신호)를 `ovOnTurnError` 가 개요 블록(`⚠ 오류` 배지·mono·✗)으로도 싣는다. 전문 카드(`renderError`)는 유지 — 추가지 대체가 아니다. 실사고: 보드 게시글이 LLM 서버에 없는 모델로 고정돼 모든 호출이 404 났는데, 서버는 `turn_error` 를 정상 방출하고 전문엔 카드가 떴지만 **기본 뷰인 개요가 그 이벤트를 안 받아** 사용자에겐 '요청만 남고 무반응'(=쿼리가 안 나감)으로 보였다. 서브에이전트 스코프(`task_id`)는 제외 — 그 실패는 부모에게 관찰로 전달된다. **슬래시 명령 출력 요약 표시(v8.43.0)**: `/sh`·`/help`·`/compact`·`/skills`·`@agents` 는 LLM 루프를 안 타고 `observation` 하나만 쏴서, 개요엔 요청만 남고 결과가 안 붙었다(전문에만 존재). `ovOnSlashOutput` 이 **tool_name 화이트리스트**(`OV_SLASH_TOOLS` = sh/help/compact/skills/agents — 배지 라벨 맵이 곧 게이트)로 그 관찰만 개요 블록으로 싣는다. 본문은 **마크다운 미적용**(`escapeHtml` 만 + `.ov-mono` 고정폭 — 셸 출력의 `**`/`|`/`#` 이 굵게/표/헤더로 뭉개지던 것 방지; `.ov-tx` 의 pre-wrap 상속으로 개행 보존, 260px 상한+스크롤은 타임라인 `.obs-body` 240px 와 동형 정책), `success=False` 면 ✗ 배지(`ov-st fail` — 종전 개요 블록은 항상 ✓ 고정이었다), **[복사]만 노출**(관찰 카드엔 nav 앵커가 없어 [전체 대화] 점프가 해소 불가). 제외: `agent`(@<agent> 실제 런 — `ovOnScopedFinal` 이 이미 싣는다, 넣으면 이중 표시)와 not-found 에러(tool_name 이 동적). 서버 방출 이름과의 정합은 `test_overview_slash_output_allowlist` 가 두 파일을 교차 고정. **개요 상단 ambient 줄 제거(v8.32.0)**: 에이전트 상태 dots 는 **채널 칩(agent 탭) 안에 직접**(v8.33.0 — 구 흐름-탭 dots 통합; `ovChanChip` 이 state 색 dot), 컨텍스트%·뷰어·모델·스킬(헤더 중복)은 삭제(`ovAmbient`/`ovOnCtx` 제거). 채널 바는 roster 로 동기화 — **4단계(v8.28.0)**: 칩마다 ❓(waiting_ask)·dead(dim+취소선) 상태, ✕(kill)/↻(resume) 컨트롤(→`/api/agent/<key>/{kill,resume}`, I-1). (🔔 안본회신 배지는 v8.31.0 에서 제거 — reconnect replay 가 전체 이력을 unread 로 세어 '글 수'처럼 부풀던 문제, 편익 대비 폐기.) 죽은 agent 채널이면 입력 비활성+사망 고지(`ovApplyChannelInput`/`ovActiveDead`, ⑤); roster 에서 완전히 사라지면 main 복귀(dead 는 유지). 채널 상태 휘발(A-1: 리로드 시 main). **구 "Agents" 드로어(헤더 🤝 `#agent-btn`·`#tm-drawer`·드로어 IIFE·`.tm-*` CSS·`agentcli:tm-*` 중계)는 v8.29.0(5단계)에서 제거** — 기능 전부 개요 채널로 이관. `agent_cleared` 는 `ovOnAgentCleared`(그 채널의 `ovChannels`/`ovAskTray`/`ovChanUnread` 정리 → resume 재생 중복 방지)로 재라우팅. **🔍 인스펙터가 활성 채널 추종(v8.30.0)**: `#insp-main-btn` 이 `window.__inspectorScope()`(메인 IIFE 노출 — main→`{scope:"",kind:"main"}`, agent→`{scope:agent key,kind:"agent"}`)로 `__openInspector` 를 열어, agent 채널이면 그 agent 의 프롬프트 스냅샷(`GET /api/debug/prompt?task_id=<key>` — 시스템+동적 컨텍스트, note_scope_ctx 로 등록됨)을 보여준다. **글로벌 ask 트레이(3단계, v8.26.0)**: `#ask-tray`(입력 위 고정, 채널 무관)가 roster 의 `waiting_ask` agent + 그 질문(`ovAskTray`, agent_msg direction="question")을 `ovRenderAskTray` 로 렌더 — 어느 채널을 보든 다른 agent 의 질문이 노출된다("agent1 보는 중 agent2 ask"). 인라인 답변은 그 asker 로 고정(`ovSubmitAsk`→`api/agent/<key>/input`, 드롭박스 채널 무관). `waiting_ask` 가 표시 진실원이라, 선착순으로 누가 답하면 상태 해제→roster 갱신→모든 뷰어 트레이에서 제거. **main prompt/confirm 도 트레이로(3b, v8.27.0)**: `input_required` 가 `ovMainAsk` 를 세팅→`ovBuildMainAskEl` 이 트레이 최상단 항목으로 렌더(검증된 `buildPromptMetaEl`/`highlightDangerHtml`/`.confirm-btn`/stall/409 재사용), **input box 는 chat 전용**(`setInputMode`/`#input-mode-badge`/`#confirm-buttons`/`currentMode` 제거). prompt→`ovSubmitMainPrompt`, confirm→`ovSubmitMainConfirm`, 409/`input_resolved`→`ovFoldMainAsk`. `isBusyChat = workerBusy && !ovMainAsk` — main ask 대기 중엔 `#chat-stop`(런 중단) 숨기고 `#abort`(입력 대기 취소)만 노출(두 Stop 방지). `ovFinalizeGen`(`failed_turn`·`worker_state` idle)은 complete 못 낸 gen 엔트리를 done 확정/제거하는 안전망. resp 본문은 done 시 `escapeAndFormat`(마크다운)·gen 중 평문+caret. 🤝 agent mail wake 도 그대로 로그에 표시(라벨은 content 의 `[…]:` 에서 추출). `ovResolveCard`: 블록의 `data-nav-ts`(메인) 또는 `data-scope-id`(dispatched)로 [▤ 전체 대화] 점프 카드 해석. **로직 테스트**: `tests/test_app_markdown.py::TestOverviewFlatRender` 가 `ovUserHtml`/`ovRespHtml` 소스를 추출해 Node 로 실행(그룹핑·귀속 없음 검증). **큐 전달상태(클라 추론)**: `queue` 이벤트 pending diff 로 큐를 떠난 항목을 `qLeaving` 로 잡아 `⏳→✓주입됨`(내 것=반영됨)/`✕취소됨` 영수증(1.8s 후 제거), 주입 판정은 `qOnUserMsg` 재방출 텍스트 매칭(1.4s 타임아웃=취소), 내가 ✕=즉시 취소(`qCancelledByMe`). 승격은 주입 메시지가 다음 블록 누적 쿼리가 되며 자동. **선택→고정 상세 패널(Tier 2) — v8.13.0 도입 후 v8.15.0 전면 제거**: 개요 응답이 본문을 인라인 렌더하고 전문 드로어가 이미 side-by-side 라 "엿보기 패널"이 중복이 되어 폐기(`#detail-panel`·`dp*`·`.dp-*` 삭제). 흐름 막대/화살표 클릭은 다시 **전문 드로어로 직행**(teamHost 핸들러가 `setViewMode('detail')`+`expandAncestors`+`scrollTimelineTo`). **개요 응답 액션(v8.13.1)**: 블록의 `⧉ 복사`(`ovCopyBlock` — 클립보드; navigator.clipboard 는 secure-context 전용이라 LAN http 대비 execCommand 폴백 + "✓ 복사됨" 플래시)·`▤ 전체 대화`(`ovOpenTimeline` — 드로어 열고 `ovResolveCard` 로 해석한 카드[메인 `data-nav-ts`/dispatch `data-scope-id`]로 점프). `ovOnFinal` 이 `navTs=d.ts`(메인 final 카드의 stampNavTs 와 동일 키) 캡처 → resp 엔트리에 `data-nav-ts` 부여. **헤더 개편 IIFE(v8.57.0)**: token_usage 리스너가 `#tok-base`(↑ 제거·↓·Σ↓)·thinking_tick 이 `#tok-think`(💭) 를 헤더에 상시 부착(게이지 갱신 제거); 노브 칩 토글(위임 클릭, data-pop, 한 번에 하나·Escape); 각 노브 IIFE 가 값 배지 갱신; **v8.60.0**: stall IIFE 가 두 축(`{seconds}`/`{attempts}` 를 **보낸 축만** POST — 서로 덮어쓰면 한쪽을 고칠 때마다 다른 쪽이 기본값으로 되돌아간다)을 다루고 배지·파생값을 서버 확정값으로 갱신하되 **포커스된 입력은 건드리지 않는다**(응답이 두 축을 다 실어 오므로, 그대로 쓰면 옆 칸에 타이핑 중이던 값이 되돌아간다). `stream_stall` 리스너 + `renderStreamStall`(wait=`.stall-line` 하나를 제자리 갱신+진행바, resend=`.stall-resend` 기록, clear=제거); **v9.2.0**: 🔌 IIFE — 로드 시 `GET api/mcp` 한 번(부팅 시점 상태라 sticky/SSE 불필요), 서버별 행(색 점·이름·전송·도구 또는 실패 이유), 실패 있으면 `has-fail` 로 칩 경고색; `el()` 은 메인 IIFE 스코프라 지역 헬퍼 재정의. **v9.4.0 ① (docs/chat-ui)**: 흐름 뷰(스윔레인) 전면 제거 — `team_view.js`·`team_model.js`(1073줄) 삭제, `TeamView.ingest` 6곳·`showNavMiss`·`$flowBtn`/`$teamView` 정리, `setBaseView` 는 base 가 개요 하나뿐이라 분기 소멸. 같은 대화를 세 가지로 그리던 구조의 첫 축소(개요/전문 통합은 ②). **②③**: 개요·전문·흐름 3뷰를 `#messages` 하나로 통합(뷰 전환·드로어·탭 소멸, 개요 렌더 23함수 제거)하고, 카드를 **한 줄 리듬**(`makeRow` — 아이콘·종류·요약·펼침표시 4칸 그리드)으로 재작성. 기본 접힘·클릭 펼침이고 **실패 관찰만 펼친 채 시작**(지금 봐야 하는 유일한 줄). 요약은 도구별(`actionSummary`)·출력의 **마지막** 유의미한 줄(`obsSummary` — 셸/테스트/린트의 결론이 끝에 있다). 관찰 행의 종류 칸은 도구 이름이라 ⚡ 호출과 ✓ 결과가 눈으로 짝지어진다. **④**: 스트리밍(라이브 타이핑) 폐기 — `card-streaming` 기계 4함수·`stream_chunk`/`stream_reset` 리스너·task group 의 스트리밍 슬롯 제거, **생성 중 한 줄**(`showGenerating` — `● 생성 중 · N tokens · 💭 사고 M`)로 대체. 본문이 흐르지 않아 카드가 자라며 화면이 튀지 않고, 거부된 원문은 `renderFailedEmission` 이 계속 보여준다. **⑥**: **채널이 타임라인의 필터**가 됐다 — `#messages` 직계 자식의 `data-ch`(`appendToTimeline` 이 도장, `scopeChannel[task_id]` 가 출처)로 걸러 main/agent 대화를 분리한다. 상주 에이전트 작업 스코프는 `scope_start` 의 `ctx_dir="agents/<key>"` 로 자기 채널을 밝히므로 **서버 무변경**. `data-ch` 없는 노드(생성 중 한 줄)는 어느 채널에서나 보인다. `agent_msg` 는 축적(구 `ovChannels`)이 아니라 **왕래 줄**(`ovRenderAgentMsg` — `← 받음 / → 보냄 / ❓ 질문` + 상대 칩)로 그려지고, 상대 칩이 **점프 버튼**을 겸한다: main→agent(`⚡ agent` 도구 호출의 `key`)·peer↔peer 는 가능, agent→main 은 매칭 키가 달라 비활성(칩이 평문). 돌아가기(`#ch-back`)는 **한 단계** — 새 점프가 이전 것을 덮고 한 번 누르면 사라진다(스택이면 라벨이 매번 바뀌어 예측 불가). 스코프 카드는 `kind`/`ctx_dir` 로 **중첩 블록**을 가른다(skill=보라 레일·inline agent=amber 레일·상주 agent=레일 없음). 죽은 개요 잔재(`ovEntries`/`ovTopScopes`/`ovScopeSrc`/`ovSkills`/`ovClock`/`.ov-umsg-to`) 동반 제거. **실장에서만 잡힌 3건**: ㉠ 시각 배지(absolute)가 상대 칩을 덮어 클릭을 먹음 → `pointer-events:none` + 꼬리 칸이 있는 줄은 시각을 **같은 그리드 안**(`.row-time`)으로 옮김(폭 상수 추정 폐기), ㉡ `renderUserMessage` 만 `$messages.appendChild` 직결이라 main 입력이 모든 채널에 샘, ㉢ 요약을 한 줄로 자르던 방침 철회 — 잘린 줄은 폭을 넓혀도 안 보이고 펼칠 본문이 없는 줄(생각)은 읽을 방법이 아예 없었다(사용자 지적) → `.row .s` 줄바꿈(`overflow-wrap:anywhere`), 시각은 **행이 있으면 그리드 안**(`.row-time`)·없으면 코너 배지로 갈라 겹침 제거(산문 블록만 `--time-w` 로 자리 비움), ㉣ `.card-user{display:flex}` 가 UA 의 `[hidden]{display:none}` 을 이겨 속성만 붙고 안 숨음 → `#messages > [hidden]{display:none!important}` 로 표면 전체에 못 박음. **⑥ 후속(실사용 보고 3건)**: ㉮ `failed_turn` 리스너가 `_emit` 이 실어 보낸 스코프 `task_id` 를 흘리고 `appendToTimeline(card)` 로 붙여, **에이전트가 낸 거부 응답(`⚠ no action`)이 main 대화에 빨간 박스로** 떴다 — `renderFailedEmission(reason, raw, taskId)` 로 스코프를 실어 준다(`renderStreamStall` 의 무진전·재전송 줄도 같은 누락이라 함께 수리). 루트에 카드를 붙이는 경로는 **빠짐없이** task_id 를 날라야 한다는 것이 계약. ㉯ 채널 전환 시 필터가 노드를 감추고 드러내면 문서 높이가 확 바뀌는데 스크롤 위치는 그대로라 아무 데나 떨어졌다 → `ovSetChannel` 이 **맨 아래로 이동 + 자동 따라가기 복구**(점프는 그 뒤 앵커로 다시 스크롤하므로 무영향). ㉰ 응답 독(`#dock` — 최신 main 답을 입력창 위에 고정) 제거: 전문이 드로어 안에 있던 시절 "응답이 왔다"는 유일한 단서였는데, 표면이 하나가 된 뒤로는 마지막 카드와 **같은 내용을 두 번** 보이고 채널과 무관하게 main 답만 띄워 혼란을 줬다. 유일한 소비처였던 `data-nav-ts` 앵커(`navTs`/`stampNavTs`, `scope_start` 의 `nav_ts` 스탬프)도 함께 사망 — 구 스윔레인 네비게이션의 잔재. **v9.7.0**: `agent_wake` — 에이전트 메일이 idle 한 main 을 깨운 런을 **한 줄 리듬**(`🤝 메일`)으로. 종전엔 `push_user_message` 로 흘러 `.card-user`(파란 말풍선)로 그려져 **기계가 만든 신호가 사람 발화와 구별되지 않았다**(사용자 제보). 서버는 이미 `author=""` 로 귀속 없음을 표시했지만 그 구분의 유일한 소비처(스윔레인)가 ① 에서 사라져 프론트로 나를 길이 없었다. `card-sys`(휘발 마커)가 아닌 이유: 이건 **그 런이 왜 시작됐는지의 기록**이고 모델 컨텍스트에도 들어간 실제 턴 입력이라 persistent 여야 한다. 요약은 사람용 한 줄, 펼치면 **모델이 받은 원문** — 표시와 지시문을 한 줄 안에서 분리한다. **v9.7.0 ②**: 상주 에이전트 작업 스코프는 **카드를 만들지 않는다** — 채널이 이미 묶으므로 자기 채널 안에서 자기를 또 묶으면 정보가 늘지 않으면서 기본 접힘 뒤로 투명성을 숨기고(⑥의 목적과 정반대) main 은 평평한데 agent 만 한 겹 들어가 통일이 깨졌다(사용자 지적). `noteScopeChannel(taskId, parent, ctxDir)` 이 채널 등록을 카드 생성에서 떼어내 boolean 을 돌려주고, `scope_start` 가 그게 참이면 `ensureTaskGroup` 을 건너뛴다 — **분기 순증은 그 한 줄뿐**이다. `appendToTimeline` 은 그룹이 없으면 `channelOf(taskId)` 로 루트에 붙이는데 그게 정확히 원하는 동작이고, `closeTaskGroup`/`updateTaskStatus` 는 이미 `if (!g) return` 이라 무변경. 중첩 skill/inline 은 `channelOf(parent)` 로 그 채널 루트에 자기 카드를 그대로 갖는다. `ensureTaskGroup` 은 파라미터 8→7, 3분기→삼항 1개로 **줄었다**(`.scope-agent` CSS 동반 삭제). **중복 회신**: `→ 보냄` 이 바로 위 최종답을 그대로 반복하면 영수증 한 줄(`.row.receipt`, 본문 없음). 조건부인 이유는 kill→resume 재생이 `agent_message` 만 내보내(최종답 없음) 그때는 이 줄이 **유일한 기록** 이기 때문 — `lastFinalByChannel` 한 칸으로 판정한다. **v9.8.0 감사(docs/audit)**: `finishCard(el, ev, channel)` — 13개 렌더러가 공유하는 건 본문이 아니라 **꼬리 배관**(도장·귀속·스크롤)이고 최근 누수가 전부 거기서 났다. **페이로드 통째**를 받아 귀속 누락이 "필드를 잊었다"가 아니라 "인자를 안 넘겼다"가 되게 한다(`transcript_truncated` 가 인자 생략으로 새던 마지막 사례). `appendToTimeline` 에 `channel` 인자 추가 → 왕래 줄이 손으로 하던 3단계 흡수. `sysLine()` 로 `card-sys` 조립 7곳 통합. `status` 리스너 추가(호출 10곳이 통째로 버려지고 있었다 — Max turns·HTTP 재시도 전부 암전). `classify()`+`CARD_KINDS` 로 export 등록부 일원화(`card-msg`·`card-sys`·`card-failed` 가 조용히 빠져 있었다). **범용 타임라인 descriptor 는 채택하지 않았다** — +200/−85 줄에 per-type 본문 548줄은 어느 쪽이든 그대로라 순증 +115 (docs/audit §3). **v9.8.0 후속**: 형식 거부(`recovery`)를 **화면에서 제거** — `renderFailedEmission`·`.card-failed`·개입 관찰 전부. 하네스의 재시도 기계지 모델의 작업이 아니고(투명성 대상과 범주가 다르다), history 에 거부 사실이 없어 resume 은 원래 안 보여줬으므로 **라이브를 resume 에 맞춰** 괴리를 없앴다. 대신 `retry_tick`(transient)이 생성 중 줄의 `↻ 재시도 N` 을 올려 **왜 오래 걸리는지**는 남기고, 재시도 소진 시 하드페일은 `turn_error` 로 영속 표면에 남는다 (docs/audit §7.3).
│       └── style.css        (1567) chat UI 스타일 — **칩 헤더 스타일(v7.1.0)**: `.hd-chip` pill(999px radius·mono 12px)+`.hd-chip-btn` hover/expanded accent 테두리, `.hd-chip-text`(내부 ellipsis 담당)·`#info` 28ch inline-block ellipsis, `#ctx-gauge` 40×5px 미니 게이지, `#ctx-popover` 는 `#theme-menu` 글래스 팝오버 미러(세로 flex, 이동 컨트롤 수납). — 가독성 우선, 모바일 폴백 단일 컬럼. **멀티-테마 디자인 토큰**: 파일 상단 `:root`(공유 다크 베이스 = "slate" ~55 토큰: surface/text/accent/status/glass/shadow) + 큐레이션 테마별 `[data-theme="midnight|terminal|amber|light"]` 오버라이드 블록(다크 변형은 surface+accent 만, light 는 전체 오버라이드). **다크 테두리=반투명 헤어라인**(`--border: rgba(255,255,255,.07)`) — 단단한 회색 선 대신 부드러운 경계(프리미엄 다크 룩의 핵심). 본문 전체가 raw hex 없이 `var(--…)` 로 파생돼 테마 추가=토큰 블록 하나(회귀가드 `test_theme_tokens_and_picker_wired`: body raw-hex 0·토큰 self-ref 0·테마 블록 존재). 테마 피커 드롭다운(`#theme-menu` + `.theme-item`/`.theme-swatch`). **폼/버튼 color 토큰화**: input/textarea/select/button 은 `color` 미상속이라 다크에서 검은 글씨가 되던 것 → 기본 `var(--text)` + placeholder `var(--muted)`. 메시지/카드 색상, 입력창 sticky, 닉네임 바·✎ `#rename-btn`, Prompt Inspector 드로어(스코프 칩 row), Export 선택 모드, Download 드로어, 헤더 아이콘 버튼(`#theme-btn`/`#inspector-btn`/`#export-btn`/`#files-btn` 공통 borderless-emoji) 등. **노브 칩/보따리(v8.57.0)**: `.knob`/`.knob-btn`(값-배지 pill)·`.knob-pop`(우측정렬 팝업); 구 `#ctx-gauge*`/`#ctx-popover` 스타일 제거. **v8.60.0**: `#stall-pop:not([hidden])` 만 3열 그리드(두 입력+파생 줄) — `:not([hidden])` 이 필수다. id 선택자는 `.knob-pop[hidden]{display:none}` 보다 특이도가 높아 그냥 `#stall-pop{display:grid}` 로 쓰면 hidden 을 이겨 팝업이 항상 펼쳐진 채 렌더된다(브라우저 테스트가 잡은 실버그). `.knob-derived`(파생값·`.is-off`)·`.stall-bar`(무진전 진행바)·`.card-sys.stall-line`/`.stall-resend` 추가. 좁은 창은 flex-wrap 줄바꿈(보따리 없음).
...
│   ├── base.py              (~75)  LLMProvider 프로토콜, LLMResponse(+thinking), TokenUsage(+cache_creation/cache_read tokens). `strip_think_blocks` 는 v5.19.1 부터 thinking_tags 재-export (openai.py·기존 테스트의 import 경로 보존). **`ThinkingPolicy`+`resolve_thinking_policy` (v8.37.0)**: request_overrides 해석의 공용 단일 소스 — supports_thinking=False→None(일절 미주입, v8.21.1 게이트), off 판정=`enable is False or eff=="off"`("off" 가 명시 enable=True 를 이김 — 프로바이더 간 동형), effort 기본 medium. 프로바이더는 정책을 자기 방언(OpenAI reasoning_effort+chat_template_kwargs / Anthropic thinking 블록)으로 번역만 — 종전 OpenAI 의 '사고 off 시 effort 잔존' 상충 신호 수리
│   ├── capabilities.py      (578)  ModelCapabilities(+`supports_grammar: bool|None`, v9.24.0 — None=모름; `grammar_probe` 가 한 단어 문법을 보내 **출력이 묶였는지**로 판정: 필드를 받아 주는 것과 강제하는 것은 다르다[omlx 는 guided_regex 를 조용히 버린다]; 판정은 모델 감지(`_detect_capabilities`) 때 한 번 하고 저장; 등록 엔트리에 키가 없으면 None=미확인 그대로 — **부트 프로브 없음**[서버가 바쁠 때 그 한 요청에 열기가 54초 밀린 실측]) + 프로브 감지 + 진행 콜백 + 자동 저장. **공유 오케스트레이터 `_detect_capabilities(model, transport)`** — context_window/thinking-태그/structured/reject/`max_output` 로직은 provider 무관 1곳, **transport 만 provider별**(`_OpenAITransport`=`/chat/completions`·Bearer, `_AnthropicTransport`=`/messages`·`x-api-key`+`anthropic-version`·`content[].text`). OpenAI 는 기존 helper 위임(parity), Anthropic 은 `_detect_anthropic_context_window`(/models 메타→/messages overflow→128K) + 프롬프트-only JSON structured probe(strict 항상 False) + `<think>` 태그 thinking 탐지. **self-register (v8.41.0)**: `providers/__init__` 가 `register_provider` 레지스트리 소유 — 각 어댑터 모듈이 import 시 자기 등록(wire_formats.register 동형), capability 프로브 transport 는 프로바이더 클래스의 `capability_transport` 훅 소유(`_detect_runtime_capabilities` 의 이름 분기 소멸; 프로바이더 추가=모듈 1개+`_BUILTIN_MODULES` 1줄). **thinking 프로브 transport 공통 2단계 계약 (v8.37.0)**: ①기본 프로브 → 미검출 시 ②방언별 사고-켜기 스위치 재프로브(OpenAI=chat_template_kwargs.enable_thinking / Anthropic=thinking 블록 budget 1024+max_tokens 증액, thinking 블록→`<think>` 정규화) — 재프로브 실패는 관용(→미지원). `_anthropic_text` 는 첫 **text 타입** 블록 스캔(thinking 활성 응답의 위치 고정 인덱싱 수리). (omlx 가 두 API 동일모델 서빙·실 Anthropic 도 GET /v1/models 지원이라 양쪽 동작; 실 Anthropic 은 /v1/models 에 window 메타 없어 overflow/fallback/registry 로.) OpenAI 호환 context window는 `/v1/models` 메타 → overflow probe → 128K fallback 3-tier. **auto-detect 시 `max_output_tokens = context_window // 4`** (예: 256K→64K, 16K→4K; 기존 4096 cap 제거). context window가 `MIN_CONTEXT_WINDOW`(16K) 미만이면 `UnsupportedModelError` raise → CLI(`_setup_provider`)가 잡아 fail-fast (registry/models.json 저장값은 이 규칙 미적용 — 저장값 그대로). **structured-output 감지**(`_probe_structured_output`): context window 수용 후 `response_format={"type":"json_object"}` → `supports_structured_output`, 이어 strict `json_schema` → `supports_strict_schema` 프로브. 산문 자연스러운 프롬프트의 반환값이 유효 JSON(스키마 준수)일 때만 인정(서버가 `response_format` 무시 시 오탐 방지), 실패 시 보수적 False
│   ├── http.py              (466, v4.48.0 C6) post_with_retry(공유 재시도 — 네트워크 예외 10회 + 게이트웨이 5xx[502/503/504] 3회, 독립 예산, 5.14.1)·interruptible_lines(TTFT-interrupt/idle 폴링)·make_stream_patient + **공용 SSE 골격 `run_sse_stream`**: 라인 순회(idle notice+StreamIdleTimeout **양 provider 동일** — 이전 openai 만)·`data:` 파싱·`[DONE]` 관용·**JSONDecodeError 관용(양쪽)** — 이전 anthropic 만·누산/ttft 타이밍·degeneration 조기종료(게이트 문자는 wire shape 소유 — `WireFormat.degeneration_trigger`: json_fc '#'/xml_fc '<', P0-4 v8.34.0; 종전 '#' 하드코딩은 xml_fc 러너웨이에서 무발화)·interrupt 라벨링. provider 는 `map_payload(dict)->StreamEvent`(이벤트 shape 해석)만 소유 — wire-format self-contained 와 같은 정신(독립 진화 필요 부분만 provider 에) — **P3 (v8.55.0)**: 유휴 기준을 '마지막 줄'→'마지막 진전(ProgressClock — keep-alive 무시)'으로, 한도는 사용자 값(ctx.stream_idle_timeout_s — web ctx 팝오버 Stall / env AGENT_CLI_STREAM_IDLE_TIMEOUT_S, 0=끔, spawn 시 상속)이 base.CallSettings(thinking·무진전 한도·클램프 max_tokens 의 단일 여행 컨테이너 — request_overrides·capabilities-replace 대체)로 매 콜 전달 — **v8.60.0**: 재연결 루프 상수를 `STREAM_MAX_RECONNECTS=3`("재전송 횟수") → **`STREAM_MAX_ATTEMPTS=4`("총 시도", 첫 전송 포함)** 으로 재정의(동작 불변 — `range(N+1)`→`range(N)`). 같은 파일의 `post_with_retry` 가 이미 _DEFAULT_ATTEMPTS 를 총 시도로 쓰므로 두 층의 의미를 통일한 것이고, 값=표시(`시도 n/4`)=최대 대기 나눗수가 되어 `(N+1)` 변환이 사라진다. 횟수도 세션 노브(`CallSettings.stream_max_attempts` — ctx.stream_max_attempts / web ⏳ 노브 2번째 입력 / env AGENT_CLI_STREAM_MAX_ATTEMPTS / CLI `--stall-attempts`, spawn 시 상속)라 한도와 한 쌍으로 최대 대기(=한도×횟수)를 정한다. 시도 좌표는 루프만 알므로 `handle_stream(r, attempt, attempts)` 로 내려가 `run_sse_stream` 의 대기 알림까지 닿는다 — **v8.61.0**: 스트림 **중간** 조기 종료(`_BROKEN_STREAM` = ChunkedEncodingError/ContentDecodingError — urllib3 "Response ended prematurely")도 재전송. 이 예외들은 RequestException 직계라 `_RETRYABLE`(Timeout/ConnectionError)에 안 걸리고, 본문 스트리밍 중이라 `post_with_retry`(pre-stream 전용) 범위 밖이라 **어느 그물에도 안 걸려 런이 죽었다**. 예산은 무진전과 **분리**된 고정 `_BROKEN_STREAM_ATTEMPTS`(10, +`_DEFAULT_DELAY` 1초): 무진전은 시도당 수 분, 조기 종료는 즉시 실패라 공유하면 플래핑 서버에서 1초에 예산이 소진된다. 사용자 노브로도 안 뺀다(기다릴 시간의 트레이드오프가 아니라 장애 복구 — post_with_retry 의 고정 예산과 같은 성격). 루프는 `while True` + 독립 카운터 2개(`stalls`/`broken`)이고 둘 다 각 except 에서 증가해 총 반복은 유계. 재전송 전엔 항상 `render_stream_reset()` — 재전송은 생성을 처음부터 다시 하므로(서버측 resume 없음) 부분 출력을 안 걷으면 새 토큰이 그 뒤에 이어붙는다(최종 content 는 시도마다 새 `StreamAccum` 이라 멀쩡 — **표시만** 어긋난다). 이 갭은 종전 무진전 재전송에도 있었으나 그쪽은 거의 항상 TTFT(토큰 0)라 안 드러났다
│   ├── anthropic.py         (240, C6 후 매퍼+usage 조립+idle 재연결 래퍼 — 이전엔 재연결/patient-socket 미배선으로 침묵 시 무한 대기)  Anthropic Messages API (tool_use + thinking blocks + streaming + TTFT + prompt cache via cache_control). **`degeneration_check`**(= `wire.is_degenerate`, provider-독립)·**`interrupt_check`**(zero-arg) 두 predicate 를 `_handle_stream` 가 처리. line read 는 `interruptible_lines` 경유라 interrupt 는 TTFT 포함 no-data gap 에서도 깨지고, 루프 뒤 `interrupt_check()` 재확인으로 `stop_reason="interrupted"`(loop 이 partial 폐기). degeneration 은 content chunk 별 `'#'` 게이팅 후 True 면 `stop_reason="degenerate_runaway"`(loop 이 라벨·복구). openai 와 동작 동일 — loop 이 두 provider 에 같은 predicate 를 넘기므로 대칭(이전엔 anthropic 이 degeneration_check 를 받고도 버리는 비대칭 부채였음). **v8.37.0**: 인라인 `<think>` 격리를 스트림/비스트림 양 경로에 적용(OpenAI 동형 — omlx 류 Anthropic-호환 서버의 태그 누출 격리), 비스트리밍 다중 text/thinking 블록 **누산**(종전 마지막 블록만 잔존), thinking 주입은 공용 `resolve_thinking_policy` 경유.
│   └── openai.py            (218) OpenAI 호환 API (function calling + reasoning_content + streaming + TTFT). **v9.24.0**: `CallSettings.grammar` → body `guided_grammar`; 제약 중엔 `chat_template_kwargs.enable_thinking` 을 **항상 명시**(문법이 `<think>` 열림 여부를 가정하므로 서버 기본값에 맡기면 어긋난다). **인라인 <think> 격리 (5.10.0)**: 두 응답 조립 경로(스트리밍/비스트리밍) 모두 `base.strip_think_blocks` 적용 — MiMo 류가 content 에 태그로 흘리는 긴 추론을 제거(안 닫힌 열림 태그=EOF 까지)하고 `thinking` 필드로 이동(reasoning_content 와 합류, 정보 무손실). 태그 vocab 은 capabilities 탐지와 동일 4종; capability 프로브는 자체 transport 라 비영향. **스트리밍 콜은 공용 재연결 루프(`http.stream_with_reconnect`, v8.41.0 — 종전 openai/anthropic 28행 복붙 2벌 단일화; run_sse_stream 은 리스트 누적+종료 1회 join, degeneration 조기종료는 `_DEGEN_WINDOW`=4096 tail 검사로 O(n²) 해소)**: post `(30,30)` timeout(헤더 바운드) → `make_stream_patient` 로 소켓 patient 리셋 → `_handle_stream`(idle 파라미터 + `on_idle`=render_status 대기 알림 전달). `StreamIdleTimeout`(10분 침묵) 잡으면 재연결 알림 렌더 후 재전송, `STREAM_MAX_RECONNECTS=3` 회 후 raise. 비스트리밍은 `(30,1200)`. **`degeneration_check`** kwarg(= `wire.is_degenerate`)가 있으면 `_handle_stream` 이 누적 텍스트에 적용 → True 면 stream 을 닫고 break(format-runaway 조기 중단, `'#'` 포함 chunk 에서만 검사해 O(headers)). truncated content 는 `stop_reason="degenerate_runaway"` 로 반환돼 downstream 에서 parse·라벨. **`interrupt_check`** kwarg(zero-arg, = loop `_interrupt_check` → `stop_event.is_set()`): line read 가 `interruptible_lines` 경유라 TTFT 포함 no-data gap 에서도 interrupt 가 깨지고(블로킹 read 를 시그널핸들러/타스레드에서 직접 닫는 reentrant/race 회피 — loop 은 flag 만, 닫기는 reader 소유 측), 루프 뒤 `interrupt_check()` 재확인으로 `stop_reason="interrupted"`. degeneration partial 과 달리 loop 이 이 partial 을 **파싱·기록 없이 폐기**(사용자가 방향 전환). **`response_format={"type":"json_object"}` 는 `kwargs["json_mode"]` 가 True 일 때만 전송 — provider 는 capability 를 직접 안 본다.** `json_mode` 는 **`WireFormat.provider_call_kwargs(capabilities)`** 가 결정(wire ⨯ capability 단일 결정점): JSON-shaped wire(react)는 `capabilities.supports_structured_output`, json_fc(markdown)는 capability 무관 항상 `False`. 이전엔 provider 가 capability 와 wire 의 `skip_json_format` 을 직접 조합하다 prefix_md 에 JSON 강제 → omlx/mlx degenerate(`[2025]`/`[1000,1000]`)하는 버그가 있었음 (prefix_md 기본 전환이 노출 — bakeoff 는 provider 우회라 못 잡음). 이제 새 wire plugin 은 `provider_call_kwargs` 만 정의하면 provider 가 잘못 조합할 여지가 없음. **`response_format={"type":"json_object"}` 는 `kwargs["json_mode"]` 가 True 일 때만 전송 — provider 는 capability 를 직접 안 본다.** `json_mode` 는 **`WireFormat.provider_call_kwargs(capabilities)`** 가 결정(wire ⨯ capability 단일 결정점): JSON-shaped wire(react)는 `capabilities.supports_structured_output`, json_fc(markdown)는 capability 무관 항상 `False`. 이전엔 provider 가 capability 와 wire 의 `skip_json_format` 을 직접 조합하다 prefix_md 에 JSON 강제 → omlx/mlx degenerate(`[2025]`/`[1000,1000]`)하는 버그가 있었음 (prefix_md 기본 전환이 노출 — bakeoff 는 provider 우회라 못 잡음). 이제 새 wire plugin 은 `provider_call_kwargs` 만 정의하면 provider 가 잘못 조합할 여지가 없음.
│
├── tools/                          도구 시스템 **도구 왕복 계약 (v9.16.0, tests/test_tool_wire_contract.py)**: 살아 있는 wire format 둘이 모두 `multi_op` 이라 인터셉트 안 되는 도구는 항상 `광고된 flat op → wrap_single_op → 중앙 검증` 을 탄다. TOOLS **전수**로 (a) 래핑이 판정을 바꾸지 않고 (b) 필수 키가 살아남고 (c) 멱등인지 본다. 종전 가드(`test_loop.TestWrapSingleOpIdempotence`)는 합성 예시가 **의미** 검증(enum)에 걸리면 그 도구를 `continue` 로 건너뛰어 15개 중 5개가 미판정이었다 — 하필 이 가드를 만들게 한 `monitor` 가 거기 있었다. 막으려던 고장은 **모양**인데 판정을 **의미**에 걸어 축이 섞인 것. 새 가드는 래핑 전후 판정을 비교해 의미를 판정에서 빼고, 마지막 테스트가 **전수 커버리지 자체**를 고정한다(가드의 가드).
...
├── monitor/                 (638, v9.11.0)  **조건 → 보고 감시** (docs/monitor/DESIGN.md). `MonitorRegistry` 가 1s 폴링 스레드를 소유하고(펌프는 `run_one()` 을 동기로 불러 턴 중 몇 분간 막히므로 전용 스레드여야 한다), 보고는 **설치한 주소로 즉시 배달**된다(v9.14.0, docs/wiring §3.2) — `Monitor.owner`(`"main"`|`"agent:<key>"`)를 `AgentRegistry.deliver` seam 에 넘겨 main 이면 메일박스, 에이전트면 그 inbox(항목 1개 = 런 1개). **종전엔 `_pending` 리스트 하나에 쌓고 main 의 턴 경계가 전부 drain 했다 — 주소가 없어 에이전트가 건 감시의 보고도 main 이 가져갔다**(요구가 안 된 게 아니라 정반대로 동작). 소유자별 drain 만 덧붙이면 main 이 빈 wake 턴을 무한히 도는 라이브락이 되므로, 질문·답·독촉이 이미 쓰던 주소 배달의 **네 번째 고객**이 되는 쪽을 택했다: 깨우기 술어도(`or monitors.has_pending()` 삭제 — 메일박스가 흡수하므로 `on_reply` 가 이미 깨운다) 두 번째 배달 지점도(`AgentLoop._deliver_monitor_reports` 삭제) `LoopConfig.monitor_registry` 포트도(#1 버그가 났던 그 파라미터) 전부 사라졌다. owner 는 `LoopPorts.owner`→`LoopConfig`→`RunContext.owner`→`MonitorTool` 로 흐르고, 중첩 seam 7곳 + 루프 밖 호출자 2곳이 **기본값 없이** 물려준다(기본값을 두면 상주 에이전트 안의 스킬이 조용히 main 으로 등록된다). 수명: `drop_owner` 를 워커 `finally`(정본) + `kill`/`shutdown_all` 의 `stop_event.set()` **앞**(창 좁히기)에서 부르고, `teardown_session(monitors=)` 가 `stop()`+`closed` 로 폴링을 멈춘다(전체 드롭이 아니다 — `_save` 가 살아 있는 행만 써서 `monitors.json` 을 비우면 "이전 세션 모니터" 통지가 사라진다). 배달 중(`_inflight`, `finally` 로 감소)은 생존 판정에 든다. **창의 `→ 보냄` 은 실제 배달만 (v9.20.1)**: `_handle_request` 의 out 레코드 `to` 는 라우팅 판정 **뒤** 실제 수신자다 — 받은 회신에 대한 산출물(`expects_reply=False`, 핑퐁 방지)과 질문을 건 런의 부분 결과(§3.7)는 빈 문자열. 종전엔 판정 전에 `to=author` 로 찍어 창이 "→ 보냄 · test (peer) · 회신했습니다" 를 그리고 conversation.jsonl 에도 남았는데 받는 쪽엔 아무것도 없었다(사용자 제보 tcx7hs). 프런트는 빈 `to` 를 "✓ 완료 · 배달 없음" 으로 그린다(`ovPeerInfo("")` 는 main 으로 읽으므로 먼저 가른다). 구 로그의 out 레코드는 그대로 `to=author` 라 resume 시 종전처럼 보인다. **큐가 아니라 메일박스인 이유**: `MailWaker` 가 큐에 넣는 건 상수 `WAKE_TEXT`(깨우기 마커)뿐이고 내용은 메일박스로 간다. 보고문을 큐에 태우면 `push_user_message` 를 타 **사람 메시지로 위장**되고(v9.17.0 까지는 `QUEUED_REQUEST_NOTICE` 까지 붙었다), `run` 은 턴 중 큐 주입을 안 해 배달이 런 종료 후로 밀리며, `--result-file` 이 모니터 보고에 대한 응답으로 덮인다. `tool="monitor"` 는 **등록 불필요** — `_classify_record` 가 `tool` 키로 관찰 판정, `is_format_intervention` 은 `tool==""` 만(그래서 빈 문자열 금지), 프런트는 `tool==="agent"` 만 특수 처리 → **새 SSE 이벤트도 새 카드도 없다**. 조건 3종(`type` 키 레지스트리, 추가=클래스 하나): `match`(파일+정규식, **등록 시점 EOF 부터**, 축소·`st_ino` 변화=커서 리셋 — 크기만 보면 logrotate rename 을 놓친다) · `silence`(`max(mtime, registered_at)` 기준 — mtime 만 쓰면 오래된/없는 로그에 거는 순간 즉시 발화) · `command`(**주기 실행**, exit 0 발화, stdout 이 보고 본문). **뺀 둘**: `exit`(= `shell` 에 `start_new_session` 이 없어 `&` 자식이 재부모화 → 비-부모는 `waitpid` 불가, 종료 코드 획득이 명세부터 불가능) · `interval`(주기 `command` 가 상위집합 — 내용까지 실어 와 턴 하나 절약). 액션 레지스트리 없음: 알림은 **암묵**이고 `shell` 은 선택 필드 `run="<cmd>"`(곱집합의 위험한 원소가 'notify 없는 shell' 이었다). 해제 3경로(once·deadline·max_wakes)가 **전부 마지막 보고를 남긴다**. 수명은 **두 곳**에 걸린다 — `_run_message_pump(monitors=)` + `web_instance_is_active(monitors)`(초판은 'web 은 무한 대기'라 후자를 빠뜨렸는데, `--idle-timeout` 이 모니터를 데리고 자기수확한다). 보안: `command`/`run` 은 **등록 시점**에 `shell.confirm_dangerous`+`_confine.guard` 를 **같은 함수로** 재사용(발화 시점이면 새벽 3시에 사람이 없고, 무엇보다 폴링 스레드가 `interactive_lock` 을 잡으면 교착). 영속은 `session_dir/monitors.json` 에 **쓰기만** 하고 `describe_previous()` 가 resume 때 알리기만 한다 — **부활 없음**: 커서가 낡아 되감아도 건너뛰어도 틀리고(어느 쪽이 맞는지 알 방법이 없다), 무엇보다 이 파일이 워크스페이스 안이라 에이전트가 고칠 수 있어 **아무도 승인하지 않은 `command`/`run` 이 되살아난다**(파일 안의 플래그는 '이 프로세스에서 사람이 답했다'를 증명하지 못한다). 재검증 매트릭스 자체가 사라진다.
│   ├── monitor_tool.py      (203, v9.11.0)  `monitor` 도구 — add/list/delete. **native 라 무조건 등록**(`schedule` 은 board 스케줄러의 얇은 클라이언트라 `AGENT_CLI_SCHEDULER=1` 게이트가 필요하지만, monitor 는 시간도 발화도 이 프로세스 안이다). `_ALL_TOOLS` **끝에** 붙여 KV 캐시 순서 보존. **도구 설명이 산출물**이다(35B 대상): `nohup CMD > log 2>&1 &`(2>&1 없으면 `shell` 이 120s 막힌다 — 실측) · `python -u`/`PYTHONUNBUFFERED` · `( CMD; echo "EXIT:$?" )` · board 면 `schedule` 유도 — 전부 TC 로 고정.
│   ├── _sensitive.py        (181, v9.10.0)  **민감 경로 사전** — 자격증명 *읽기*에 확인을 붙인다(봉쇄와 직교하는 축). `sensitive_reason(path) -> str|None` 이 사람이 읽을 이유를 돌려주고 그게 그대로 확인 창에 들어간다(`_confine` 이 "워크스페이스 밖"이라고만 말해 진짜 이유를 못 밝히던 실수 반복 회피). 세 종류: DIR 접두(`~/.ssh` `~/.aws` `~/.gnupg` `~/.config/gcloud` `~/.azure` `~/.kube` 키체인/키링 `~/.password-store` `~/.config/op`) · EXACT(`~/.agent-cli/config.json`=**이 도구 자신의 api_key**, `~/.netrc` `~/.npmrc`(홈만) `~/.config/gh/hosts.yml` `~/.cargo/credentials.toml` 셸/DB 히스토리 다른 AI 도구 토큰 `/etc/shadow` 계열) · BASE 파일명(`id_rsa`/`id_dsa`/`id_ecdsa`/`id_ed25519` — **`id_*` 아님**: `.pub` 까지 잡는다, `privkey.pem`, `ssh_host_*_key`, 바이너리 컨테이너 `*.p12`/`*.pfx`/`*.jks`/`*.keystore`/`*.keychain-db`). **의도적 제외**(정밀도 > 재현율 — 오탐 하나가 allow-all 피로를 학습시킨다): `.env`(에이전트는 보통 그 `.env` 를 가진 프로젝트를 작업 중) · `*.pem`(certifi·homebrew·node_modules 의 CA 번들) · `*.key`(macOS 에선 Keynote) · `~/.zshrc`(실제 벡터지만 PATH 디버깅으로 매일 읽음) · `~/.config/`·`~/.cargo/`·`~/Library/` 같은 **디렉터리**(툴체인·캐시 상시 읽기 — 안의 정확한 파일만 집는다). `canonical()` 정규화가 사전만큼 중요 — **실측 확인된 macOS 우회 셋**: `/etc`→`/private/etc` 심볼릭 · `/System/Volumes/Data/Users/x/.ssh` firmlink(`resolve()` 가 **접지 않는다** → 접두 제거) · 기본 APFS 대소문자 무시(darwin 은 casefold 비교). 소비자 둘: `_confine.guard`(shell·write·edit) + `read_file`(`check_workspace=False` 로 이 축만).
│   ├── _confine.py          (264)  **워크스페이스 경로 봉쇄** (default-on, `AGENT_CLI_WORKSPACE_CONFINE=0` 로 off). write_file/edit_file/shell 이 워크스페이스 루트(`os.getcwd()` 또는 `AGENT_CLI_WORKSPACE_ROOT`) *밖* 경로를 건드리면 `guard(paths, action)` 이 y/n/a 확인. **read_file 은 워크스페이스 봉쇄 안 함**(커널/드라이버가 밖 헤더·툴체인을 대량 read → 프롬프트 폭풍 방지) — 단 **민감 경로 사전(`_sensitive.py`)은 read_file 에도 건다**(v9.10.0, `guard(check_workspace=False)`): `cat ~/.ssh/id_rsa` 는 묻는데 도구로는 무음이면 일관성이 아니라 우연.  `resolve_within` 은 `expanduser()` + `Path.resolve()` 로 `~`·`..`·심볼릭링크 탈출까지 canonical 비교 — **`~` 확장은 v9.10.0 수리**(셸은 `shell=True` 라 `~` 를 실제 홈으로 확장해 실행하는데 `Path.resolve()` 는 `expanduser` 를 안 해 `<워크스페이스>/~/...` 로 풀렸고 **안에 있음**으로 판정 → `cp x ~/.ssh/authorized_keys` 가 무프롬프트 통과. `_path_candidate` 가 `~/` 를 후보로 잡아 놓고도 다음 단계가 무효화하던 소리 없는 구멍). `a` 는 해당 디렉토리 서브트리를 `_session_root_allowlist`(shell `_session_allowlist` 의 path 판)에 추가 → 이후 무프롬프트. shell 은 `extract_shell_paths` 로 절대경로·`../` 토큰을 **best-effort** 추출 — **2단계(v9.10.0)**: ① `_shell_segments` 로 배칭(`&& || | ;`, `;` 는 shlex 가 안 떼므로 별도 처리)을 가르고 `_can_mutate` 로 파일을 바꿀 수 있는 세그먼트인지 판정(`_READ_ONLY_CMDS` 에 없으면 **바꿀 수 있음** = fail-safe; 리다이렉션과 `sed -i`/`find -delete`/`sort -o` 플래그가 읽기 판정을 이긴다) → ② 그 세그먼트에서만 경로를 뽑되 `_IMPLAUSIBLE_IN_PATH`(`^ $ | {}` 만 — 공백·괄호·`+` 는 실제 경로에 나오므로 **일부러 제외**)로 정규식을 거른다. **`//` 주석**도 뺀다(v9.11.1, 사용자 제보 — `echo '// note' >> x.c` 가 리다이렉션이라 쓰기로 판정돼 주석 전체가 경로 후보였다): 단 `_is_line_comment` 는 **슬래시뿐**(`//`·`///`)과 **`//` 뒤 공백** 둘만 거른다 — `//etc/passwd` 는 셸이 실제로 `/private/etc/passwd` 로 풀어 쓰므로(실측) 접두를 통째로 버리면 조용한 우회가 된다. 남는 틈: `//주석`(공백 없이 붙인 주석)은 `//etc` 와 문자열로 구별되지 않아 여전히 후보 — 알고 남긴 틈이고 TC 로 고정했다. **민감 경로는 읽기 세그먼트에서도 뽑는다**(안 그러면 `cat ~/.ssh/id_rsa` 가 조용해진다). 종전엔 ②만 있어 `sed -n '/^start/,/^end/p'` 의 주소 정규식이 경로로 잡혀 엉뚱한 확인을 물었다(사용자 제보) — 오탐은 allow-all 피로를 학습시켜 게이트 전체를 죽이므로 이 위협 모델에선 정밀도 > 재현율. (shlex; `-I/usr/include`·`--dir=/p`·redirect `> /p` 커버, `$(...)`·`python -c "…"`·변수 `$FILE` 는 **의도적 blind spot** — 사고 방지 speed bump 이지 샌드박스 아님). confirm/`can_prompt`/`interactive_lock` 은 shell 위험가드와 동일 인프라 재사용; `can_prompt()` False → hang 대신 refuse. **shell 은 raw command 전달 → 확인 창에서 밖 경로 토큰 강조**: `guard(command=)` + `_outside_spans`(trailing-boundary 정규식·리터럴 위치, flag-attached `-I/path` 도 경로부만) → `confirm(command=, danger_spans=)` 위험-키워드와 동일 계약(write/edit=command 없음→프롬프트-only). 게이트 disabled·전부 내부 경로면 renderer import 없이 즉시 통과(zero-overhead).
│   ├── shell.py             (279)  셸 명령 실행 (**flat-native, Step 3**: 스키마 `{command, timeout?}` — `shell_` prefix 제거, `wrap_single_op`=identity; shell 이 마지막 flat 전환이라 이로써 **모든 builtin 도구 flat**) + 위험 명령 (rm/rmdir/mv) y/n/a 확인 (decision + 선택적 코멘트, env `AGENT_CLI_DANGEROUS_SHELL_CONFIRM=0`로 비활성) → ToolResult. **위험 키워드 강조**: `_detect_dangerous`(shlex 토큰 매칭)가 확인을 유발하면 `_danger_spans`(토큰-경계 정규식)가 강조할 `(start,end)` 문자 span 을 **origin 에서 단일 계산**해, 명령 원문·span 을 `confirm(command=, danger_spans=)` 구조화 필드로 전달(프롬프트 문자열에서 `$ cmd` 제거 → CLI 이중 표시 방지). 렌더러는 span 만 칠함 — CLI 볼드-레드, web `.danger` 스팬. 유발 토큰만 표시(따옴표 안·유사문자열은 span 비어 무강조). **위험가드 통과 후 `_confine.guard(extract_shell_paths(cmd), …)` 로 워크스페이스-밖 경로 게이트**(별개 축 — 파괴적이지 않아도 밖이면 물음). **프롬프트 가능 여부는 `get_renderer().can_prompt()` 로 판정** (구 `_is_tty()` 대체) — CLI는 TTY, web은 연결된 클라이언트(SSE+/api/input, TTY 불필요). 못 물어보면 hang 대신 명확한 refuse 에러. **확인 직렬화는 렌더 레이어의 공유 `interactive_lock`(RLock)** 사용 (confirm·ask 공통) — parallel delegate가 task별 워커 스레드로 돌기에 "한 번에 하나의 outstanding 프롬프트"를 보장해 응답이 물어본 워커로만 라우팅. shell은 락을 잡고 `_session_allowlist` 재확인 후 `renderer.confirm`을 호출(같은 스레드 RLock 재진입). `ask`(`_handle_ask`)도 동일 `can_prompt` 게이트 — 못 띄우면 `"(no response)"` 치환. 위험 확인 별칭: `y`(+yes/ok/okay/yep/yeah/sure), `a`(+always/**allow**), `n`(+no/nope) — 긍정 의도가 안전 기본값 deny로 오인되지 않게 확장(특히 프롬프트 라벨이 "always allow"라 `allow`→a). 출력은 잘리지 않고 그대로 LLM observation으로 전달 — 단 마스터 캡(`context_window//10`) 초과 시 `ShellTool.render_oversized` 가 **전체 출력을 `session_dir/shell-output-<hash>.txt` 에 lazy 저장(over-cap 때만)** 하고 그 파일을 가리키는 on-disk nudge(read/search·run 섹션 팬아웃)로 치환(headless=tee 폴백). **전체는 디스크에 보존·경로 announce** 라 옛 shell_artifact head/tail 의 silent 누락 문제(2026-05-19 제거 사유)와 무관 — 정보 유실 없이 컨텍스트만 보호. (이전 shell_artifact 가드는 head/tail 미리보기가 중간 디버깅 정보를 silent하게 누락시키는 사례로 제거.)
│   ├── fetch.py             (275)  웹 페이지 fetch → 마크다운 변환 → ToolResult (+ FetchTool). **flat-native (Step 3 완결, v4.38.0)**: 스키마 = flat `{url, depth?}` (required `url`) — 마지막까지 `fetch_` prefix 를 쓰던 builtin 이라 `claims`/`infer_action` 이 fetch 에만 live 였고 "모든 builtin flat-native" 불변식을 거짓으로 만들던 것을 flatten 으로 완결. `wrap_single_op`=identity; `key_prefix` 기본 유지 → 레거시 `fetch_url` emission(구 세션 재공급 prior)은 `strip_prefix` 가 계속 관용. **`render_oversized` override**: over-cap 내용을 `session_dir/fetch-output-<hash>.txt` 로 lazy 저장→가리키는 on-disk nudge(read range/search·run 섹션 팬아웃·더 좁은 URL/얕은 depth); headless/저장실패=제네릭 폴백
│   ├── agent_tool.py        (232)  **agent 도구 스키마 (5.0.0 통합 — 구 DelegateTool+TeammateTool)**: `RUN_DESCRIPTION`(v9.24.1 — 메인 `description`·`SUBLOOP_DESCRIPTION` 이 공유하는 run 문장: 이유는 동시성이 아니라 **분리된 컨텍스트**[기본 fresh, `context:"fork"` 면 부모 대화 복사], "속도를 위해 나누지 마라 — 로컬 단일 서버에선 처리량을 나눠 쓴다, 전부 끝날 때까지 블로킹"; 종전 "fan out … in PARALLEL" 은 속도로 읽혀 Harbor v4 extract-elf 에서 run 둘이 11+11 tok/s 로 나눠 쓰는 동안 부모가 10분 멈췄다), mode enum(run/spawn/request/status/resume/kill), C7 `validate`(mode 조건부 필수 — **v8.38.0 모드 테이블화**: `AGENT_MODES` 단일 테이블(required/engine/batchable)에서 스키마 enum·validate·`parallel_batchable`·tool_bridge 라우팅(engine="oneshot")·agents_live `_MODE_HANDLERS` 커버리지가 전부 파생, `depth_gated=True` 선언 — run/request:task(+key), resume/kill:key; **`message` 필드는 필드-정밀 에러로 거부** — A5 되먹임→자가 교정), 파라미터 profile(구 role/agent)·instructions(instant-agent — `compose_role_prompt` 합성, manifest 영속)·name·task·key·tools·context. **spawn 스케일링 가이드 (v8.1.0)**: description 에 컨텍스트-창 스케일링 논리(에이전트당 자기 창 → 팀 작업기억 N배·main 창 보호·전문 축적) + 조건('small one-off 는 run/직접이 쌈')·규율(ONE area/disjoint) 동봉 — 무조건 장려 금지 계약(TestSpawnScalingGuidance, SUBLOOP 미유출 포함). **지시 필드=전 모드 `task` 단일 (v8.0.0)**: 구 request 의 `message` 는 스키마에서 제거 — 모드별 이중 어휘가 spawn/resume 에서 지시를 조용히 유실시키던 '씹힘'의 근본 수리(v7.29.1 별칭은 이중 어휘 영구화라 철회, 사용자 결정=한 필드+시끄러운 교정). peer `message` **도구**(to/text)는 별개 표면 무변경. **`parallel_safe=True` + `parallel_batchable(args)`=mode=="run"** — mode-aware 배칭: 한 턴의 연속 agent op 이 전부 run 일 때만 병렬(`{tasks:[...]}` 조립→`_run_parallel`), 상주 모드 섞이면 순차. **`SUBLOOP_DESCRIPTION`** — 레지스트리 없는 루프(서브에이전트·headless)용 축소 설명(run 만 문서화; §3.2 모드 축소: 도구 인스턴스는 하나, 프롬프트 렌더만 분기 — 상주 모드는 tool_agent 가 "main 전용" 에러로 거부). **`render_oversized`**: run 결과 over-cap 시 `<run_dir>/result.md`(=artifact) 가리키는 on-disk nudge + re-run-narrower(구 DelegateTool 정책 이식). `touched_paths`=`<agent:key|profile>` 마커. 실행은 루프 인터셉트(tool_bridge `_invoke_agent` — run→oneshot 엔진 / 상주→agents_live.tool_agent)
│   ├── context.py           (417)  read_context 도구 — **history 를 SQL 로 질의**. 필터 파라미터 더미 대신 **단일 `query`(SQL SELECT)** 프리미티브: history.jsonl 을 인메모리 sqlite `history` 테이블로 온-디맨드 적재(컬럼 `session/loc/seq/kind/turn/ts/tools/files/author/text`)하고 LLM 이 SELECT 작성. `text` 컬럼이 각 레코드의 **전체 내용**(검색·읽기 표면). 컬럼은 **읽기 시점에 `manager._classify_record`(kind/tools/text) + `extract_file_paths`(files) 로 유도** → 어떤 레코드 shape 든 동작, prefix-관습 재추측 없음. `turn`/`ts`/`author` 는 레코드에서. SQLite 는 `code_index._sqlite` shim 경유 **lazy 로드**(stdlib→pysqlite3 폴백; **코어 도구라 sqlite 부재여도 모듈 import 안 깨짐** — 쿼리 시 친절 에러). **읽기전용**: 비-SELECT prefix 거부 + 인메모리 DB 콜마다 재빌드·폐기(쓰기 무해)가 1차 가드, sqlite authorizer(SELECT/READ 외 거부)는 belt-and-suspenders(상수 없는 pysqlite3 빌드면 skip). **결과는 행/셀 캡 없이 VERBATIM 반환**(이전 50행 cap + 200자 셀 절단·공백 collapse 제거 — read_context 가 청크 회수 시 내용을 망가뜨리던 버그 수정) — 결과 크기는 loop 의 과대 출력 캡이 관장하되 **`ReadContextTool.render_oversized`** 가 SQL-native nudge(LIMIT/projection/`substr(text,1,200)` 재쿼리 — `narrow_oversized_nudge`)로 치환, 모델은 그 안내대로 작게 유지. `query` 생략 시 스키마+예시+세션목록(discovery). `sessions`(current/all/<id>) = 테이블에 적재할 데이터 범위. `files` 컬럼은 다음 BM25(FTS5)와 같은 sqlite 기반.
│   ├── memory_tool.py       (158)  memory 도구 — `agent_cli.memory` 스토어의 LLM 인터페이스. flat-native `{mode,...}`(add/get/update/delete/list), `_run(args, session_dir=)` → `_dispatch`. `_as_id` 가 int·bare-int 문자열 수용(bool 거부). `MemoryError` → 친절 실패 ToolResult(크래시 아님). description 이 "실패=반복회피/발견=재사용, dead-end·비자명 학습 시 proactive 기록" 유도. session_dir=None → 명확 에러.
│   └── code_index.py        (745)  code_index 도구 — `agent_cli.code_index` 패키지의 native-tool wrapper. **flat-native (consolidation Step 3)**: `CodeIndexTool` 스키마 = flat 단일쿼리 `{mode, path?, name?, symbol_kind?, ref_kind?, search?, with_*?, depth?, max_bytes?}` (required `mode`) — `code_index_queries` 배치 배열·`code_index_` prefix 제거, `wrap_single_op`=identity. 한 op=한 쿼리; 여러 쿼리는 멀티-op 으로 code_index op 을 여러 개 emit(읽기전용이라 순서/상태 의존 없음 — read_file `reads[]` 와 동형). `_run(args)→_dispatch_one(args)` 직결. (옛 batch `tool_code_index`/`_format_batch` 는 read_file `_format_batch` 와 함께 제거.) `key_prefix` 유지(latent). `_dispatch_one`(per-query mode dispatch)는 유지: 10 mode dispatch (list/fetch/lookup/kind/file/refs/callers/callees/slice/build). 인덱스 root 자동 해석 (cwd 또는 가장 가까운 조상 `.agent-cli/`), lazy build + per-query incremental refresh. list/fetch는 root 바깥 path에 대해 on-demand parse fallback (DB 갱신 없음); 나머지 모드는 index-scoped (out-of-root 명시적 거부). fetch 결과는 hashline 포맷 → edit_file 직결. `post_hook(path)`는 edit_file/write_file 성공 직후 호출되어 자동 incremental refresh — 모든 예외 swallow (인덱싱 hiccup이 user-facing op 막지 않음). `_resolve_defs_path(root)`가 `<root>/.agent-cli/defconfig` 존재 시 `build(defs_path=...)`로 전달 — kernel/driver처럼 `#ifdef CONFIG_*` 가 함수 시그니처를 분기하는 코드에서 tree-sitter 파싱이 ERROR로 떨어져 정의가 누락되는 케이스를 unifdef 사전 분기 제거로 살림. 파일 부재 시 `None`이 그대로 통과해 기존 무전처리 동작 유지. 모듈 레벨 `_BUILD_LOCK` (threading.Lock) 이 `_ensure_index` / `post_hook` / `_do_build` 의 `build()` 호출을 직렬화 — 병렬 delegate worker 가 동시 진입해도 중복 빌드 없음. (atomic write 가 correctness 를 책임지고, 락은 효율 + SQLite 락 경합 회피 책임). **`CodeIndexTool.render_oversized`**: over-cap 결과는 index-native nudge(`mode=fetch` 단일 심볼·`search` 필터·`max_bytes`·특정 path/name 스코프 — `narrow_oversized_nudge`)로 치환(파일/팬아웃 아님 — 재쿼리가 정답).
│
...
│   ├── oneshot.py           (475) **run 모드 실행 엔진 (구 tools/delegate/exec.py 이동 — 함수명 tool_delegate/_run_single/_run_parallel 보존)**: `tool_delegate({tasks:[...]})` 진입 → 단건 `_run_single`(빈 task/provider 가드·사이클/깊이 체크·`run_{name}_{hash}_{ts}/` dir 명명·리포트 조립·instructions 합성 — 실행 몸통은 runner) / 복수 `_run_parallel`(worker 스레드·renderer begin/end_delegate_task lifecycle — **스레드 start 전에 스폰 스레드에서 `renderer.current_scope()`(부모)+`time.time()`(배치 시각)을 한 번 캡처해 모든 워커의 `begin_scope(parent=, ts=)` 로 전달**: 워커 스택은 비어 있어 자기가 skill 안에서 떴는지 모르고, 워커마다 시각을 찍으면 동시 팬아웃이 서수축에서 ms 차이로 여러 행에 흩어져 순차 실행처럼 보였다. 종료 시각은 워커별 유지 — 누가 오래 걸렸는지는 실제 정보). 워커 라벨 `agent:{profile}`
...
│   └── agents_live.py       (2352) **상주 에이전트 코어 (구 teammate.py — P1~P5+U 확장)**: `AgentInstance`(key·profile_name·합성 role_prompt·영속 ctx·inbox(SimpleQueue)·자기 stop_event·데몬 worker — 상태 전이 starting→idle→busy→…→dead 는 worker 루프 한 곳) + `AgentRegistry`(main 루프 수명 소유, spawn(profile=)/request/drain_replies/resume_agent/kill/shutdown_all + 회신 mailbox=Condition+list, `runner` DI seam, 동시 생존 상한 `max_agents`(인스턴스 필드, 기본 10, `clamp_max_agents`로 정규화 — 0=무제한; `set_max_agents`로 세션 한정 변경, `_at_agent_limit`가 spawn/resume 게이트, 낮춰도 기존 미살상; 웹 `GET/POST /api/max-agents`+sticky `broadcast_max_agents`, 5.16 — 구 env `AGENT_CLI_MAX_AGENTS` 제거) + `compose_role_prompt`(파일 본문→"## Additional instructions"→인라인 — instant-agent U4, manifest 영속로 resume/부활 동일 정체성) + `build_reply_record`(배달 레코드 — **`tool:"agent"`+additive `source:"agent_reply"`; `tool:""` 금지**(형식-개입 마커 오인 방지); over-cap 회신은 디스크 포인터 치환 — 전문은 `agents/<key>/replies/reply-<seq>.md` 영속. **v7.11.0 `registry=` 동봉**: 배달부(_deliver_agent_mail)가 넘기면 회신 말미에 배달-시점 잔여 상태 한 줄 — `working · N queued`+자동배달 안내(폴링 넛지 정합) 또는 `idle — ready`; question/died/peer·registry 미전달 경로는 무변경) + `tool_agent`(상주 mode 디스패치 — tool_bridge 인터셉트; **v8.38.0 모드 테이블화**: 종전 if-체인을 `_agent_<mode>` 핸들러 5개 + `_MODE_HANDLERS` 테이블로 — agent_tool.AGENT_MODES 의 registry 모드와 1:1(커버리지 테스트 고정), 에러 문구도 테이블 파생(신·구 출력 20케이스 바이트 동일 검증); 레지스트리 없으면 run 안내와 함께 거부=모드 축소) + `format_agent_label`("agt-x (coder · ui)") + **발신자 창 out 기록 (`_log_outbound`, v7.11.1 실사고 수리)**: `message_to_main`/peer request 발신이 mailbox·수신 inbox 만 채우고 발신 에이전트의 🤝 창(agent_message)·conversation.jsonl 을 건너뛰어 — main 챗 관찰로는 보이는데 대화창 무기록·resume 소실. 발신 시 out 방향 payload 로 표면+로그 동시 기록(수신측 in 은 request() 기존 담당 — 각 창=그 에이전트 관점 완결 대화). **★요청 화살표 ts 는 enqueue 전에 스탬프**: `request()` 는 `inbox.put` **직전** `send_ts` 를 잡아 in 메시지에 싣고, peer send 는 `handler` 가 `request()` 전 `send_ts` 를 잡아 `_log_outbound(ts=)` 로 넘긴다 — 워커가 큐잉 즉시 `begin_agent_work`(scope_start) 를 낼 수 있어, put 뒤에 ts 를 찍으면 팀 뷰에서 요청이 작업보다 뒤로 밀리기 때문. **에이전트↔에이전트 메시징 (5.11.0, 5.12.0 유일화)**: `message` 가상 도구(virtual.py `MessageTool`, 커널 기본 — message_handler 주입 루프에만 `tools_list` 강제 탑재/그 외 strip) → `_make_message_handler(tm)` → `request(to, text, author=f"agent:{tm.key}", expects_reply=True)`(대상 inbox) 또는 `message_to_main`(→`_push_reply` kind=`peer_message`→build_reply_record 관찰). `request` 에 `hop`·`expects_reply` 추가 — worker 회신 라우팅(5.12 단순화 3분기): `author="agent:X"` & expects_reply → `_deliver_peer_reply`(요청자 inbox 로 `expects_reply=False` **terminal** 재주입 + "계속 + 조건부 보고" 가이던스 꼬리표, `_MAX_PEER_HOPS` 안전망); expects_reply=False 는 소비만(핑퐁 방지, 데드락은 비동기라 불가); `author="main"` → `_push_reply`. registry 는 서브루프에 안 넘김(agent 상주 모드 차단 유지) — 대신 `message_handler`+미리 만든 `peer_agents_section`(`build_live_agents_section(exclude_key, via_message_tool=True)`)만 `_run_message`→run_loop→LoopConfig 로 배선. 메시징=유일 채널(명시 요청, 같은 inbox 배관 — 5.8.0 도구 이벤트 구독은 5.12.0 에서 완전 제거). 회신 라우팅: main 관찰 배달·질문은 main mailbox (D8 확장) + `MailWaker`(idle 자동 재기동 — mark_idle/on_mail/on_run_end/handle_dequeued, web 무의존; **`mark_idle`(5.18.1)**: 펌프가 큐 블록 직전에 호출 — idle set 후 미배달 회신이 이미 있으면 즉시 재무장해 `on_run_end()→idle.set()` 창에서 도착해 `on_mail` 이 드롭한 lost-wakeup 을 봉합. 정합성: idle 를 먼저 set 후 pending 확인 → 동시 도착분은 여기 has_pending 또는 idle 을 본 on_mail 중 최소 한쪽이 무장, append/has_pending 은 registry cv 로 직렬화. web 은 timeout 없는 무한 블록이라 이 봉합 없이는 회신이 영구 park, CLI 는 poll timeout 스핀으로 미배달) + `notify/consume_agents_changed`(멤버십 변화만 재조립 — KV 프리픽스 보호, 인스펙터 즉시 반영 `update_prompt_section`). worker 는 시작 시 `begin_prompt_scope(key)` 상시 스코프. **사람-직접 요청 drain-all 배치 (agent-channels C-1)**: worker 루프가 `_is_human_direct`(author `user:*`+expects_reply) 항목을 만나면 inbox 에 더 쌓인 사람-직접 항목을 `get_nowait` 로 모아 **한 턴에 배치**(`_handle_human_batch` — 메시지 라벨링만, 회신 1회→대화창만 ⑥; v9.22.0: 배치 안내문 `_AGENT_BATCH_NOTICE` 는 per-turn tail 의 `## Open Requests` 와 중복이라 제거) — main 의 drain-all(v8.16.1)과 대칭. main 위임/peer/배달회신은 회신 목적지가 달라 **개별 처리**(`_handle_request`, 종전 로직 그대로 추출), 배치 수집 중 만난 비-사람 항목은 1칸 stash 로 다음 이터레이션 이월(SimpleQueue put-front 부재). qsize>0 일 때만 수집하므로 단건 경로는 종전과 동일(회귀 안전). **ask→main 라우팅**: 상주 에이전트의 ask 는 **비동기 질문 포트**로 간다(v9.12, docs/agent-ask). 구 블로킹 경로(`_make_ask_handler` → `LoopConfig.ask_handler` → `_handle_ask(handler=)`)는 v9.14.0 C3 에서 **전부 제거** — 실값 생산자가 사라진 뒤로 테스트만 그 분기를 붙들고 있었다. **🤝 대화창 resume 복원 (5.13)**: `_log_conversation`/`_replay_conversation` — in/out/question 대화 메시지를 `agents/<key>/conversation.jsonl` 에 append(대화창의 **진짜 소스** — ctx=history 는 에이전트 내부 tool 작업까지 담아 대화창과 추상화 레벨이 달라 소스 부적합). `restore`(세션 resume) + `resume_teammate`(mode:"resume") 가 부활 후 재생 — 라이브와 같은 `agent_message` 표면 통과라 web replay 버퍼에 다시 쌓여 재접속 뷰어까지 복원되고 저장한 `ts` 로 원래 시각 유지. 재생 전 `clear_agent_conversation` 선행으로 멱등(비정상 사망 잔존 중복 방지). `kill` 은 `clear_agent_conversation`(replay 버퍼에서 그 key `agent_msg` 제거+count 보정, 라이브 `agent_cleared`)로 표면만 정리하고 jsonl 은 남김 — "kill=정리 / resume=재생" 대칭. **resume 재생성 (D7)**: `agents.json`(fsio 원자 교체, **구 teammates.json 레거시 읽기 없음**) = manifest(합성 role_prompt+profile+instructions 통째 — 프로파일 md 소실 무관) + 미배달 pending 미러. 부트스트랩이 `restore(parent_ctx)` 무조건 호출(fresh=no-op) + `auto_spawn()`(frontmatter `auto-spawn: true`, 동일 프로파일 dedup). **mode:"resume"**: dead 에이전트를 같은 key·fresh worker·ctx resume 모드로 부활(전 문답 기억·seq 이어가기·revivable 복귀). **대기 지시 일원화 (v7.9.0 status 넛지 → v7.11.2 ACK 전면)**: spawn(초기 task)/request/resume ACK 가 "폴링·재전송 금지, `complete` 로 턴 종료, 도착 시 기상"을 명시 — 실사고(spawn 후 main 이 status 폴링 4회+동일 요청 재전송 4회로 에이전트 방해) 대응. request ACK 는 적체 ≥2 면 큐 수와 간섭 경고 동봉. ★U-A "wait 유도 문구 소멸" 의 의도적 뒤집기(당시엔 MailWaker 부재로 wait 유도가 위험, 지금은 자동 기상이라 wait 가 정답 — 테스트에 기록). **status 대기 넛지 (v7.9.0)**: `format_status` 가 working/미처리 inbox 존재 시 말미에 "회신은 자동 배달·도착 시 깨움 — status 폴링 말고 complete 로 턴 종료" 힌트 — 모델이 status 폴링으로 회신을 기다리는 패턴 차단(전부 idle 로스터 조회엔 미부착). **worker 사망 통지 (Q4)**: 비정상 종료는 `kind:"died"`→`source:"agent_died"` 관찰 능동 통지·revivable=False; kill/세션종료는 통지 없음. **락 규율 (P0-9, v8.34.0)**: ⓐ seq 발급(`tm.queued`)은 `_cv`(RLock 기반) 아래 원자화 — 동시 request() 의 중복 seq(reply 파일 덮어쓰기·UI dedup 충돌) 봉합 ⓑ worker 가 dequeue 직후 즉시 `busy` 전이 — 분류/배치 수집 사이 창에서 idle-reap 의 any_activity() 오판(작업 중 세션 reap) 봉합 ⓒ MailWaker `_armed` 는 전용 Lock 으로 check-then-set 원자화(wake 유실→영구 park 봉합). **인간 개입 비배달 (D8)**: `tm.current_author`≠"main" 이면 회신·질문을 main mailbox 에 안 넣음 — 🤝 창(SSE)에만. 엔드포인트 `POST /api/agent/{key}/input`·`/resume`·`/kill`. 렌더러 표면 `agent_roster`(sticky)+`agent_message`(persistent, `to`·resume 재생용 `ts` 필드)+`clear_agent_conversation`(5.13 kill 정리 — 버퍼 selective 제거+`agent_cleared`)+`begin/end_agent_work(profile=)` **v9.6.0**: `_duplicate_label` — 같은 **표시 이름**(`(profile, name)` 쌍, 사람이 채널 칩에서 보는 것)의 개체가 있으면 spawn 을 **거절**한다. 죽은 개체도 센다(오히려 흔한 경로: 죽어 안 보이던 것을 모델이 새로 띄우고 → 사용자가 원본을 resume → 동명 둘). 키는 유일해 배달은 안전하지만 **사람이 글자로 구별할 수 없는 것 자체가 고장**이다. 종전의 사후 안내 ("NO memory … resume 이 맞았다")는 이미 벌어진 일을 되돌리라는 것이라 먹지 않았다 — 이름이 다른 다중 인스턴스에는 그 안내가 그대로 남는다. **ask 답변 페어링 (v9.12.0, docs/agent-ask/DESIGN.md)**: `ask` 는 종전 자기 inbox 에서 기다려 **도착 순서가 곧 답**이었다 — peer 의 무관한 메시지나 다른 주소의 새 일감이 답으로 먹혔고, 소비된 항목은 `_handle_request` 를 안 거쳐 **라우팅째 유실**됐다(peer 요청이면 요청자가 영원히 대기). 이제 답은 **슬롯**(`awaiting`/`awaiting_to`/`answer`/`answered` — 미결 질문이 언제나 0 또는 1 이라 큐가 아니다)으로 오고, 분류는 막혀 있지 않은 **생산자**(`submit()`)가 한다 (막힌 worker 는 `_worker→_handle_request→run_loop→dispatch→_op_ask` 콜스택 **아래**라 분류할 수 없다). 판정 축 `awaiting_to` 는 질문 페이로드가 이미 `to` 로 싣던 **주소**다 — 발명한 기준이 아니라 배달되지 않던 주소를 판정이 따르게 한 것. 규칙: 사람=**운영자**라 주소 무관 답변자("main 과 선착순"은 `agent_input` 이 명시한 기능이자 답변자 부재 시 탈출구) · main 은 주소가 자기일 때 · **peer 는 답변자가 아니다**(질문이 배달되지 않으므로 — 배달은 후속 단계). `submit() -> (error, verdict)` 가 배달 결과를 돌려줘 `_agent_request` 의 "답변으로 전달됨" 힌트가 **사실**이 된다(종전엔 `request()` 앞에서 state 를 읽는 TOCTOU); `request()` 는 얇은 래퍼로 남아 호출자 8곳 무변경(튜플은 항상 truthy 라 `if err:` 를 깬다). **arm 은 공개보다 먼저**(즉답이 inbox 로 새지 않게) 단 `state="waiting_ask"` 는 **공개 뒤에**(그 배리어를 깨면 로스터를 보고 메일박스를 읽는 쪽이 빈손). `kill`/`shutdown_all` 이 `answered.set()` 도 한다(슬롯 대기자는 inbox sentinel 로 안 깨어나 `join` 타임아웃). **`has_active_work` 가 막힌 슬롯 뒤의 큐를 제외** — 이 변경의 유일한 새 교착이었다(답 대기 중엔 inbox 를 못 꺼내는데 활동으로 세면 `run` 펌프가 영원히 돈다; 웹 `any_activity` 는 무변경). 주소가 peer 인데 운영자 창구도 없으면 **arm 하지 않고 즉시 반환**(`Renderer.can_answer_agent()` 기본 False·웹 True — `can_prompt()` 는 'main 프롬프트에 답이 언젠가 오나'라 재사용 불가). **남는 것**: 주소가 main 인 질문에 main 이 새 일감을 보내면 그대로 먹힌다(`author` 로는 '답'과 '새 일'을 구별 불가) — `mode:"answer"`+거부가 필요한 별도 단계. **비동기 질문 목록 (1단계, docs/agent-ask/DESIGN.md 3판)**: 위 슬롯 방식(블로킹)을 대체할 코어 — 아직 **호출자가 없어 동작 변화 0**이다. `Question`(id·asker·target·text·**asked_seq**·**delivered_seq**·nags) + `AgentRegistry._questions`(전 접근 `_cv` 아래 — 답 claim 이 원자적이어야 동시 답변자 둘 중 하나만 답이 된다) + `QuestionPort`(루프가 닿는 **유일한 seam** — 레지스트리를 넘기면 "teammate 안 teammate 금지" 가드가 깨지므로 포트 객체 하나; main 도 `question_port(None)` 으로 받는다) + `LoopConfig.questions` 배선(`handler_resources["questions"]` — `MessageTool` 과 같은 `requires_handler` 선언 방식). 불변식: **질문의 주소 = ask 시점의 `current_author` = 원 요청자**이므로 답 배달은 `submit(q.asker, body, author=q.target, expects_reply=True)` 한 줄이고 기존 `_handle_request` 라우팅이 제자리로 보낸다(origin 필드 불필요). **두 seq 가 런 스코프를 만든다** — 질문은 등록 즉시 목록에 오르지만 상대 inbox 에서는 줄을 서므로, 이 비대칭을 안 보면 상대가 꺼내지도 않은 질문에 강제·sweep 이 걸려 **바쁜 peer 에게 묻기가 상시 오동작**한다(`delivered_seq` 는 `_handle_request` 가 항목의 `question_id` 로 찍고, `asked_seq` 는 §3.7 회신 억제가 읽는다). 사람 주소(`user`/`user:{nick}`)는 배달할 inbox 가 없어 트레이가 표면이고, **`user*` 는 하나의 주체**라 답변자를 문자열 동치로 검사하면 두 번째 뷰어의 답이 거부된다. 사망 정리는 **양방향**(앞으로 온 질문은 사유와 함께 닫아 asker 를 풀고, 그가 건 질문은 폐기) 이되 **`not tm.revivable` 아래에서만** — `_worker` finally 는 `shutdown_all` 에서도 돌아, 거기서 지우면 직후 `_save_state` 가 빈 목록을 저장해 resume 이 알릴 열린 질문이 항상 0건이 된다. 영속은 `agents.json` 의 `questions` 배열이고 resume 은 **되살리지 않고 `stale_questions` 로 건수만** 알린다(되살려도 asker 의 런이 없어 답을 받을 주체가 없다). main 이 asker 면 `submit` 대상이 없으므로(`_agents` 에 main 슬롯 없음) 답도 메일박스 `kind:"answer"`→`source:"agent_answer"` 로 간다. **회신 계약 (v9.21.0)**: `complete` 은 국소 — 런 출력은 자동 배달되지 않는다(종전 `_handle_request` 의 peer/main 에코 삭제). 요청 항목(`expects_reply=True`)은 **빚**: `AgentInstance.current_expects_reply/current_answers/replied_this_run` 을 런 시작에 세팅하고, `QuestionPort.reply(text)`(새 도구 `reply`, `requires_handler=message_handler` 라 상주 전용)가 갚는다(`expects_reply=False` 배달, 빚 없으면 거부 — 이미 갚은 경우와 요청 아닌 런을 다른 문구로). `message` 는 언제나 요청(`expects_reply=True`)이되 `to` 가 내 요청자면 `replied_this_run` 에 넣어 빚도 갚는다. 런 끝에 `owed`(요청·사람 아님·질문 안 걸음·미상환)면 `_NO_REPLY_LABEL` 붙은 런 요약을 폴백 배달(peer=`_deliver_peer_reply`, main=`_push_reply(kind:reply, answers=스냅샷)`). 창 out 레코드는 실제 발신에만(폴백·user:*) — reply/message 는 보낼 때 `_log_outbound`. **빚 통일 (v9.22.0) → 장부 하나 (v9.22.1)**: 회신 빚은 레지스트리의 **주소별 장부** `_reply_debts[debtor][to] = ReplyDebt(to, text, answers, hop, settled)` 하나다 — `begin_run(addr)`/`note_owes(addr, to, text, answers=, hop=)`/`settle(addr, to)`/`reply_debt`/`reply_debts(addr)`(미상환·사람 주소 제외·이 런이 질문을 건 주소(`asked_targets`, v9.22.2 — 종전 bool 은 `ask(to="user")` 런의 main 빚까지 억제해 main 이 결과를 영영 못 받았다)의 빚 제외 §3.7)/`end_run(addr, output, **meta)`(폴백 배달 + `close_unanswered`). main 은 `_deliver_agent_mail` 이 배달된 `expects_reply` message 를 적고 `request(author="main")` 이 갚으며 `runtime.main_run_ended` → `end_run("main")`; 상주는 `_handle_request` 가 시작 항목을 적고(사람 주소도 적는다 — `reply` 거부 사유 "창을 보는 사람" 의 근거) `reply`/요청자에게 `message` 가 갚으며 런 끝에 `end_run(me, …)`. 종전엔 `AgentInstance` 필드 넷(current_expects_reply/current_text/current_answers/replied_this_run)과 main 의 dict 둘(`_main_reply_debts`/`_main_repaid`)이 같은 규칙을 두 벌로 들었고 `QuestionPort.debts()`/`reply_owed()` 가 `key is None` 으로 갈라 읽었다. `_main_repaid` 의 "같은 런에 이미 보냈으면 그 상대의 새 message 도 갚은 셈" 은 제거 — 갚은 뒤 온 `message` 는 새 질문이라 새 빚이다(`settled` 플래그로 "이미 갚았다" 사유만 남긴다). `QuestionPort.debts()` = `questions_owed_by(me)` + `reply_debts(me)` (owner 분기 없음). 루프는 **수신 유형**으로만 판정한다: 사용자 요청 → `answers` 정산 후 남은 요청 독촉(요청당 1회) / message·ask 미상환 → `_refuse_for_debts`(거부) 또는 `_nag_debts`(수락 후 독촉) 를 `MAX_DEBT_NAGS`(3)회까지 / 섞임 → 수락 → 정산 → 독촉. **거부/독촉의 갈림은 `LoopState.user_run`**(이 런에 `is_user` 메시지가 있었나 — `_add_user_message` 가 세운다; 상주 러너는 `query_author_is_user=bool(user_requests)` 로 넘겨 에이전트 항목 런은 사용자 런이 아니다)이지 소유자가 아니다(v9.22.1; 종전 `port.nonblocking` 분기는 근사였고 main 의 🤝 웨이크 런과 상주의 사람 창 런에서 틀렸다 — `TestRuleFollowsInboundTypeNotOwner`). `_debt_lines` 의 수단(`reply` vs `agent request`)만 도구 존재(`port.nonblocking`)로 고른다. 상한 뒤엔 런이 끝나고 `end_run` 이 정리한다(질문은 `_UNANSWERED_CLOSE` 로 닫음, `kill` 중이면 생략; 미상환 message 엔 `_NO_REPLY_LABEL` 런 요약 폴백 — main→peer 는 `request`, 상주→peer 는 `_deliver_peer_reply`, 상주→main 은 mailbox `kind:"reply"`). **런 블록 필드 (v9.23.0)**: `begin_agent_work(seqs=)` — 이 런의 inbox 항목 seq 들(`_handle_human_batch` 는 N개) 이 `scope_start.seqs` 로 나가 웹 블록의 머리를 정한다; 폴백 out 레코드는 `fallback: True` 를 실어(`conversation.jsonl` 에도) 재생이 같은 `⚠ 폴백` 줄을 그린다. **idle 은 정리 뒤 (v9.22.4)**: `_handle_request`/`_handle_human_batch` 는 out 왕래 줄 렌더 → `end_run` → `handled += n` → `_save_state` → `state = "idle"` 순이다 — 종전엔 idle 을 정리 앞에 세워 idle 을 기다린 관찰자(테스트 79곳·로스터)가 아직 안 그려진 out 줄·안 나간 폴백을 볼 수 있었다(Linux CI 낙방). `TestIdleMeansTheRunIsFullyDone` 이 `end_run` 시점의 `state == "busy"` 를 고정한다 — 런 뒤 알림 `remind_owed` 는 제거, `kick_owed` 는 resume 기상에만. **사람 창 요청도 회계** (④): `_handle_request`/`_handle_human_batch` 가 `user_requests=[{id:str(seq), author, text}]` 를 `run_subagent_message`→`run_loop`→`AgentLoop._setup` 으로 넘겨 `run_requests` 를 시드 — 에이전트 대화창 입력이 main 채팅과 같은 `answers` 규칙을 받는다(그 런의 `reply` 는 여전히 거부: 요청자가 창을 보는 사람). 거부는 ✗ 관찰 카드이고 그 관찰이 물린 complete 의 원문을 **인용**해 자기완결로 안내한다("이게 답이었다면 reply 로, 남은 일이 없으면 complete"); emission 은 저장하지 않는다(v9.21.1, 사용자 결정 — 남기면 `ops:[complete]` 가 history 에서 final 로 읽힌다). `answers` 되돌림도 같다(v9.21.2): 관찰이 결과 원문을 인용하고 emission 은 저장하지 않으며(`store_emission=False`), `recovery_kind="format"` 이라 성공 뒤 fold — 컨텍스트에는 성공 궤적만 남는다. v9.21.0 의 `rejected` 태그(records/web 재생 분기)는 원인이 사라져 제거. 무제한이면 아무것도 런을 못 멈춘다(개입은 max_turns 미계수, B1 은 도구 경로만).
├── skills/                         프롬프트 스킬 시스템
...
│   ├── executor.py          (304)  인자 치환 + 도구 교집합 + Role 상속 + skill subdir + stop_event
│   └── builtin/                    패키지 내장 스킬
...
│   ├── __init__.py          (1)
│   ├── config.py            (108)  mcp.json 로드/병합 (프로젝트 > 유저) **v9.3.0**: `is_streamable_http`/`is_remote` — MCP 2025-03-26 Streamable HTTP 전송 추가(별칭 `streamable-http`/`streamable_http`/`http`). `url` 있고 `transport` 생략 시 기본은 **종전대로 `sse`** — 바꾸면 기존 손편집 설정이 조용히 다르게 동작한다(마법사는 항상 명시 저장하므로 신규는 무관).
│   ├── client.py            (258)  McpClientManager (stdio/SSE 연결, 도구 호출, stderr 격리) **v9.2.0**: `status`/`configs`(마지막 connect_all 의 서버별 결과·설정 — 실패한 서버도 기억; 종전엔 stderr 한 줄로 흘려보내고 잊었다) + `summary()`(웹 🔌 칩용 스냅샷: 서버별 전송·연결·도구·실패 이유 + 집계) + `humanize_error(msg, cfg)`(연결 실패 원문 → 다음 행동이 보이는 문장 — 마법사에서 이관해 CLI 와 웹이 **같은 오류를 같은 말로**) + `_leaf_error`(anyio ExceptionGroup 을 잎까지 — "unhandled errors in a TaskGroup" 에 원인이 묻히던 것). **v9.3.0**: `_connect_streamable_http`(SDK `streamablehttp_client` — 사용자 제보로 드러난 공백: 구 sse 로 신규 서버에 붙으면 Accept 헤더 부족으로 -32600) + `_load_tools`(전송 3종 공용, 종전 복붙 2벌) + `connect_all(warn=)`(판별은 실패를 전제로 두 번 시도하므로 예상된 실패를 stderr 로 내보내지 않는다) + **`CancelledError` 포획**(BaseException 이라 `except Exception` 을 빠져나가 마법사가 통째로 터졌다 — 실장에서 잡힘; KeyboardInterrupt 는 여전히 전파) + `_drain`(streamable-http 는 내부 task group 을 써서 `__aexit__` 만으로는 취소가 전파될 틈이 없어 "Task was destroyed but it is pending!" 이 stderr 로 샜다).
...
│   └── wizard.py            (~430) **`agent-cli mcp` (v9.1.0, docs/mcp-ui 시안)**: 등록·진단 마법사 — `SetupWizard` 와 같은 구조(순수 헬퍼는 모듈 함수로 프롬프트 없이 테스트, `McpWizard` 는 대화만). 두 축: **저장 전 실제 연결**(`probe_server` — 도구 수 확인, 붙지 않는 설정은 기본적으로 파일에 안 남김) + **env 참조 사전 검사**(`env_ref_status` — `${VAR}` 미정의는 에러가 아니라 빈 문자열이 되어 인증만 실패하므로 저장 전에 보여줌). 프로브는 `_connect_with_timeout`(데몬 스레드 + join — `session.initialize()` 에 타임아웃이 없어 응답 없는 서버가 마법사를 영원히 세움; 매니저가 자기 루프를 들고 있어 asyncio 로 감싸면 루프가 겹치므로 스레드) 45s 상한(npx 첫 실행 = 패키지 설치 고려). 실장 검증에서 잡힌 것 둘: ① `list` 가 파이프 뒤에서 레거시 가져오기 **프롬프트에 걸려 멈춤** → 진단 명령은 `sys.stdin.isatty()` 일 때만 제안(`add` 는 항상) ② SSE 거부가 "unhandled errors in a TaskGroup" 으로 보임 → `client._leaf_error` 가 ExceptionGroup 을 잎까지 풀어 status·`[warn]` 에 실음(부팅 진단도 개선). 저장은 `.agent-cli/mcp.json` 하나(v9.0.0), `~/.agent-cli/mcp.json` 은 가져오기 제안 용도로만 조회(원본 보존). 진입점 `main.py` `mcp_app`(typer 서브그룹, `invoke_without_command` — 인자 없이 = 목록+진단, 실패 수를 exit code 로). 계약: `test_mcp_wizard.py`(헬퍼·프로브·스크립트 프롬프트 대화·CLI 배선) · `test_mcp.py::TestLeafError`. **v9.3.0 전송 자동 판별**: 선택지는 `stdio`/`remote` 둘 — 사용자가 **확실히 아는 것**(로컬이냐 원격이냐)만 묻고, HTTP 세대는 `_probe_remote_autodetect` 가 streamable-http → sse 순으로 붙여 보며 판별해 `entry["transport"]` 에 **명시 저장**한다(문서의 URL 만으로 세대를 아는 사용자가 드물어, 물으면 추측을 떠넘기고 틀리면 -32600 을 만난다; 마법사는 어차피 저장 전에 붙어 보므로 추가 비용이 사실상 없다). 둘 다 실패하면 각각의 이유를 보여주고 저장하지 않는다.
...
- **과대 도구 출력 캡 + nudge (loop `_tool_observation`, 결과→관찰 seam)**: 도구 관찰 토큰이 **`max(context_window, MIN_CONTEXT_WINDOW) // 10`**(loop `_oversized_cap`) 초과면 전체 출력을 컨텍스트·history 어디에도 안 넣고 **over-cap 관찰**로 치환 — 호출 자체는 성공. 캡 하한(`MIN_CONTEXT_WINDOW//10`)은 capabilities 미탐지/`context_window=0` 때 캡이 0(=무제한)으로 꺼지던 구멍을 막는다(하한값이 새 매직넘버가 아니라 "에이전트가 기동조차 안 하는 최소 윈도우"에서 파생돼 자기일관적). **정책은 단 하나** (`Tool.render_oversized` — v8.42.0 일원화; **어떤 도구도 오버라이드하지 않는다**): ① 전문은 **항상 파일로** — 도구가 이미 쓴 파일(`oversized_source_path`: read_file=원본 경로·agent run=`<run_dir>/result.md`) 아니면 `persist_oversized()` 가 `<session_dir>/oversized/<tool>-<sha1>.txt` 로 lazy 저장(세션 없으면 tempdir 폴백 — "넘치면 파일에 남는다"가 예외 없이 성립), ② **head/tail 발췌**(`oversized_excerpt`, cap×15% 클램프·tail 60% 가중 — 빌드/테스트 로그는 답이 꼬리에 있어 왕복 1회를 아낌; 병적으로 긴 단일 라인도 char 예산으로 하드슬라이스), ③ **회수 경로 3종**(regex `read_file(search=)` / 라인범위 / **분할정복 팬아웃** — 섹션 크기를 `cap×0.6` 목표로 산정해 각 섹션이 실제로 캡에 들어가게 하고(구 clamp-8 은 섹션이 여전히 over-cap 이라 한 단계 아래서 같은 벽에 부딪혔다), 상한 16 에 걸리면 "각 서브에이전트가 더 좁히라"고 명시; `agent ∈ tools_available` 일 때만), ④ **근본 원인 한 줄** = **`Tool.oversized_retry_hint`** — 도구별로 다른 유일한 부분(shell=명령에 grep/head/tail, read_file=search/범위/code_index fetch, fetch=좁은 URL·얕은 depth, agent=좁은 task, code_index=단일 심볼·search·max_bytes, read_context=LIMIT/projection/substr; 미선언 도구=`GENERIC_RETRY_HINT`). 종전의 빌더 3종(`default_`/`on_disk_`/`narrow_oversized_nudge`)+도구 오버라이드 6개는 이 하나로 흡수 — MCP 등 신규 도구가 아무 것도 선언하지 않아도 올바르게 동작하고, 파일조차 안 남기던 code_index·read_context 가 순수 개선됐다. per-result `body`/`tokens` 는 명시 인자, per-call 컨텍스트는 **`ctx`(`RunContext`)**(`session_dir`·`oversized_cap`·`tools_available`; loop `_run_ctx()` 단일 조립점, 실행 seam `run`/`_run` 과 동형). **턴 단위 예산 (`_turn_obs_budget` = cap × `TURN_OBS_BUDGET_MULT`(3), `ToolBridge.accumulate_observation`)**: 캡은 op 단위라 각각 캡 바로 아래인 op 이 N 개면 합계 N×cap — 10개로 윈도우가 찬다(그리고 시스템 프롬프트는 독립 read 를 한 턴에 몰라고 권장한다). 누적이 예산을 넘으면 **그 이후** op 의 관찰이 같은 정책으로 치환되고(앞쪽 op = 모델이 먼저 요청한 것이 원문 유지), nudge 의 `reason` 이 "이 결과가 컸다"가 아니라 "이 턴이 예산을 넘었다"고 정직하게 말한다(고치는 법이 다르므로). 각 누적 엔트리가 자기 토큰비용을 들고 있어 러닝합은 O(N). **`run_skill` 도 이 seam 을 지난다** (v8.42.0 — 종전 유일한 우회로: 스킬은 서브루프 전체 출력을 반환해 단일 관찰 중 가장 커질 수 있는데 무제한이었다). loop 가 `messages.append`·`ctx.add` **양쪽 전에** 최종 본문을 만들므로 일관 → **`add` 는 순수 저장**. 비-도구 관찰(개입·ask/message)·사용자/어시스턴트 메시지는 캡 대상 아님(ask/message 는 `accumulate_raw` 로 턴 예산에는 계상). 관찰 렌더는 `_append_observation` 단일 지점, multi-op 은 flush 합본 1카드. 스킬 템플릿의 `` !`cmd` `` 주입은 이 seam 밖이라 자체 상한(`_INJECT_MAX_CHARS`=4000, head+tail 보존)을 가진다.
- **경로 표기 규칙 (v8.48.0)**: 프로세스 cwd 는 수명 내내 고정이고(`os.chdir` 부재, 서브프로세스 `cwd=` 인자 부재) 파일 도구가 전부 `Path(path)` 라 상대경로는 cwd 기준으로 풀린다 — 즉 상대/절대가 같은 파일을 가리키고, `_confine` 은 상대경로를 워크스페이스 안으로 간주해 통과시키므로 상대 쪽이 마찰도 적다. 그런데 프롬프트가 `Working directory:` 만 알려주고 규칙을 안 줘서 실측 세션에서 **절대 23 : 상대 2** 였다. 경로는 `action_input` 으로 매 턴 재공급되고 compaction 파일 목록에도 누적되므로 비용이 반복 청구된다. ① `_build_environment_section` 이 규칙을 명시(도구 인자=상대, 예외는 작업 디렉토리 밖 파일과 최종 답변의 사용자 안내), ② 루프도 같은 규칙을 지킨다 — `tools/base.display_path()` 가 over-cap 넛지의 경로를 cwd 상대로 줄인다(작업 디렉토리 밖인 headless tempdir 폴백은 절대 유지). 넛지 하나에 경로가 3~4회 등장하므로 절약이 배가되고, 무엇보다 루프가 절대경로를 뱉으면 over-cap 마다 프롬프트 규칙의 **반대를 가르치는** 셈이 된다.
- **세션 상태 블록 (프롬프트 꼬리, v8.46.0 — `prompts/session_state.py`)**: 매 턴 바뀌는 휘발 상태(컨텍스트 사용량·턴 예산·`## Live Agents`·`## Session Memory`·압축 임박 경고)를 **마지막 메시지 뒤**에 붙인다. **왜 꼬리인가**: 프로바이더는 프롬프트 접두사를 캐시하고 첫 불일치 토큰에서 재사용이 끊기므로 위치가 곧 비용이다. ① 시스템 프롬프트에 두면 그 섹션 이후 전부(남은 시스템 프롬프트 + **대화 전체**)가 무효화 — `memory add` 한 번이 전체 재프리필이었고, 이 압박 때문에 로스터가 busy/idle 을 못 실었고 `_build_environment_section` 이 날짜를 뺐다. ② "사용자 요청 뒤"는 에이전트 루프에서 프롬프트 **앞쪽**(요청 1개 뒤로 N턴이 자람)이라 직관과 반대로 최악. ③ 꼬리는 비용 0 — 턴 N 이 `… obs_{N-1} + STATE_N` 로 끝나고 턴 N+1 은 `… obs_{N-1}, assistant_N, obs_N + STATE_{N+1}`, 접두사 일치는 `obs_{N-1}` 에서 끝나는데 이는 새 토큰 때문에 **어차피 거기서 끝났을 자리**다(손실 = 이전 블록 자신, 대화 길이와 무관한 상수). recency attention 이 가장 강한 위치이기도 하다. **전달**: 새 메시지 추가가 아니라 마지막 메시지 본문에 append — 프로바이더가 `messages` 를 verbatim 전송해(`providers/anthropic.py:97`) 끝에 `role=user` 를 더하면 연속 동일-롤이 되고 처리가 프로바이더마다 갈린다. `_OBS_COMPLETE_NUDGE` 와 같은 seam(`ContextManager.get_messages` 꼬리 주석 블록, copy-before-mutate)이며 **history.jsonl 에 절대 미영속** (영속하면 낡은 수치가 매 턴 재공급 + resume 프리뷰 오염). **조립**: `LLMCaller._build_session_state` — 턴 카운터(state)·레지스트리(config)·예산이 다 거기 있어서. `ContextManager` 는 `set_session_state()` 저장만 한다. 블록은 요청에 들어가지만 `system` 이 아니라 **`_state_tokens` 로 압축 예산에서 별도 예약**(직전 턴 크기 — 정확한 수치는 `ensure_within` **후**에 만들어야 하는데 예산은 그 전에 필요하므로). 경고는 예산의 `COMPACTION_WARN_RATIO`(0.75)에서 발화하고, 문구는 조기 `complete` 유발을 피하려 "여유 없음"이 아니라 **행동 지시**(지금 memory 에 남겨라 / 끝낼 이유 아니다)다. **동반 제거**: 로스터가 매 턴 재조립되므로 `notify_agents_changed`·`consume_agents_reload`·`_membership_changed`·`Renderer.update_prompt_section`(+ web `prompt_changed` 이벤트와 프론트 중계)이 전부 소멸 — 남겨뒀다면 스냅샷에 없는 "Live Agents" 를 **삽입**해 LLM 이 받지 않은 유령 섹션을 인스펙터에 만들었을 것. `memory`/`teammates` 재조립 트리거도 제거되어 세션 중 시스템 프롬프트를 바꾸는 소스는 **DIRECTIVE.md 뿐**이다.
- **Prompt Inspector 이중 계상 제거 (v8.46.0)**: `build_inspector_sections` 가 압축 요약·touched-file 목록을 `"⊙ … (user-injected)"` **가짜 시스템 섹션**으로 합성해 `note_system_prompt` 로 보냈는데, 이 둘은 실제로는 `role=user` 메시지라 `_dynamic_context_sections`(= `ctx.get_messages()` 순회)가 **이미** 내보내고 있었다 → `/api/debug/prompt` 가 같은 내용을 두 번 반환하고 `est_tokens`/`total_chars` 를 중복 합산했다. 합성을 지우니 함수가 항등이 되어 통째로 삭제; 루프는 `prompt.sections` 를 그대로 넘긴다. 부수 효과로 인스펙터는 꼬리 주입물(세션 상태 블록 포함)을 **자동으로** 올바른 위치에 보여준다 — `get_messages()` 를 읽기 때문.
...
**DelegateResult 필드**: `output`, `duration_secs`, `activity_log`, `last_actions`, `iterations` (`iterations` = 루프가 실제로 돈 턴 수 `ctx.current_turn`, v9.24.1 — 20줄로 잘린 활동 로그의 줄 수를 세던 종전 값은 60턴을 "20 iterations" 로 보고했고, 기록 기반 계산은 압축·재시도 미기록에 흔들린다)
...
### 7.3½ stop_reason 정규화 (P0-1, v8.34.0)
...
Anthropic budget 상한 32768 은 "이 이상은 배치 처리 권장" 하는 자연 상한 — 256K 컨텍스트에서 `budget + max_output` 여유 충분.

**게이트 준수 (v8.21.1)**: 런타임 오버라이드는 `supports_thinking`을 **존중**한다 — 미지원 모델(`supports_thinking=False`)이면 양 provider 모두 기본값·오버라이드(`enable_thinking` 포함)를 **일절 주입하지 않는다**. 비추론 모델에 `reasoning_effort`/`thinking` 블록/`chat_template_kwargs`를 보내 엄격 백엔드(공식 OpenAI/Anthropic)에서 400 나는 것을 방지. 이에 맞춰 web UI 도 `GET /api/thinking` 의 `supports_thinking`(runtime capabilities 노출)이 False 면 사고/노력 셀렉트를 잠근다(`setSupported`).
...
build_system_prompt(capabilities, active_tools, skill_stack, session_id, agent_role, agent_registry, ...)
    ...
    │
    ├─ TASK_GUIDELINES (작업 원칙 13개 — **v8.52.0 부터 세션-상태 꼬리 블록으로 렌더 이동**, 상수는 여기 단일 소스로 유지; v8.51.0 에서 벤치 실측 2개 흡수: ① 입력 파일을 변경/소비할 수 있는 명령 전 원본 백업 — Harbor tb21 db-wal-recovery 에서 백업 없는 sqlite3.connect 가 WAL 을 소비하는 실수 2/2 재현, ② 긴 심사숙고 대신 작은 검증 스텝 — 사고 폭주로 시간 예산 소진 2/2 재현. 벤치 템플릿이 아니라 제품 프롬프트에 두는 이유: 측정 대상은 agent-cli 자체여야 하고(벤치 템플릿의 태스크 유래 힌트는 오버피팅), 일반 원칙은 모든 사용자에게 유효)
...
**실브라우저 테스트 (`tests/browser/`, v7.12.0):** playwright + 헤드리스 chromium 으로 jsdom 유닛이 원리적으로 못 잡는 부류 — origin 당 6연결 고갈, secure context, CSS 렌더링(칩 세로 클리핑·flex ellipsis), 실 SSE 타이밍, 서버↔프런트 실계약 — 를 검증 (confirm-starvation 사가의 프런트 버그 4건이 전부 이 층에서만 잡힌 실증). `conftest.py::WebStack`=실 uvicorn 위 WebRenderer+WebServer(포트 0 임시할당) + worker 헬퍼(`start_confirm_loop`/`start_ask`, teardown 이 `push_abort` 로 블록된 worker 를 풀어 모듈-전역 interactive_lock 해제 — 테스트 간 간섭 방지). 시나리오: confirm 클릭·해결·comment·다중뷰어 409 fold, ANSWERING 왕복, 부재 중 pending replay, 헤더 칩 가시성·복사·ctx 팝오버, stall 경고 표출/정리. **카드 렌더 회귀 (`test_card_rendering.py`, v8.42.3)**: ① 관찰 헤더의 `<span class="icon">` 마크업이 `el()`(textContent)에 실려 화면에 태그가 문자로 노출(`<span class="icon">✓</span> shell`)된 실사고 — v8.36.0 el/elHtml 분리 때 놓친 콜사이트, `elHtml` 로 수리 + 아이콘이 **요소**로 렌더되는지·도구명 주입이 여전히 막히는지 DOM 으로 검증. ② 공백 없는 긴 절대경로가 `.action-detail` 박스를 가로로 넘던 것(뮤테이션 실측 **185px** 넘침) — `overflow-wrap:anywhere` 수리 + `scrollWidth<=clientWidth` 기하로 검증(짧은 경로 대조군 포함). **el() 오용 자동 탐지 (`test_web_server.TestStaticUI.test_no_markup_passed_to_text_el`)**: 소스를 파싱해 `el(` 호출 인자에 마크업 리터럴/HTML-생성 함수 결과가 있으면 실패 — 종전엔 콜사이트 전수를 손으로 감사해 이 버그를 놓쳤으므로 기계적 가드로 승격. **성능 회귀 가드 + KPI (`test_render_perf.py`, v8.42.2)**: 세션 로드(스냅샷 재생)의 렌더 병합을 **하드웨어 무관한 카운팅 불변식**으로 단언 — 240이벤트를 재생해도 `#overview` innerHTML 재작성·`#messages` scrollTop 쓰기가 프레임 수준 상수(실측 2회/1회, 상한 20; 병합 풀리면 121회/240회)여야 하고, 동시에 카드 수·개요 최신성·자동 스크롤 바닥 도달(기능 계약)과 **라이브 경로**도 함께 검증(스냅샷만 고치는 반쪽 회귀 방지). 계측은 `Element.prototype` 의 innerHTML/scrollTop setter 카운팅(MutationObserver 는 마이크로태스크 배칭으로 **과소계상** → 회귀를 놓치는 방향이라 미사용). 뮤테이션 검증: v8.41.0 app.js 로 되돌리면 3/3 실패. 벽시계 시간은 단언하지 않고 `TestRenderKpi`(`AGENT_CLI_PERF_KPI=1`)가 측정만 해 `docs/PERF-KPI.md` 표로 추세 관리 — 절대 임계는 머신 종속이라 느린 CI 오탐 ∧ 빠른 머신 무력의 이중 실패이기 때문. 정적 소스 핀 `test_web_server.TestRenderCoalescing`(직접 호출 부활 금지)과 상호 보완. **옵트인**: `AGENT_CLI_BROWSER_TESTS=1` 아니면 루트 conftest `collect_ignore=["browser"]` 로 **수집조차 안 함**(per-item skip 만으로는 pytest-asyncio 가 수집 단계 이벤트루프를 남겨 이후 async e2e 를 깨뜨림 — 실측 수리). CI 는 별도 `browser` 잡(chromium install). **omlx 통합 테스트:** `tests/test_integration_omlx.py` + `tests/test_integration_omlx_builtin.py`. 실 OpenAI 호환 omlx 서버를 대상으로 `run_loop`(질문/read/shell/write/edit/multi-step), `provider.call` ReAct 파싱, 런타임 capability 감지, 스킬 실행(fork·dynamic injection·allowed_tools·디렉토리 구조·bracket args), 훅(Pre/PostToolUse), agent run(none/fork), code-analyst 프로파일, plan 스킬, @<profile> dispatch를 검증. conftest fixtures(`omlx_provider`, `integration_model`, `model_capabilities`)는 서버 `/v1/models` 프로브로 가용성을 확인하고, 미가용 시 전부 skip → `pytest tests/`는 항상 green. 연결은 env(`OMLX_BASE_URL` 기본 `http://127.0.0.1:8000/v1`, `OMLX_API_KEY`, `INTEGRATION_MODELS` 기본 `Qwen3.6-27B-MLX-8bit`)로 override. 순수 로딩/프롬프트 검증은 유닛(test_builtin_skills/agents)에 있어 통합에서는 제외.

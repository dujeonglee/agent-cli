...
│   ├── delegate/            (952 총, v4.44.0 C2 패키지化) in-process 서브에이전트 패키지 — loop/ 와 동형 패턴, `__init__` 재수출로 기존 import 표면 무변경. 테스트 monkeypatch 는 사용 서브모듈 patch(_run_single·_load_agent→exec). `_reset_agent_loader`(테스트 전용 전역 mutator)는 prod 삭제 — 테스트가 `agents._agent_loader` 직접 교체 + conftest autouse 스냅샷/복원(구식부터 있던 순서-의존 누수 클래스 근본 수리).
...
│   │   ├── exec.py          (474)  실행 엔진: _run_single(config 병합·가드·run_loop)·_run_parallel(worker 스레드·renderer lifecycle)·tool_delegate 진입

...
그 vocab 과 strip 구현이 4곳(providers/base ①②, react ①②, md_array ③
...
통합한다. 층은 그대로 둔다 — 경로마다 지키는 것이 다르기 때문:
  ...
  - md_array (③): 고아/트레일링 태그 — 발생 위치가 포맷의 수리
...
문자열 값 안의 고아 태그는 보존된다 (md_array 계약 테스트로 고정).
"""
...
# ③ 고아 태그 낱개 (여닫이 무관) — thought 산문 청소용 (md_array sanitize).

...
# dump every line back into context (that would defeat edit_file's cost model).
# Past this, the model is told to read_file for the rest — mirrors
# ``_diff.MAX_DIFF_LINES``.
...
def _updated_region_echo(original_text: str, result_text: str) -> str:
    """Fresh ``LINE#HASH`` tags for the regions this edit changed (± context).

    Lets the model chain a follow-up ``edit_file`` on what it just edited
    WITHOUT a ``read_file`` round-trip: the diff shows WHAT changed, this shows
    the new hashline refs for the changed region at correct POST-edit line
    numbers (via :func:`format_hashlines_range`). Overlapping/adjacent windows
    merge; total emitted lines are capped at ``_MAX_REGION_LINES``. Empty
...
def apply_edits_batch(path: str, edits: list[dict]) -> ToolResult:
    ...
    if diff:
        msg += "\n\n" + diff
    region = _updated_region_echo(original_text, result_text)
    if region:
        msg += f"\n\n{_REGION_HEADER}\n{region}"
